...
public class SourceLocationMapper {
  ...
  public static class OriginLineDescriptor extends FileNameDescriptor
    ...
    public OriginLineDescriptor(
        Optional<String> pOriginFileName,
        int pOriginLineNumber,
        boolean pMatchBaseName) {
  ...
  private static void collectLine(
      final SortedSet<Integer> target,
      final FileLocation loc,
      boolean overApproximateTokens) {
      ...
      switch (edge.getEdgeType()) {
        ...
        default:
          throw new AssertionError(
              "Unhandled edge type in switch statement: " + edge.getEdgeType());
      }
    }
  ...
  public static synchronized Set<FileLocation> getFileLocationsFromCfaEdge(CFAEdge pEdge) {
    ...
    result =
        FluentIterable.from(result).filter(Predicates.not(Predicates.equalTo(FileLocation.DUMMY)))
            .toSet();
