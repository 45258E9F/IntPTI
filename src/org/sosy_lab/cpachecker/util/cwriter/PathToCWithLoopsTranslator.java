...
public class PathToCWithLoopsTranslator extends PathTranslator {
  ...
  public static Appender translateSinglePath(ARGPath pPath, CFA cfa, Configuration config)
    ...
    LoopCollectingEdgeVisitor loopCollectingVisitor =
        new LoopCollectingEdgeVisitor(cfa.getLoopStructure().get(), config);
  ...
  private Appender createNonUniqueFunctions(Appender app) {
    ...
    for (Pair<CFunctionEntryNode, FunctionBody> p : nonUniqueFunctions.values()) {
      ...
      body.write(recreateFunction(p.getFirst(), p.getFirst(), p.getFirst().getExitNode(),
  ...
  protected String startFunction(
    ...
    if (!mFunctionDecls.contains(lFunctionHeader + ";")) {
      ...
      if (uniqueFunction.matcher(lFunctionHeader).matches()) {
        ...
        nonUniqueFunctions.put(lFunctionHeader,
            Pair.<CFunctionEntryNode, FunctionBody>of(functionStartNode, newFunction));
      }
    }
  ...
  private final void processEdge0(
    ...
    if (edge instanceof CFunctionCallEdge) {
      ...
      if (uniqueFunction.matcher(callStack.peek()).matches()) {
        currentFunction
            .write(processSimpleEdge0(edge, currentFunction.getCurrentBlock(), childElement));
      }
    }
  }
  ...
  private String processSimpleEdge0(
    ...
    if (loopsAfter.isEmpty() || !loopsInPathToRecreate.get(loopsAfter.get(loopsAfter.size() - 1))
        .contains(state)) {
  ...
  private Pair<String, CFAEdge> recreateIf(
    ...
    CFANode ifEnd = findEndOfBranches(singletonList((CFANode) entryNode.getExitNode()),
        pCFAEdge.getPredecessor(), branch1.getSuccessor(), branch2.getSuccessor());
  ...
  private Pair<String, CFAEdge> recreateLoop(
    ...
    while ((!edgesToHandle.isEmpty() || !outOfLoopEdgesStack.isEmpty() || !ifStack.isEmpty())) {
      ...
      if (leaving.isEmpty()) {
        ...
        handledFunctions
            .add(((CFunctionEntryNode) realLeavingEdge.getSuccessor()).getFunctionName());
...
//    assert ifOutLabelEnd.isEmpty() && loopStack.isEmpty();
   ...
   * @param suffix       the label to jump to, may be the empty string (in most cases it is) but not
   *                     null
    ...
    switch (edge.getEdgeType()) {
      ...
      case FunctionCallEdge: {
        ...
        CFunctionEntryNode entryNode =
            ((CFunctionSummaryEdge) ((FunctionCallEdge) edge).getSummaryEdge()).getFunctionEntry();
        ...
        if (!nonUniqueFunctions.containsKey(functionHeader)) {
          nonUniqueFunctions
              .put(functionHeader, Pair.of(entryNode, new FunctionBody(-1, functionHeader)));
        }
   ...
   *                                 point. E.g. the function start or the node from which the two
   *                                 branches are leaving
  ...
  private CFANode findEndOfBranches(
    ...
    return findEndOfBranchesNonRecursive(elseNode, thenNode, functionStart,
        intermediateFunctionEnds.get(0));
  }
  ...
  private CFANode findEndOfBranchesNonRecursive(
      ...
      CFANode head,
      CFANode bottom) {
    Set<CFANode> foundNodes = CFATraversal.dfs().backwards().ignoreFunctionCalls()
    ...
    if (foundNodes.contains(branch1) && foundNodes.contains(branch2)) {
      ...
      while (!branchStack.isEmpty()) {
        ...
        if (CFATraversal.dfs().backwards().ignoreFunctionCalls()
            .collectNodesReachableFromTo(branch1, head).contains(branch2)) {
          return branch1;
        }
