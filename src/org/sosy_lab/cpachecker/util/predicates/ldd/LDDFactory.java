...
class LDDFactory {
  ...
  public LDD createIfThenElse(LDD condition, LDD positive, LDD negative) {
    int ldd = JLDD.Ldd_Ite(this.lddManagerPtr, condition.getLddPtr(), positive.getLddPtr(),
        negative.getLddPtr());
    return createLDD(ldd);
  }
   ...
   */
  public LDD makeConstantAssignment(
   ...
   * @param varCoeffs  the variable coefficients.
   * @param varCount   the number of total variables.
   * @param leq        if <code>true</code>, the term uses less or equal, otherwise it uses less
   *                   than.
   * @param constValue the constant value.
  ...
  public LDD replace(
    ...
    int linearTerm = this.theory.create_linterm(array.cast(), varCount);
    int ldd =
        JLDD.Ldd_SubstTermForVar(this.lddManagerPtr, previous.getLddPtr(), varIndex, linearTerm,
            constant);
    return createLDD(ldd);
  }
