...
public class NestedInterpolation<T> extends AbstractTreeInterpolation<T> {
  ...
  public List<BooleanFormula> getInterpolants(
    ...
    List<BooleanFormula> interpolants =
        Lists.newArrayListWithExpectedSize(formulasWithStatesAndGroupdIds.size() - 1);
    ...
    for (int positionOfA = 0; positionOfA < formulasWithStatesAndGroupdIds.size() - 1;
      ...
      lastItp = getNestedInterpolant(formulasWithStatesAndGroupdIds, interpolants, callstack,
          interpolator, positionOfA, lastItp);
    }
    return interpolants;
  }
  ...
  private BooleanFormula getNestedInterpolant(
    ...
    try (final InterpolatingProverEnvironment<T> itpProver = interpolator.newEnvironment()) {
      ...
      for (Triple<BooleanFormula, AbstractState, T> t : Iterables
          .skip(formulasWithStatesAndGroupdIds, positionOfA + 1)) {
