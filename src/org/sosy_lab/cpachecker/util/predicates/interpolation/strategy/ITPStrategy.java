...
public abstract class ITPStrategy<T> {
   ...
   *
   * @param interpolator                is the interface towards the SMT-solver and contains an
   *                                    ITP-solver with all formulas asserted on its solver-stack.
   * @param formulasWithStateAndGroupId is a list of (F,E,T) where the path formula F starting at an
   *                                    abstract state E (abstraction state?) corresponds with the
   *                                    ITP-group T. We assume the sorting of the list matches the
   *                                    order of abstract states along the counterexample.
   ...
   * @param interpolants                   computed with {@link #getInterpolants} and will be
   *                                       checked.
   */
