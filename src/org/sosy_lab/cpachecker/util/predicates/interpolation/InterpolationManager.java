...
public final class InterpolationManager {
  ...
  public void printStatistics(StatisticsWriter w0) {
    w0.put("Counterexample analysis",
        cexAnalysisTimer + " (Max: " + cexAnalysisTimer.getMaxTime().formatAs(TimeUnit.SECONDS)
    ...
    if (cexAnalysisGetUsefulBlocksTimer.getNumberOfIntervals() > 0) {
      w1.put("Cex.focusing",
          cexAnalysisGetUsefulBlocksTimer + " (Max: " + cexAnalysisGetUsefulBlocksTimer.getMaxTime()
    ...
    if (reuseInterpolationEnvironment && satCheckTimer.getNumberOfIntervals() > 0) {
      w1.put("Reused formulas on solver stack",
          reusedFormulasOnSolverStack + " (Avg: " + div(reusedFormulasOnSolverStack,
              satCheckTimer.getNumberOfIntervals()) + ")");
    }
  ...
  public InterpolationManager(
    ...
    if (itpTimeLimit.isEmpty()) {
      ...
      executor =
          Executors.newSingleThreadExecutor(Threads.threadFactoryBuilder().setDaemon(true).build());
    }
   ...
   *                           path. The first state (root) of the path is missing, because it is
   *                           always TRUE. (can be empty, if well-scoped interpolation is disabled
   *                           or not required)
    ...
    try {
      ...
      if (maxRefinementSize > 0) {
        ...
        if (size > maxRefinementSize) {
          logger.log(Level.FINEST, "Skipping refinement because input formula is", size,
              "bytes large.");
      ...
      try {
        try {
          return currentInterpolator
              .buildCounterexampleTrace(f, pAbstractionStates, elementsOnPath, computeInterpolants);
        } finally {
          if (!reuseInterpolationEnvironment) {
        ...
        try (ProverEnvironment prover = solver
  ...
  private List<Triple<BooleanFormula, AbstractState, Integer>> orderFormulas(
    ...
    ImmutableList<Triple<BooleanFormula, AbstractState, Integer>> result =
        direction.orderFormulas(traceFormulas,
            pAbstractionStates,
            variableClassification,
            loopStructure,
            fmgr);
    assert traceFormulas.size() == result.size();
    assert ImmutableMultiset
        .copyOf(from(result).transform(Triple.<BooleanFormula>getProjectionToFirst()))
   ...
   * to be unsatisfiable.
   *
   * @param pInterpolator                  The references to the interpolation groups, sorting
   *                                       depends on the solver-stack.
   * @param formulasWithStatesAndGroupdIds list of (F,A,T) of path formulae F with their
   *                                       interpolation group I and the abstract state A, where a
   *                                       path formula F is the path before/until the abstract
   *                                       state A. The list is sorted in the "correct" order along
   *                                       the counterexample.
  ...
  private Path formatFormulaOutputFile(String formula, int index) {
    return fmgr
        .formatFormulaOutputFile("interpolation", cexAnalysisTimer.getNumberOfIntervals(), formula,
            index);
  }
  ...
  public class Interpolator<T> {
    ...
    private final List<Triple<BooleanFormula, AbstractState, T>> currentlyAssertedFormulas =
        new ArrayList<>();
    ...
    private CounterexampleTraceInfo buildCounterexampleTrace(
      ...
      assert pAbstractionStates.size() == f.size()
      ...
      try {
        ...
        formulasWithStatesAndGroupdIds = new ArrayList<>(
            Collections.<Triple<BooleanFormula, AbstractState, T>>nCopies(f.size(), null));
     ...
     *
     * @param traceFormulas                  The list of formulas to check, each formula with its
     *                                       index of where it should be added in the list of
     *                                       interpolation groups.
     * @param formulasWithStatesAndGroupdIds The list where to store the references to the
     *                                       interpolation groups. This is just a list of
     *                                       'identifiers' for the formulas.
