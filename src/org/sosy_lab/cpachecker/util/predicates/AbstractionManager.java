...
public final class AbstractionManager {
  ...
  public AbstractionPredicate makePredicate(BooleanFormula atom) {
    ...
    if (result == null) {
      ...
      logger.log(Level.FINEST, "Created predicate", absVar, "from variable", symbVar, "and atom",
          atom);
  ...
  /**
   * Calculates the partition(s) the new predicate is similar to and inserts the new predicate. If
   * there is no similar partition found, a new partition containing the new predicate will be
   * created. If there is more than one partition similar to the new predicate, the similar
   * partitions are merged together and the new predicate is inserted into the partition that
  ...
  private void updatePartitions(AbstractionPredicate newPredicate) {
    ...
    if (predVars.isEmpty()) {
      ...
      if (!varIntersection.isEmpty()) {
        ...
        predVarsCoveredByPartition
            .addAll(partitionIDToPredVars.get(firstPartition.getPartitionID()));
        ...
        while (varIterator.hasNext()) {
          ...
          predVarsCoveredByPartition
              .addAll(partitionIDToPredVars.get(nextPartition.getPartitionID()));
        }
      } else {
        firstPartition = createNewPredicatePartition();
      }
