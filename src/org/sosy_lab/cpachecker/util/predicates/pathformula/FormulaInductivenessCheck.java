...
public class FormulaInductivenessCheck {
   ...
   *
   * @return A new version of the formula that only contains current variables (no variables with
   * outdated SSA indices), or "true" if the formula is not inductive.
   */
  ...
  private boolean isInductive(
      CFANode pNode, Set<String> pOutVariables,
  ...
  /**
   * @return whether {@code formulaSlice} is inductive under {@code edge}: that is, whether {@code
   * formulaSlice /\ edge /\ NOT formulaSlice} is unsatisfiable.
   */
  ...
  /**
   * @return strongly connected set of edges (for each edge {@code e} in the returned set, there
   * exists a path from {@code node} to {@code e} and from {@code e} to {@code node}.
   */
