...
public class SSAMap implements Serializable {
  ...
  public static class SSAMapBuilder {
    ...
    // Do not update without updating varsHashCode!
    ...
    public int getFreshIndex(String variable) {
      return freshValueProvider
          .getFreshValue(variable, SSAMap.getIndex(variable, vars, ssa.defaultValue));
    }
    ...
    public SSAMapBuilder setIndex(String name, CType type, int idx) {
      Preconditions
          .checkArgument(idx > 0, "Indices need to be positive for this SSAMap implementation:",
              name, type, idx);
      int oldIdx = getIndex(name);
      Preconditions
          .checkArgument(idx >= oldIdx, "SSAMap updates need to be strictly monotone:", name, type,
              idx);
  ...
  public SSAMap withDefault(final int defaultValue) {
    return new SSAMap(this.vars, this.freshValueProvider, this.varsHashCode, this.varTypes,
        defaultValue);
  }
  ...
  /**
   * @return index of the variable in the map, or the [defaultValue].
   */
  public int getIndex(String variable) {
