...
public class CToFormulaConverterWithArrays extends CtoFormulaConverter {
  ...
  public CToFormulaConverterWithArrays(
  ...
  protected CRightHandSideVisitor<Formula, UnrecognizedCCodeException> createCRightHandSideVisitor(
    ...
    return new ExpressionToFormulaVisitorWithArrays(this, fmgr, machineModel, pEdge, pFunction,
        pSsa, pConstraints);
  }

  @Override
  protected Formula buildLvalueTerm(
      CLeftHandSide pExp, CFAEdge pEdge, String pFunction, SSAMapBuilder pSsa,
      PointerTargetSetBuilder pPts, Constraints pConstraints, ErrorConditions pErrorConditions)
      throws UnrecognizedCCodeException {

    return pExp.accept(new LvalueVisitorWithArrays(this, pEdge, pFunction, pSsa, pPts, pConstraints,
        pErrorConditions));
  }
...
//    }
  ...
  private <TI extends Formula, TE extends Formula> ArrayFormula<TI, TE> makeArrayVariable(
      String pName,
      CType pType, SSAMapBuilder pSsa, boolean bMakeFresh) {

    if (bMakeFresh) {
      int freshIndex = makeFreshIndex(pName, pType, pSsa);
      return (ArrayFormula<TI, TE>) fmgr
          .makeVariable(this.getFormulaTypeFromCType(pType), pName, freshIndex);
    } else {
      int useIndex = getIndex(pName, pType, pSsa);
      return (ArrayFormula<TI, TE>) fmgr
          .makeVariable(this.getFormulaTypeFromCType(pType), pName, useIndex);
    }
  }

  @SuppressWarnings("unchecked")
  protected <TI extends Formula, TE extends Formula> ArrayFormula<TI, TE> makeAssignedArrayVariableForStore(
      String pName,
  ...
  @SuppressWarnings("unchecked")
  protected <TI extends Formula, TE extends Formula> ArrayFormula<TI, TE> makeAssignedArrayVariableForEquivalence(
      String pName,
      CType pType, SSAMapBuilder pSsa) {
  ...
  protected BooleanFormula makeAssignment(
    ...
    if (pLhs instanceof CIdExpression
      ...
      logger
          .logOnce(Level.WARNING, "Result might be unsound. Aliasing of array variables detected!",
      ...
      final ArrayFormula<Formula, Formula> unchangedArrayFormula =
          makeAssignedArrayVariableForStore(
              //Overwrite makeVariable so that it uses a cache of formulas??
              arrayVariableName,
      ...
      //    (= A2 (store A1 IX FR))
      final ArrayFormula<Formula, Formula> changedArrayFormula =
          makeAssignedArrayVariableForEquivalence(
              arrayVariableName,
              lhsArrExpr.getExpressionType(),
              rhsSsaBuilder);
      ...
      return super
          .makeAssignment(pLhs, pLhsForChecking, pRhs, pEdge, pFunction, pSsa, pPts, pConstraints,
              pErrorConditions);
    }
  }
}
