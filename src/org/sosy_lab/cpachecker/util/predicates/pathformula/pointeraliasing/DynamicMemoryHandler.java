...
class DynamicMemoryHandler {
  ...
  Value handleDynamicMemoryFunction(
      final CFunctionCallExpression e, final String functionName,
    ...
    if ((conv.options.isSuccessfulAllocFunctionName(functionName) ||
      ...
      return Value
          .ofValue(handleSucessfulMemoryAllocation(functionName, e.getParameterExpressions(), e));
  ...
  private Formula handleMemoryAllocation(
    ...
    if (functionName.equals(CALLOC_FUNCTION) && parameters.size() == 2) {
      ...
      CBinaryExpression multiplication = builder.buildBinaryExpression(FileLocation.DUMMY,
      ...
      if (value0 != null && value1 != null) {
        long result = ExpressionValueVisitor.calculateBinaryOperation(
            new NumericValue(value0.longValue()), new NumericValue(value1.longValue()),
            multiplication,
   ...
   */
  private Formula handleSucessfulMemoryAllocation(
  ...
  private Value handleMemoryFree(
    ...
    if (errorConditions.isEnabled()) {
      final Formula operand =
          expressionVisitor.asValueFormula(parameters.get(0).accept(expressionVisitor),
      ...
      for (String base : pts.getAllBases()) {
        Formula baseF =
            conv.makeConstant(PointerTargetSet.getBaseName(base), CPointerType.POINTER_TO_VOID);
  ...
  private Formula makeAllocation(final boolean isZeroing, final CType type, final String base)
    ...
    if (isZeroing) {
      AssignmentHandler assignmentHandler =
          new AssignmentHandler(conv, edge, base, ssa, pts, constraints, errorConditions);
  ...
  static String makeAllocVariableName(
      final String functionName, final CType type,
   ...
   *
   * @param type        the revealing <em>pointed</em> type (e.g. {@code char} for {@code char *})
  ...
  private CType refineType(
      ...
      final @Nonnull CIntegerLiteralExpression sizeLiteral) {
    ...
    if (type instanceof CArrayType) {
      ...
      return new CArrayType(false, false, type,
          new CIntegerLiteralExpression(sizeLiteral.getFileLocation(),
  ...
  private CType getAllocationType(
      final @Nonnull CType type,
      final @Nullable CIntegerLiteralExpression sizeLiteral) {
  ...
  private void handleDeferredAllocationTypeRevelation(
      final @Nonnull String pointerVariable,
      final @Nonnull CType type)
      throws UnrecognizedCCodeException, InterruptedException {
    final DeferredAllocationPool deferredAllocationPool =
        pts.removeDeferredAllocation(pointerVariable);
  ...
  private void handleDeferredAllocationPointerEscape(final String pointerVariable)
    ...
    final CIntegerLiteralExpression size = deferredAllocationPool.getSize() != null ?
                                           ...
                                           new CIntegerLiteralExpression(
                                               ...
                                               CNumericTypes.SIGNED_CHAR,
                                               BigInteger
                                                   .valueOf(conv.options.defaultAllocationSize()));
  ...
  private void handleDeferredAllocationsInAssignment(
    ...
    for (final Map.Entry<String, CType> usedPointer : rhsUsedDeferredAllocationPointers
        .entrySet()) {
      ...
      if (CExpressionVisitorWithPointerAliasing.isRevealingType(usedPointer.getValue())) {
        ...
        assert CExpressionVisitorWithPointerAliasing.isUnaliasedLocation((CExpression) rhs)
            : "Wrong assumptions on deferred allocations tracking: rhs " + rhsExpression
            + " is not unaliased";
        ...
        assert rhsUsedDeferredAllocationPointers.size() == 1
            :
            "Wrong assumptions on deferred allocations tracking: rhs is not a single pointer, rhsUsedDeferredAllocationPointers.size() is "
                + rhsUsedDeferredAllocationPointers.size();
        ...
        if (lhsType.equals(CPointerType.POINTER_TO_VOID) &&
          ...
          assert CExpressionVisitorWithPointerAliasing.isUnaliasedLocation(lhs)
              : "Wrong assumptions on deferred allocations tracking: lhs " + lhsLocation
              + " is not unaliased";
          ...
          final Map.Entry<String, CType> lhsUsedPointer =
              !lhsUsedDeferredAllocationPointers.isEmpty() ?
              lhsUsedDeferredAllocationPointers.entrySet().iterator().next() :
              null;
          assert lhsUsedDeferredAllocationPointers.size() <= 1
              :
              "Wrong assumptions on deferred allocations tracking: lhsUsedDeferredAllocationPointers.size() is "
                  + lhsUsedDeferredAllocationPointers.size();
          assert (lhsUsedPointer == null ||
              (lhsLocation.asUnaliased().getVariableName()).equals(lhsUsedPointer.getKey()))
              : "Wrong assumptions on deferred allocations tracking: lhs " + lhsUsedPointer
              + " does not match rhs " + rhsExpression;
          ...
          pts.addDeferredAllocationPointer(lhsLocation.asUnaliased().getVariableName(),
              usedPointer.getKey());
          passed = true;
          handled = true;
    ...
    for (final Map.Entry<String, CType> usedPointer : lhsUsedDeferredAllocationPointers
        .entrySet()) {
      ...
      if (CExpressionVisitorWithPointerAliasing.isRevealingType(usedPointer.getValue())) {
        ...
        assert lhsUsedDeferredAllocationPointers.size() == 1
            :
            "Wrong assumptions on deferred allocations tracking: lhs is not a single pointer, lhsUsedDeferredAllocationPointers.size() is "
                + lhsUsedDeferredAllocationPointers.size();
        if (!passed) {
          // e.g. __tmp = 0;
    ...
    for (final Map.Entry<String, CType> usedPointer : usedDeferredAllocationPointers.entrySet()) {
      if (!usedPointer.getValue().equals(CPointerType.POINTER_TO_VOID)) {
        ...
        switch (binaryExpression.getOperator()) {
          ...
          case LESS_EQUAL:
          case LESS_THAN:
            final CType operand1Type =
                CTypeUtils.simplifyType(binaryExpression.getOperand1().getExpressionType());
            final CType operand2Type =
                CTypeUtils.simplifyType(binaryExpression.getOperand2().getExpressionType());
            CType type = null;
  ...
  void handleDeferredAllocationInFunctionExit(final String function) {
    SortedSet<String> localVariables =
        CFAUtils.filterVariablesOfFunction(pts.getDeferredAllocationVariables(), function);
