...
public interface PointerTargetSetBuilder {
  ...
  void addDeferredAllocationPointer(
      String newPointerVariable,
  ...
  Iterable<PointerTarget> getMatchingTargets(
      CType type,
      PointerTargetPattern pattern);

  Iterable<PointerTarget> getSpuriousTargets(
      CType type,
      PointerTargetPattern pattern);
  ...
  final static class RealPointerTargetSetBuilder implements PointerTargetSetBuilder {
    ...
    private static final Function<Pair<CCompositeType, String>, Triple<CCompositeType, String, CType>>
        typeFieldFunction =
        new Function<Pair<CCompositeType, String>, Triple<CCompositeType, String, CType>>() {
          @Override
          public Triple<CCompositeType, String, CType> apply(Pair<CCompositeType, String> field) {
            ...
            for (final CCompositeTypeMemberDeclaration declaration : fieldComposite.getMembers()) {
              if (declaration.getName().equals(fieldName)) {
                return Triple
                    .of(fieldComposite, fieldName, CTypeUtils.simplifyType(declaration.getType()));
              }
            }
            throw new AssertionError(
                "Tried to start tracking for a non-existent field " + fieldName +
    ...
    private static final Comparator<Triple<CCompositeType, String, CType>>
      ...
      @Override
      public int compare(
          Triple<CCompositeType, String, CType> field1,
    ...
    public BooleanFormula prepareBase(final String name, CType type) {
      ...
      final BooleanFormula nextInequality =
          ptsMgr.getNextBaseAddressInequality(name, bases, lastBase);
      bases = bases.putAndCopy(name, PointerTargetSetManager.getFakeBaseType(
          ptsMgr.getSize(type))); // To prevent adding spurious targets when merging
      lastBase = name;
...
//                                  "The base should be prepared beforehead with prepareBase()");
      ...
      if (type instanceof CElaboratedType) {
        assert ((CElaboratedType) type).getRealType() == null : "Elaborated type " + type
    ...
    public boolean tracksField(final CCompositeType compositeType, final String fieldName) {
      return fields
          .containsKey(CompositeField.of(CTypeUtils.typeToString(compositeType), fieldName));
    }
      ...
      if (cType instanceof CElaboratedType) {
        ...
        for (final CCompositeTypeMemberDeclaration memberDeclaration : compositeType.getMembers()) {
          ...
          if (isTargetComposite && memberDeclaration.getName().equals(memberName)) {
            targets = ptsMgr.addToTargets(base, memberDeclaration.getType(), compositeType, offset,
     ...
     * actual pointer target. This can happen if we try to track a field of a composite that has no
     ...
     * grouping the requested fields into chunks by their nesting and avoid optimizations when
        ...
        if (!done.contains(field)) {
          ...
          do {
            ...
            for (final Triple<CCompositeType, String, CType> parentField : typedFields) {
              if (!done.contains(parentField) && parentField.getThird()
                  .equals(current.getFirst())) {
                current = parentField;
                break;
              }
            }
    ...
    public void addDeferredAllocationPointer(
        final String newPointerVariable,
      ...
      for (final String pointerVariable : newDeferredAllocationPool.getPointerVariables()) {
        deferredAllocations =
            deferredAllocations.putAndCopy(pointerVariable, newDeferredAllocationPool);
      }
    ...
    public DeferredAllocationPool removeDeferredAllocation(final String allocatedPointerVariable) {
      final DeferredAllocationPool deferredAllocationPool =
          deferredAllocations.get(allocatedPointerVariable);
    ...
    public boolean isTemporaryDeferredAllocationPointer(final String pointerVariable) {
      final DeferredAllocationPool deferredAllocationPool =
          deferredAllocations.get(pointerVariable);
      ...
      return deferredAllocationPool != null && deferredAllocationPool.getBaseVariables().get(0)
          .equals(pointerVariable);
    }
  ...
  public static enum DummyPointerTargetSetBuilder implements PointerTargetSetBuilder {
    ...
    public void addTemporaryDeferredAllocation(
        boolean pIsZeroing,
        CIntegerLiteralExpression pSize,
        String pBaseVariable) {
      throw new UnsupportedOperationException();
    }

    @Override
    public void addDeferredAllocationPointer(
        String pNewPointerVariable,
        String pOriginalPointerVariable) {
      throw new UnsupportedOperationException();
    }
