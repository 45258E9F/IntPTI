...
public class CTypeUtils {
   ...
   * encoded as a bunch of variables.
   *
  ...
  /**
   * <p> The method returns the type of a base variable by the type of the given memory location.
   * </p> <p> Here we need special handling for arrays as their base variables are handled as
   * pointers to their first (zeroth) elements. </p>
   *
  ...
  static boolean isCompositeType(CType type) {
    ...
    assert !(type instanceof CCompositeType)
  ...
  /**
   * <p> The method should be used everywhere the type of any expression is determined. This is
   * because the encoding uses types for naming of the UFs as well as for over-approximating
   * points-to sets (may-aliases). To make the encoding precise enough the types should correspond
   * to actually different types (requiring explicit casts to be converted to one another), so
   * {@link CCompositeType}s, corresponding  {@link CElaboratedType}s and {@link CTypedefType}s
   * shouldn't be distinguished and are converted to the same canonical type by this method. </p>
  ...
  /**
   * The method is used in two cases: <ul> <li> by {@link CToFormulaConverterWithPointerAliasing#getUFName(CType)}
   * to get the UF name corresponding to the given type. </li> <li> to convert {@link CType}s to
   * strings in order to use them as keys in a {@link PathCopyingPersistentTreeMap}. </li> </ul>
   *
   * @param type The type
