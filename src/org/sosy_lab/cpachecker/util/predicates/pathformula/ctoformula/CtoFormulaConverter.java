...
public class CtoFormulaConverter {
  ...
  public CtoFormulaConverter(
      FormulaEncodingOptions pOptions, FormulaManagerView fmgr,
    ...
    this.nfmgr =
        fmgr.getIntegerFormulaManager(); // NumeralMgr is only used for String-Literals, so Int or Real does not matter, however Princess only supports Int.
...
//      : "Saving variables with mutliple types is not possible!";
  ...
  protected Formula makeCast(
      final CType pFromType, final CType pToType,
  ...
  private Formula encodeOverflowsWithUF(
    ...
    if (type instanceof CSimpleType && ((CSimpleType) type).getType().isIntegerType()) {
      ...
      final Formula overflowUF = ffmgr.declareAndCallUF(
          ...
          String.format("_%s%s(%d)_", "overflow", (signed ? "Signed" : "Unsigned"),
  ...
  protected void addRangeConstraint(final Formula variable, CType type, Constraints constraints) {
    ...
    if (type instanceof CSimpleType && ((CSimpleType) type).getType().isIntegerType()) {
      ...
      final boolean signed = machineModel.isSigned(sType);
      final BooleanFormula lowerBound = fmgr.makeLessOrEqual(
          fmgr.makeNumber(numberType, machineModel.getMinimalIntegerValue(sType)), variable,
          signed);
      final BooleanFormula upperBound = fmgr.makeLessOrEqual(
          variable, fmgr.makeNumber(numberType, machineModel.getMaximalIntegerValue(sType)),
          signed);
  ...
  private Formula makeCast0(
    ...
    if (fromCanBeHandledAsInt || toCanBeHandledAsInt) {
      ...
      if (fromCanBeHandledAsInt) {
        fromType =
            fromIsPointer ? machineModel.getPointerEquivalentSimpleType() : CNumericTypes.INT;
  ...
  private Formula makeSimpleCast(CSimpleType pFromCType, CSimpleType pToCType, Formula pFormula) {
    ...
    assert fmgr.getFormulaType(ret).equals(toType) : "types do not match: " + fmgr
        .getFormulaType(ret) + " vs " + toType;
    return ret;
  }
   ...
   */
  public CExpression convertLiteralToFloatIfNecessary(
      final CExpression pExp,
  ...
  public PathFormula makeAnd(
    ...
    if (edge.getPredecessor() instanceof CFunctionEntryNode) {
      addParameterConstraints(edge, function, ssa, pts, constraints, errorConditions,
          (CFunctionEntryNode) edge.getPredecessor());
      addGlobalAssignmentConstraints(edge, function, ssa, pts, constraints, errorConditions,
          PARAM_VARIABLE_NAME, false);
    }
    ...
    if (options.useParameterVariables()
      ...
      for (CParameterDeclaration formalParam : entryNode.getFunctionParameters()) {
        ...
        CParameterDeclaration tmpParameterExpression = new CParameterDeclaration(
            formalParam.getFileLocation(), formalParam.getType(),
            formalParam.getName() + PARAM_VARIABLE_NAME);
        tmpParameterExpression
            .setQualifiedName(formalParam.getQualifiedName() + PARAM_VARIABLE_NAME);
   ...
   *
   * @param tmpAsLHS if tmpAsLHS:  tmp_result1_f := global1 else          global1       :=
   *                 tmp_result1_f
   */
  private void addGlobalAssignmentConstraints(
    ...
    if (options.useParameterVariablesForGlobals()) {
      ...
      for (final CVariableDeclaration decl : globalDeclarations) {
        ...
        if (tmpAsLHS) {
          eq = makeAssignment(tmp, glob, glob, edge, function, ssa, pts, constraints,
              errorConditions);
        } else {
          eq = makeAssignment(glob, glob, tmp, edge, function, ssa, pts, constraints,
              errorConditions);
        }
        constraints.addConstraint(eq);
      }
  ...
  private BooleanFormula createFormulaForEdge(
    ...
    switch (edge.getEdgeType()) {
      ...
      case DeclarationEdge: {
        return makeDeclaration((CDeclarationEdge) edge, function, ssa, pts, constraints,
            errorConditions);
      }
      ...
      case MultiEdge: {
        List<BooleanFormula> multiEdgeFormulas =
            new ArrayList<>(((MultiEdge) edge).getEdges().size());
        ...
        for (CFAEdge singleEdge : (MultiEdge) edge) {
          ...
          multiEdgeFormulas.add(
              createFormulaForEdge(singleEdge, function, ssa, pts, constraints, errorConditions));
  ...
  protected BooleanFormula makeExitFunction(
    ...
    addGlobalAssignmentConstraints(ce, calledFunction, ssa, pts, constraints, errorConditions,
        RETURN_VARIABLE_NAME, false);
    ...
    if (retExp instanceof CFunctionCallStatement) {
      ...
      return makeAssignment(exp.getLeftHandSide(), rhs, ce, callerFunction, ssa, pts, constraints,
          errorConditions);
    } else {
  ...
  protected CType getReturnType(CFunctionCallExpression funcCallExp, CFAEdge edge)
    ...
    if (funcDecl == null) {
      ...
      if (expressionType instanceof CFunctionType) {
        ...
        throw new UnrecognizedCCodeException(
            "Cannot handle function pointer call with unknown type " + expressionType, edge,
            funcCallExp);
      }
      assert retType != null;
    } else {
    ...
    if (!expType.getCanonicalType().equals(retType.getCanonicalType())) {
      ...
      String functionName = funcDecl != null ? funcDecl.getName()
  ...
  protected BooleanFormula makeFunctionCall(
    ...
    for (CParameterDeclaration formalParam : formalParams) {
      ...
      BooleanFormula eq =
          makeAssignment(paramLHS, lhs, paramExpression, edge, callerFunction, ssa, pts,
              constraints, errorConditions);
      result = bfmgr.and(result, eq);
    }

    addGlobalAssignmentConstraints(edge, fn.getFunctionName(), ssa, pts, constraints,
        errorConditions, PARAM_VARIABLE_NAME, true);
   ...
   *                       lhsForChecking), that is used to check, if the assignment is important.
   ...
   */
  public Formula buildTermFromPathFormula(
  ...
  protected Formula buildTerm(
      CRightHandSide exp, CFAEdge edge, String function,
  ...
  protected Formula buildLvalueTerm(
    ...
    return exp
        .accept(new LvalueVisitor(this, edge, function, ssa, pts, constraints, errorConditions));
  }
  ...
  protected BooleanFormula makePredicate(
      ...
      ErrorConditions errorConditions) throws UnrecognizedCCodeException, InterruptedException {
  ...
  public BooleanFormula makePredicate(
      CExpression exp,
      CFAEdge edge,
      String function,
      SSAMapBuilder ssa) throws UnrecognizedCCodeException, InterruptedException {
    PointerTargetSetBuilder pts =
  ...
  Formula replaceField(CFieldReference fExp, Formula pLVar, Optional<Formula> pRightVariable)
    ...
    if (pRightVariable.isPresent()) {
      assert efmgr.getLength((BitvectorFormula) pRightVariable.get())
  ...
  Formula makeVariableUnsafe(
      CExpression exp, String function, SSAMapBuilder ssa,
      boolean makeFresh) {

    if (makeFresh) {
      logger.logOnce(Level.WARNING,
          "Program contains array, or pointer (multiple level of indirection), or field (enable handleFieldAccess and handleFieldAliasing) access; analysis is imprecise in case of aliasing.");
    }
