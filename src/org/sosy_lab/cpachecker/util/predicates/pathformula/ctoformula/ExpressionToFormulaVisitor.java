...
public class ExpressionToFormulaVisitor
      ...
      case MINUS:
        if (!(promT1 instanceof CPointerType)
          ...
          if (promT1.equals(promT2)) {
            ...
            throw new UnrecognizedCCodeException("Can't subtract pointers of different types", edge,
                exp);
          }
        } else {
          throw new UnrecognizedCCodeException("Can't subtract a pointer from a non-pointer", edge,
              exp);
        }
        break;
      case MULTIPLY:
  ...
  public Formula visit(CIdExpression idExp) throws UnrecognizedCCodeException {
    ...
    return conv
        .makeVariable(idExp.getDeclaration().getQualifiedName(), idExp.getExpressionType(), ssa);
  }
  ...
  public Formula visit(CFieldReference fExp) throws UnrecognizedCCodeException {
    ...
    if (fieldRef instanceof CIdExpression) {
      ...
      if (decl instanceof CDeclaration && ((CDeclaration) decl).isGlobal()) {
        ...
        return conv
            .makeVariable(CtoFormulaConverter.exprToVarNameUnscoped(fExp), fExp.getExpressionType(),
                ssa);
      }
    }
  ...
  public Formula visit(CFunctionCallExpression e) throws UnrecognizedCCodeException {
    ...
    if (functionNameExpression instanceof CIdExpression) {
      ...
      if (conv.options.isNondetFunction(functionName)
        ...
        if (parameters.size() == 1) {
          ...
          if (formulaType.isFloatingPointType()) {
            ...
            FloatingPointFormula zero = mgr.getFloatingPointFormulaManager()
                .makeNumber(0.0, (FormulaType.FloatingPointType) formulaType);
        ...
        if (parameters.isEmpty()) {
          // function of arity 0
          conv.logger.logOnce(Level.INFO, "Assuming external function", functionName,
      ...
      conv.logfOnce(Level.WARNING, edge, "Ignoring function call through function pointer %s",
    ...
    if (parameters.isEmpty()) {
      ...
      while (formalParameterTypesIt.hasNext() && parametersIt.hasNext()) {
        ...
        arguments.add(
            conv.makeCast(parameter.getExpressionType(), formalParameterType, argument, constraints,
                edge));
      }
