...
public class CustomInstruction {
  ...
  public String getSignature() {
    ...
    if (outputVariables.size() > 0) {
      Joiner.on(", ")
          .appendTo(sb, Iterables.transform(outputVariables, CIUtils.GET_SMTNAME_WITH_INDEX));
    }
    sb.append(")");
...
//          sb.append("@1 0)");
    ...
    for (int i = 0; i < BracketCounter + 1;
   ...
   *                                                        and aci is not clear, or their structure
   *                                                        dosen't fit.
   */
  public AppliedCustomInstruction inspectAppliedCustomInstruction(final CFANode aciStartNode)
    ...
    return new AppliedCustomInstruction(aciStartNode, aciEndNodes, inVars, outVars, inVarsConst,
        getFakeSMTDescriptionForACI(mapping), ssaMapBuilder.build());
  }

  private List<String> getVariablesOrdered(
      final Map<String, String> pMapping,
  ...
  private Pair<List<String>, String> getFakeSMTDescriptionForACI(final Map<String, String> map) {
    ...
    for (String var : outputVariables) {
      outputVariableList
          .add("(declare-fun " + CIUtils.getSMTName(map.get(var) + "@1") + " () Int)");
    }
  ...
  private void computeMappingOfCiAndAci(
      final CFAEdge ciEdge, final CFAEdge aciEdge,
    ...
    switch (ciEdge.getEdgeType()) {
      ...
      case AssumeEdge:
        ...
        return;
      case StatementEdge:
        compareStatementEdge((CStatementEdge) ciEdge, (CStatementEdge) aciEdge, ciVarToAciVar,
            outVariables);
        return;
      case DeclarationEdge:
        compareDeclarationEdge((CDeclarationEdge) ciEdge, (CDeclarationEdge) aciEdge, ciVarToAciVar,
            outVariables);
        return;
      case ReturnStatementEdge:
        compareReturnStatementEdge((CReturnStatementEdge) ciEdge, (CReturnStatementEdge) aciEdge,
            ciVarToAciVar);
        return;
      case FunctionCallEdge:
        compareFunctionCallEdge((CFunctionCallEdge) ciEdge, (CFunctionCallEdge) aciEdge,
            ciVarToAciVar);
        return;
      ...
      case CallToReturnEdge:
        compareStatementsOfStatementEdge(((CFunctionSummaryEdge) ciEdge).getExpression(),
            ((CFunctionSummaryEdge) aciEdge).getExpression(), ciVarToAciVar, outVariables);
        return;
      default:
  ...
  private void compareAssumeEdge(
      final CAssumeEdge ciEdge, final CAssumeEdge aciEdge,
      final Map<String, String> ciVarToAciVar)
      throws AppliedCustomInstructionParsingFailedException {

    if (ciEdge.getTruthAssumption() != aciEdge.getTruthAssumption()) {
      throw new AppliedCustomInstructionParsingFailedException(
          "The truthAssumption of the CAssumeEdges " + ciEdge + " and " + aciEdge
              + "are different!");
    }
    ciEdge.getExpression()
        .accept(new StructureComparisonVisitor(aciEdge.getExpression(), ciVarToAciVar));
  }

  private void compareStatementEdge(
      final CStatementEdge ciEdge, final CStatementEdge aciEdge,
      final Map<String, String> ciVarToAciVar, final Collection<String> outVariables)
      throws AppliedCustomInstructionParsingFailedException {

    if (ciEdge.getStatement() instanceof CFunctionSummaryStatementEdge && aciEdge
        .getStatement() instanceof CFunctionSummaryStatementEdge) {
      ...
      if (!ciStmt.getFunctionName().equals(aciStmt.getFunctionName())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The functionName of the CFunctionSummaryStatementEdges " + ciEdge + " and " + aciEdge
                + " are different!");
      }

      compareStatementsOfStatementEdge(ciStmt.getFunctionCall(), aciStmt.getFunctionCall(),
          ciVarToAciVar, outVariables);

    }
    compareStatementsOfStatementEdge(ciEdge.getStatement(), aciEdge.getStatement(), ciVarToAciVar,
        outVariables);

  }

  private void compareStatementsOfStatementEdge(
      final CStatement ci, final CStatement aci,
      final Map<String, String> ciVarToAciVar, final Collection<String> outVariables)
      throws AppliedCustomInstructionParsingFailedException {
    ...
    if (ci instanceof CExpressionAssignmentStatement
      ...
      ciStmt.getLeftHandSide().accept(
          new StructureExtendedComparisonVisitor(aciStmt.getLeftHandSide(), ciVarToAciVar,
              currentCiVarToAciVar));
      outVariables.addAll(currentCiVarToAciVar.values());

      // right side: just proof it
      ciStmt.getRightHandSide().accept(
          new StructureExtendedComparisonVisitor(aciStmt.getRightHandSide(), ciVarToAciVar,
              currentCiVarToAciVar));
      ...
      compareFunctionCallExpressions(ciStmt.getFunctionCallExpression(),
          aciStmt.getFunctionCallExpression(), ciVarToAciVar);
  ...
  private void compareFunctionCallExpressions(
    ...
    exp.getFunctionNameExpression().accept(
        new StructureComparisonVisitor(aexp.getFunctionNameExpression(),
  ...
  private void compareDeclarationEdge(
      final CDeclarationEdge ciEdge, final CDeclarationEdge aciEdge,
      final Map<String, String> ciVarToAciVar, final Collection<String> outVariables)
      throws AppliedCustomInstructionParsingFailedException {
    ...
    if (ciDec instanceof CVariableDeclaration && aciDec instanceof CVariableDeclaration) {
      ...
      if (!ciVarDec.getCStorageClass().equals(aciVarDec.getCStorageClass())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The CVariableDeclaration of ci " + ciVarDec + " and aci " + aciVarDec
      ...
      if (ciVarToAciVar.containsKey(ciVarDec.getQualifiedName())
        ...
        throw new AppliedCustomInstructionParsingFailedException(
            "The mapping is not clear. The map contains " + ciVarDec.getQualifiedName()
                + " with the value "
                + ciVarToAciVar.get(ciVarDec.getQualifiedName()) + ", which is different to "
                + aciVarDec.getQualifiedName() + ".");
      }

      compareInitializer(ciVarDec.getInitializer(), aciVarDec.getInitializer(), ciVarToAciVar,
          outVariables);
      ...
      throw new AppliedCustomInstructionParsingFailedException(
          "The declaration of the CDeclarationEdge ci " + ciDec + " and aci " + aciDec
              + " have different classes.");
    }
  }

  private void compareInitializer(
      final CInitializer ciI, final CInitializer aciI,
      final Map<String, String> ciVarToAciVar, final Collection<String> outVariables)
      throws AppliedCustomInstructionParsingFailedException {
    ...
    if (ciI == null && aciI == null) {
      ...
      ((CInitializerExpression) ciI).getExpression().accept(
          new StructureComparisonVisitor(((CInitializerExpression) aciI).getExpression(),
              ciVarToAciVar));
      ...
      if (ciList.size() != aciList.size()) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The CInitializerList of the Initializer of ci " + ciI + " and aci " + aciI
  ...
  private void compareReturnStatementEdge(
    ...
    if (ciEdge.getExpression().isPresent() && aciEdge.getExpression().isPresent()) {
      ciEdge.getExpression().get()
          .accept(new StructureComparisonVisitor(aciEdge.getExpression().get(), ciVarToAciVar));
      ...
      throw new AppliedCustomInstructionParsingFailedException(
          "The expression of the CReturnStatementEdge of ci " + ciEdge + " and aci " + aciEdge
              + " is present in one of them, but not in the otherone.");
    }
  }

  private void compareFunctionCallEdge(
      final CFunctionCallEdge ciEdge, final CFunctionCallEdge aciEdge,
      final Map<String, String> ciVarToAciVar)
      throws AppliedCustomInstructionParsingFailedException {
    ...
    if (ciArguments.size() != aciArguments.size()) {
      throw new AppliedCustomInstructionParsingFailedException(
          "The amount of arguments of the FunctionCallEdges " + ciEdge + " and " + aciEdge
              + " are different!");
    }
  ...
  private void compareMultiEdge(
      final MultiEdge ciEdge, final MultiEdge aciEdge,
      final Map<String, String> ciVarToAciVar, final Collection<String> outVariables)
      throws AppliedCustomInstructionParsingFailedException {
    if (ciEdge.getEdges().size() != aciEdge.getEdges().size()) {
      throw new AppliedCustomInstructionParsingFailedException(
          "The MulitEdges of ci " + ciEdge + " and aci " + aciEdge
              + " have a different amount of edges");
    }
    for (int i = 0; i < ciEdge.getEdges().size(); i++) {
      computeMappingOfCiAndAci(ciEdge.getEdges().get(i), aciEdge.getEdges().get(i), ciVarToAciVar,
          outVariables);
    }
  }
  ...
  private static class StructureComparisonVisitor
    ...
    public Void visit(final CArraySubscriptExpression ciExp)
      ...
      if (!(aciExp instanceof CArraySubscriptExpression)) {
        throw new AppliedCustomInstructionParsingFailedException("The aci expression " + aciExp
    ...
    public Void visit(final CFieldReference ciExp)
      ...
      if (!ciExp.getExpressionType().equals(aciFieldRefExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the FieldReference of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci " + aciFieldRefExp
                + " (" + aciFieldRefExp.getExpressionType() + ").");
      }
      if (ciExp.isPointerDereference() != aciFieldRefExp.isPointerDereference()) {
        throw new AppliedCustomInstructionParsingFailedException(
            "One of the ci " + ciExp + " and aci " + aciFieldRefExp
    ...
    public Void visit(final CIdExpression ciExp)
      ...
      if (aciExp instanceof CIdExpression) {
        ...
        if (!ciExp.getExpressionType().equals(aciIdExp.getExpressionType())) {
          throw new AppliedCustomInstructionParsingFailedException(
              "The expression type of the IdExpression of ci " + ciExp + " (" + ciExp
                  .getExpressionType() + ") is not equal to the one of the aci " + aciIdExp + " ("
                  + aciIdExp.getExpressionType() + ").");
        }
        if (ciVarToAciVar.containsKey(ciExp.getDeclaration().getQualifiedName()) && !ciVarToAciVar
            .get(ciExp.getDeclaration().getQualifiedName())
            .equals(aciIdExp.getDeclaration().getQualifiedName())) {
          throw new AppliedCustomInstructionParsingFailedException(
              "The mapping is not clear. The map contains " + ciExp.getDeclaration()
                  .getQualifiedName() + " with the value " + ciVarToAciVar
                  .get(ciExp.getDeclaration().getQualifiedName()) + ", which is different to "
                  + aciIdExp.getDeclaration().getQualifiedName() + ".");
        } else {
          computeMapping(ciExp.getDeclaration().getQualifiedName(),
              aciIdExp.getDeclaration().getQualifiedName());
        }
        ...
        throw new AppliedCustomInstructionParsingFailedException(
            "The aci expression " + ciExp + " is not a CSimpleType.");
      }
      return null;
    }
    ...
    private void compareSimpleTypes(
        final CIdExpression ciExp,
        final Number aciExpValue,
        final CSimpleType aciType) throws AppliedCustomInstructionParsingFailedException {
      if (ciExp.getExpressionType() instanceof CSimpleType) {
        ...
        if (isValidSimpleType(ciST, aciType)) {
          ...
          } else if (!ciVarToAciVar.get(ciExp.getDeclaration().getQualifiedName())
              .equals(aciExpValue.toString())) {
            throw new AppliedCustomInstructionParsingFailedException(
                "The mapping is not clear. The map contains " + ciExp.getDeclaration()
                    .getQualifiedName() + " with the value " + ciVarToAciVar
                    .get(ciExp.getDeclaration().getQualifiedName()) + ", which is different to "
                    + aciExpValue.toString() + ".");
          }
        } else {
          throw new AppliedCustomInstructionParsingFailedException(
    ...
    public Void visit(final CPointerExpression ciExp)
      ...
      if (!ciExp.getExpressionType().equals(aciPExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the CPointerExpression of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci " + aciPExp + " ("
    ...
    public Void visit(final CComplexCastExpression ciExp)
      ...
      if (ciExp.isImaginaryCast() != aciCExp.isImaginaryCast()) {
        throw new AppliedCustomInstructionParsingFailedException(
            "One of the ci " + ciExp + " and aci " + aciCExp
      ...
      if (!ciExp.getExpressionType().equals(aciCExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the CComplexCastExpression of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci " + aciCExp + " ("
                + aciCExp.getExpressionType() + ").");
      }
      if (!ciExp.getType().equals(aciCExp.getType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The type of the CComplexCastExpression of ci " + ciExp + " (" + ciExp.getType()
                + ") is not equal to the one of the aci " + aciCExp + " (" + aciCExp.getType()
                + ").");
      }
    ...
    public Void visit(final CBinaryExpression ciExp)
      ...
      if (aciExp instanceof CBinaryExpression) {
        ...
        if (!ciExp.getExpressionType().equals(aciBinExp.getExpressionType())) {
          throw new AppliedCustomInstructionParsingFailedException(
              "The expression type of the CBinaryExpression of ci " + ciExp
                  + " is not equal to the one of the aci " + aciBinExp + ".");
        }

        // operators are different
        if (!ciExp.getOperator().getOperator().equals(aciBinExp.getOperator().getOperator())) {
          throw new AppliedCustomInstructionParsingFailedException(
              "The operators of the CBinaryExpression the ci " + ciExp + " and aci " + aciBinExp
                  + " are different.");
        }

        if (!ciExp.getCalculationType().equals(aciBinExp.getCalculationType())) {
          throw new AppliedCustomInstructionParsingFailedException(
              "The calculationType of the CBinaryExpression of ci " + ciExp + " and aci "
                  + aciBinExp + " are different.");
        }
    ...
    public Void visit(final CCastExpression ciExp)
      ...
      if (!ciExp.getExpressionType().equals(aciPExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the CCastExpression of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci " + aciPExp + " ("
                + aciPExp.getExpressionType() + ").");
      }
      this.aciExp = aciPExp.getOperand();
    ...
    public Void visit(final CCharLiteralExpression ciExp)
      ...
      if (!ciExp.getExpressionType().equals(aciCharExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the CharLiteralExpression of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci " + aciCharExp + " ("
                + aciCharExp.getExpressionType() + ").");
      }
      if (ciExp.getCharacter() != aciCharExp.getCharacter()) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The value of the CCharLiteralExpression of ci " + ciExp + " and aci " + aciCharExp
                + " are different.");
      }
      return null;
    }
    ...
    public Void visit(final CFloatLiteralExpression ciExp)
      ...
      if (!ciExp.getExpressionType().equals(aciFloatExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the FloatLiteralExpression of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci " + aciFloatExp + " ("
                + aciFloatExp.getExpressionType() + ").");
      }
      if (!ciExp.getValue().equals(aciFloatExp.getValue())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The value of the CCharLiteralExpression of ci " + ciExp + " and aci " + aciFloatExp
                + " are different.");
      }
      return null;
    }
    ...
    public Void visit(final CIntegerLiteralExpression ciExp)
      ...
      if (!ciExp.getExpressionType().equals(aciIntegerLiteralExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the IntegerLiteralExpression of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci "
                + aciIntegerLiteralExp + " (" + aciIntegerLiteralExp.getExpressionType() + ").");
      }
      if (!ciExp.getValue().equals(aciIntegerLiteralExp.getValue())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The value of the CIntegerLiteralExpression of ci " + ciExp + " and aci "
                + aciIntegerLiteralExp + " are different.");
      }
      return null;
    }
    ...
    public Void visit(final CStringLiteralExpression ciExp)
      ...
      if (!ciExp.getExpressionType().equals(aciStringLiteralExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the StringLiteralExpression of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci " + aciStringLiteralExp
                + " (" + aciStringLiteralExp.getExpressionType() + ").");
      }
      if (!ciExp.getValue().equals(aciStringLiteralExp.getValue())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The value of the CIntegerLiteralExpression of ci " + ciExp + " and aci "
                + aciStringLiteralExp + " are different.");
      }
      return null;
    }
    ...
    public Void visit(final CTypeIdExpression ciExp)
      ...
      if (!ciExp.getExpressionType().equals(aciIdExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the CTypeIdExpression of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci " + aciIdExp + " ("
                + aciIdExp.getExpressionType() + ").");
      }
      if (!ciExp.getType().equals(aciIdExp.getType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The type of the CTypeIdExpression of ci " + ciExp + " (" + ciExp.getType()
                + ") is not equal to the one of the aci " + aciIdExp + " (" + aciIdExp.getType()
                + ").");
      }
      if (!ciExp.getOperator().equals(aciIdExp.getOperator())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The operator of the CTypeIdExpression of ci " + ciExp + " (" + ciExp.getOperator()
                + ") is not equal to the one of the aci " + aciIdExp + " (" + aciIdExp.getOperator()
                + ").");
      }
      return null;
    }
    ...
    public Void visit(final CUnaryExpression ciExp)
      ...
      if (aciExp instanceof CUnaryExpression) {
        ...
        if (!ciExp.getExpressionType().equals(aciUnExp.getExpressionType())) {
          throw new AppliedCustomInstructionParsingFailedException(
              "The expression type of the CUnaryExpression of ci " + ciExp + " (" + ciExp
                  .getExpressionType() + ") is not equal to the one of the aci " + aciUnExp + " ("
        ...
        if (!ciExp.getOperator().getOperator().equals(aciUnExp.getOperator().getOperator())) {
          throw new AppliedCustomInstructionParsingFailedException(
              "The operators of the ci expression " + ciExp + " and aci expression " + aciUnExp
                  + " don't fit together!");
        }
    ...
    public Void visit(final CAddressOfLabelExpression ciExp)
      ...
      if (!ciExp.getExpressionType().equals(aciAExp.getExpressionType())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The expression type of the CAddressOfLabelExpression of ci " + ciExp + " (" + ciExp
                .getExpressionType() + ") is not equal to the one of the aci " + aciAExp + " ("
                + aciAExp.getExpressionType() + ").");
      }
      if (!ciExp.getLabelName().equals(aciAExp.getLabelName())) {
        throw new AppliedCustomInstructionParsingFailedException(
            "The label name of the CAddressOfLabelExpression of ci " + ciExp + " and aci " + aciAExp
                + " are different.");
      }
      return null;
    }
