...
public class CustomInstructionRequirementsWriter {
  ...
  public void writeCIRequirement(
      final ARGState pState, final Collection<ARGState> pSet,
    ...
    if (enableRequirementSlicing) {
      convertedRequirements = abstractReqTranslator
          .convertRequirements(pState, pSet, pACI.getIndicesForReturnVars(),
              pACI.getInputVariables(), pACI.getOutputVariables());
    } else {
      convertedRequirements = abstractReqTranslator
          .convertRequirements(pState, pSet, pACI.getIndicesForReturnVars(), null, null);
    }
    if (convertedRequirements.getSecond().getSecond()
    ...
    List<String> set = removeDuplicates(convertedRequirements.getFirst().getFirst(),
        convertedRequirements.getSecond().getFirst(), fakeSMTDesc.getFirst());
