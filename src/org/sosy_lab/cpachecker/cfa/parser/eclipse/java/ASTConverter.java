...
class ASTConverter {
  ...
  public JStatement convert(final SuperConstructorInvocation sCI) {
    ...
    if (idExpression.getDeclaration() != null) {
      ...
      functionName =
          new JIdExpression(idExpression.getFileLocation(), idExpression.getExpressionType(), name,
              declaration);
    }

    return new JMethodInvocationStatement(getFileLocation(sCI),
        new JSuperConstructorInvocation(getFileLocation(sCI),
  ...
  private JAstNode convert(SuperMethodInvocation e) {
    ...
    if (canBeResolved) {
      declaringClassType =
          (JClassOrInterfaceType) convert(e.resolveMethodBinding().getDeclaringClass());
    ...
    JMethodInvocationExpression miv =
        new JMethodInvocationExpression(getFileLocation(e), convert(e.resolveTypeBinding()),
            methodName, params, declaration);
   ...
   *                              has to be a specific class or interface. The resulting expression
   *                              will be evaluated to <code>true</code> if the the left operand's
   *                              type is equal to this type or a subtype of this type
  ...
  private JAstNode convert(ClassInstanceCreation cIC) {
    ...
    JExpression functionName =
        new JIdExpression(getFileLocation(cIC), convert(cIC.resolveTypeBinding()), name,
            declaration);
    /*JIdExpression idExpression = (JIdExpression) functionName;
  ...
  private JAstNode convert(ArrayAccess e) {
    ...
    return new JArraySubscriptExpression(getFileLocation(e), convert(e.resolveTypeBinding()),
        subscriptExpression,
        index);
  }
  ...
  private boolean isMainArgumentArray(QualifiedName e, JExpression qualifierExpression) {
    ...
    if (qualifierExpression instanceof JIdExpression) {
      ...
      return lengthBinding == null && isArrayType(qualifierDecl.getType())
  ...
  public JMethodInvocationExpression convert(
      FunctionEntryNode newFunctionEntryNode,
    ...
    JIdExpression methodName =
        new JIdExpression(oldMethodCall.getFileLocation(), JSimpleType.getUnspecified(), name,
            declaration);
    ...
    if (oldMethodCall instanceof JReferencedMethodInvocationExpression) {
      return new JReferencedMethodInvocationExpression(oldMethodCall.getFileLocation(),
          oldMethodCall.getExpressionType(), methodName, oldMethodCall.getParameterExpressions(),
          declaration,
          ((JReferencedMethodInvocationExpression) oldMethodCall).getReferencedVariable());
    } else {
      return new JMethodInvocationExpression(oldMethodCall.getFileLocation(),
  ...
  private JAstNode convert(MethodInvocation mi) {
    ...
    if (methodName instanceof JIdExpression) {
      ...
      if (idExpression.getDeclaration() != null) {
        ...
        methodName =
            new JIdExpression(idExpression.getFileLocation(), idExpression.getExpressionType(),
                name, declaration);
      }
    }
    ...
    if (!(referencedVariableName instanceof JIdExpression)) {
      return new JMethodInvocationExpression(getFileLocation(mi), convert(mi.resolveTypeBinding()),
          methodName, params,
          declaration);
    } else {
      return new JReferencedMethodInvocationExpression(getFileLocation(mi),
  ...
  private JAstNode convert(SimpleName e) {
    ...
    return new JIdExpression(getFileLocation(e), convert(e.resolveTypeBinding()), name,
        declaration);
  }
  ...
  private JAstNode handlePostFixIncOrDec(
      PostfixExpression e,
  ...
  private boolean isNumericCompatible(JBasicType pType) {
    return pType != null
        && (pType.isIntegerType() || pType.isFloatingPointType()
  ...
  private JExpression convert(NumberLiteral e) {
    ...
    switch (t) {
      ...
      default:
        return new JIntegerLiteralExpression(getFileLocation(e),
            BigInteger.valueOf(Long.parseLong(e.getToken())));
    }
  }
  ...
  public JMethodInvocationAssignmentStatement getIteratorFromIterable(Expression pExpr) {
    ...
    JReferencedMethodInvocationExpression mi =
        new JReferencedMethodInvocationExpression(fileLoc, iteratorTyp, name, parameters, null,
            (JIdExpression) iterable);
  ...
  public JMethodInvocationAssignmentStatement assignParameterToNextIteratorItem(
    ...
    JReferencedMethodInvocationExpression mi =
        new JReferencedMethodInvocationExpression(fileLoc, param.getType(), name, parameters, null,
            enhancedForLoopIterator);
  ...
  public JExpression convertBooleanExpression(Expression e) {
    ...
    if (!isBooleanExpression(exp)) {
      ...
      return new JBinaryExpression(exp.getFileLocation(), exp.getExpressionType(), exp, zero,
  ...
  JObjectReferenceReturn getConstructorObjectReturn(ITypeBinding declaringClass) {
    ...
    assert declaringClass.isClass() || declaringClass.isEnum() : declaringClass.getName()
        + " is not a Class";
