...
class ASTConverter {
  ...
  public ASTConverter(
      Configuration pConfig, Scope pScope, LogManagerWithoutDuplicates pLogger,
    ...
    this.logger = pLogger;
    this.typeConverter =
        new ASTTypeConverter(scope, this, pStaticVariablePrefix, pNiceFileNameFunction);
  ...
  CExpression convertExpressionWithoutSideEffects(
      IASTExpression e) {
  ...
  private CIdExpression addSideAssignmentsForUnaryExpressions(
    ...
    final CBinaryExpression postExp = buildBinaryExpression(FileLocation.DUMMY, exp,
        CIntegerLiteralExpression.ONE, op);
    sideAssignmentStack
        .addPreSideAssignment(new CExpressionAssignmentStatement(fileLoc, exp, postExp));
    return tmp;
  }
  ...
  CAstNode convertExpressionWithSideEffects(IASTExpression e) {
  ...
  private CAstNode convertExpressionWithSideEffectsNotSimplified(IASTExpression e) {
    ...
    if (e == null) {
      ...
      throw new CFAGenerationRuntimeException(
          "Unknown expression type " + e.getClass().getSimpleName(), e, niceFileNameFunction);
    }
  }
...
//            e, niceFileNameFunction);
  ...
  private CAstNode convert(IASTBinaryExpression e) {
    ...
    boolean isAssign = opPair.getSecondNotNull();
    ...
    if (isAssign) {
      if (!(leftHandSide instanceof CLeftHandSide)) {
        throw new CFAGenerationRuntimeException("Lefthandside of Assignment " + e.getRawSignature()
            + " is no CLeftHandside but should be.", leftHandSide);
      }
      CLeftHandSide lhs = (CLeftHandSide) leftHandSide;

      if (op == null) {
        // a = b
        CAstNode rightHandSide = convertExpressionWithSideEffects(
        ...
        if (rightHandSide instanceof CExpression) {
          ...
          // a = f()
          return new CFunctionCallAssignmentStatement(fileLoc, lhs,
          ...
          return new CExpressionAssignmentStatement(fileLoc, lhs,
              ((CAssignment) rightHandSide).getLeftHandSide());
        } else {
        ...
        CBinaryExpression exp = buildBinaryExpression(FileLocation.DUMMY, leftHandSide,
            rightHandSide, op);
      ...
      CExpression rightHandSide = convertExpressionWithoutSideEffects(e.getOperand2());
      return buildBinaryExpression(fileLoc, leftHandSide, rightHandSide, op);
    }
  }

  private CBinaryExpression buildBinaryExpression(
      FileLocation location, CExpression operand1, CExpression operand2, BinaryOperator op) {
    try {
      return binExprBuilder.buildBinaryExpression(location, operand1, operand2, op);
  ...
  private CAstNode convert(IASTCastExpression e) {
    ...
    if (e.getOperand() instanceof IASTFieldReference && ((IASTFieldReference) e.getOperand())
        .isPointerDereference()) {
      return createInitializedTemporaryVariable(loc, castType,
          new CCastExpression(loc, castType, operand));
    } else {
  ...
  private CFieldReference convert(IASTFieldReference e) {
    ...
    while (ownerType instanceof CArrayType ||
        ownerType instanceof CPointerType) {
      if (ownerType instanceof CArrayType) {
        ownerType = ((CArrayType) ownerType).getType().getCanonicalType();
      } else {
    ...
    if (ownerType instanceof CElaboratedType) {
      ...
      throw new CFAGenerationRuntimeException(
          "Cannot access the field " + fieldName + " in type " + ownerType
              + " which does not have a definition", e, niceFileNameFunction);
      ...
      assert ownerType instanceof CCompositeType :
          "owner of field has no CCompositeType, but is a: " + ownerType.getClass() + " instead.";

      wayToInnerField =
          getWayToInnerField(ownerType, fieldName, loc, new ArrayList<Pair<String, CType>>());
      if (!wayToInnerField.isEmpty()) {
        ...
        for (Pair<String, CType> field : wayToInnerField) {
          fullFieldReference =
              new CFieldReference(loc, field.getSecond(), field.getFirst(), fullFieldReference,
                  isPointerDereference);
          isPointerDereference = false;
        }
      } else {
        throw new CFAGenerationRuntimeException(
            "Accessing unknown field " + fieldName + " in " + ownerType + " in file "
                + staticVariablePrefix.split("__")[0], e, niceFileNameFunction);
      }
    }
    ...
    if (simplifyPointerExpressions && (wayToInnerField.size() > 1
        || owner instanceof CFieldReference) && !scope.isGlobalScope()) {
      ...
      if (typeDefType instanceof CPointerType) {
        ...
        throw new CFAGenerationRuntimeException(
            "The owner of the struct with field dereference has an invalid type", owner);
      }

      CPointerExpression exp = new CPointerExpression(loc, newType, owner);

      return new CFieldReference(loc, fullFieldReference.getExpressionType(), fieldName, exp,
          false);
    }
  ...
  private CRightHandSide convert(IASTFunctionCallExpression e) {
    ...
    if (functionName instanceof CIdExpression) {
      if (FUNC_TYPES_COMPATIBLE.equals(((CIdExpression) functionName).getName())) {
        ...
        sideAssignmentStack.leaveBlock();
        if (params.size() == 2) {
          if (areCompatibleTypes(params.get(0).getExpressionType(),
              params.get(1).getExpressionType())) {
    ...
    if (functionName instanceof CIdExpression) {
      ...
      if ((declaration == null)
        ...
        return buildBinaryExpression(getLocation(e), params.get(0), params.get(1), BinaryOperator
            .EQUALS);
      }
    }
    ...
    switch (e.getOperator()) {
      ...
      case IASTUnaryExpression.op_labelReference:
        ...
        if (!(operand instanceof CIdExpression)) {
          throw new CFAGenerationRuntimeException("Invalid operand for address-of-label operator",
              e, niceFileNameFunction);
        }
      ...
      case IASTUnaryExpression.op_prefixDecr:
        ...
        CBinaryExpression preExp = buildBinaryExpression(FileLocation.DUMMY, operand,
            CIntegerLiteralExpression.ONE, preOp);
      ...
      case IASTUnaryExpression.op_postFixDecr:
        ...
        CBinaryExpression postExp = buildBinaryExpression(FileLocation.DUMMY, operand,
      ...
      case IASTUnaryExpression.op_not:
        return simplifyUnaryNotExpression(fileLoc, operand);

      case IASTUnaryExpression.op_tilde:
        return new CUnaryExpression(fileLoc, operandType, operand,
            operatorConverter.convertUnaryOperator(e));
      ...
      default:
        ...
        return new CUnaryExpression(fileLoc, type, operand,
            operatorConverter.convertUnaryOperator(e));
    }
  }
   ...
   */
  private CExpression simplifyUnaryNotExpression(
      final FileLocation location, final CExpression
      expr) {
    ...
    if (expr instanceof CBinaryExpression) {
      ...
      if (CBinaryExpressionBuilder.relationalOperators.contains(binExpr.getOperator())) {
        ...
        return buildBinaryExpression(location, binExpr.getOperand1(), binExpr.getOperand2(),
            inverseOperator);
      }
    }
    ...
    return buildBinaryExpression(location, CIntegerLiteralExpression.ZERO, expr, BinaryOperator
        .EQUALS);
  }
  ...
  private CTypeIdExpression convert(IASTTypeIdExpression e) {
    ...
    return new CTypeIdExpression(getLocation(e), type,
  ...
  CStatement convertExpressionToStatement(final IASTExpression e) {
    CAstNode node = convertExpressionWithSideEffects(e);
  ...
  public CReturnStatement convert(final IASTReturnStatement s) {
    ...
    if (returnVariableDeclaration.isPresent()) {
      ...
      if (returnExp.isPresent()) {
        ...
        logger.log(Level.WARNING, loc + ":",
            "Return statement without expression in non-void function.");
        CInitializer defaultValue =
            CDefaults.forType(returnVariableDeclaration.get().getType(), loc);
      ...
      if (returnExp.isPresent()) {
        logger.log(Level.WARNING, loc + ":", "Return statement with expression", returnExp.get(),
            "in void function.");
      }
      returnAssignment = Optional.absent();
    }
  ...
  public CFunctionDeclaration convert(final IASTFunctionDefinition f) {
    ...
    if (!(cStorageClass == CStorageClass.AUTO
      ...
      throw new CFAGenerationRuntimeException("Unsupported storage class for function definition",
          f, niceFileNameFunction);
    }
    ...
    if (!(declarator.getFirst() instanceof CFunctionTypeWithNames)) {
      throw new CFAGenerationRuntimeException(
          "Unsupported nested declarator for function definition", f, niceFileNameFunction);
    }
    ...
    return new CFunctionDeclaration(getLocation(f), declSpec, declarator.getThird(),
  ...
  public List<CDeclaration> convert(final IASTSimpleDeclaration d) {
    ...
    if (declarators != null) {
      for (IASTDeclarator c : declarators) {
        IASTAttribute[] attributes = c.getAttributes();
        for (IASTAttribute attribute : attributes) {
          String name = String.valueOf(attribute.getName());
          switch (name) {
            case "noreturn": {
              // we add no-return functions into the stop function list
              String stopName = c.getName().toString();
              GlobalInfo.getInstance().addStopFunction(stopName);
              break;
            }
            case "__mode__": {
              if (type instanceof CSimpleType) {
                String value = String.valueOf(attribute.getArgumentClause().getTokenCharImage());
                boolean isSigned = ((CSimpleType) type).isGeneralSigned();
                boolean isInt = ((CSimpleType) type).getType().isIntegerType();
                int times;
                switch (value) {
                  case "__QI__":
                    times = 1;
                ...
                CType newType = machineModel.getType(isInt, isSigned, times);
                if (newType != null) {
                  type = newType;
                }
              }
        ...
        result.add(createDeclaration(getLocation(c), cStorageClass, type, c));
      }
    }
  ...
  private CDeclaration createDeclaration(
    ...
    if (d != null) {
      ...
      if (!isGlobal && cStorageClass == CStorageClass.EXTERN) {
        //YX, handle extern declaration in functiuons like global declarations
      ...
      try {
      ...
      } catch (CFAGenerationRuntimeException re) {
        IASTNode parent = d.getParent();
        boolean print = true;
        while (!(parent instanceof CASTFunctionDefinition)) {
          if (parent instanceof CASTCompositeTypeSpecifier) {
            if (((CASTCompositeTypeSpecifier) parent).getName().toString().equals("")) {
              print = false;
              break;
            }
          }
        ...
        if (print) {
          re.printStackTrace();
          throw new CFAGenerationRuntimeException("Variable " + name + " already declared",
              declaration);
        }
      }
  ...
  private List<CCompositeTypeMemberDeclaration> convertDeclarationInCompositeType(
    ...
    if (!(d instanceof IASTSimpleDeclaration)) {
      throw new CFAGenerationRuntimeException(
          "unknown declaration type " + d.getClass().getSimpleName(), d, niceFileNameFunction);
    }
    ...
    if (type instanceof CCompositeType) {
      ...
      type = new CElaboratedType(compositeType.isConst(), compositeType.isVolatile(),
          compositeType.getKind(), compositeType.getName(), compositeType.getOrigName(),
          compositeType);
    }
  ...
  private Triple<CType, IASTInitializer, String> convert(IASTDeclarator d, CType specifier) {
    ...
    if (d instanceof IASTFunctionDeclarator) {
      //it is always right to assume that here is no static storage class because static
      // functions are handled in createCFA..handleFunctionDefinition(CFAFunctionBuilder)
      ...
      //YX
      //handle function pointers that are resolved as nested function definitions
      IASTDeclarator currentDecl = d;
      IASTDeclarator nested = currentDecl.getNestedDeclarator();
      if (nested != null && nested instanceof CASTFunctionDeclarator) {
        return convert((IASTFunctionDeclarator) nested, specifier, false);
      }
      ...
      while (currentDecl != null) {
        ...
        if (currentDecl instanceof IASTFunctionDeclarator) {
          throw new CFAGenerationRuntimeException(
              "Unsupported declaration nested function declarations", d, niceFileNameFunction);
        }
        ...
        if (currentDecl.getInitializer() != null) {
          if (initializer != null) {
            throw new CFAGenerationRuntimeException("Unsupported declaration with two initializers",
                d, niceFileNameFunction);
          }
          //xxx
      ...
      name = Strings
          .nullToEmpty(name); // there may be no name at all, for example in parameter declarations
          ...
          if (initClause instanceof IASTInitializerList) {
            ...
            for (IASTInitializerClause x : ((IASTInitializerList) initClause).getClauses()) {
              ...
              if (x instanceof ICASTDesignatedInitializer) {
                for (ICASTDesignator designator : ((ICASTDesignatedInitializer) x)
                    .getDesignators()) {
  ...
  private CType convert(IASTArrayModifier am, CType type) {
    if (am instanceof org.eclipse.cdt.core.dom.ast.c.ICASTArrayModifier) {
      org.eclipse.cdt.core.dom.ast.c.ICASTArrayModifier a =
          (org.eclipse.cdt.core.dom.ast.c.ICASTArrayModifier) am;
  ...
  private Triple<CType, IASTInitializer, String> convert(
    ...
    Triple<CType, IASTInitializer, String> result;
    if (d instanceof IASTStandardFunctionDeclarator) {
      result = convert((IASTStandardFunctionDeclarator) d, returnType, isStaticFunction);
    } else if (d instanceof ICASTKnRFunctionDeclarator) {
      result = convert((ICASTKnRFunctionDeclarator) d, returnType, isStaticFunction);
    } else {
      throw new CFAGenerationRuntimeException("Unknown non-standard function definition", d,
          niceFileNameFunction);
    }
    // register transformed name for each static function
    if (isStaticFunction) {
      String transformedName = result.getThird();
      GlobalInfo.getInstance().getPreInfoManager().addStaticFunctionInfo(d.getContainingFilename
          (), d.getName().getRawSignature(), transformedName);
    }
    return result;
  }

  private Triple<CType, IASTInitializer, String> convert(
      ICASTKnRFunctionDeclarator d, CType
      returnType, boolean isStaticFunction) {
    ...
    // handle parameters
    List<CParameterDeclaration> paramsList = convert(d.getParameterDeclarations());

    // TODO constant and volatile
    CFunctionTypeWithNames fType = new CFunctionTypeWithNames(false, false, returnType,
        paramsList, false);
    CType type = fType;
  ...
  private Triple<CType, IASTInitializer, String> convert(
      IASTStandardFunctionDeclarator d, CType
      returnType, boolean isStaticFunction) {
    ...
    // handle parameters
    List<CParameterDeclaration> paramsList = convert(d.getParameters());

    // TODO constant and volatile
    CFunctionTypeWithNames fType =
        new CFunctionTypeWithNames(false, false, returnType, paramsList, d
            .takesVarArgs());
    CType type = fType;
  ...
  private CCompositeType convert(IASTCompositeTypeSpecifier d) {
    ...
    switch (d.getKey()) {
      ...
      case IASTCompositeTypeSpecifier.k_union:
        ...
        break;
      default:
        throw new CFAGenerationRuntimeException("Unknown key " + d.getKey() + " for composite type",
            d, niceFileNameFunction);
    }
    ...
    compositeType
        .accept(new FillInBindingVisitor(kind, scope.getFileSpecificTypeName(name), compositeType));
  ...
  private CInitializer convert(
    ...
    if (i instanceof IASTExpression) {
      ...
      throw new CFAGenerationRuntimeException(
          "unknown initializer claus: " + i.getClass().getSimpleName(), i, niceFileNameFunction);
    }
  }
  ...
  private CInitializer convert(IASTInitializer i, @Nullable CVariableDeclaration declaration) {
    if (i == null) {
      ...
      throw new CFAGenerationRuntimeException(
          "unknown initializer: " + i.getClass().getSimpleName(), i, niceFileNameFunction);
    }
  }

  private CInitializer convert(
      org.eclipse.cdt.core.dom.ast.c.ICASTDesignatedInitializer init,
      @Nullable CVariableDeclaration declaration) {
  ...
  private CInitializerList convert(
  ...
  private CInitializer convert(
    ...
    if (ic instanceof IASTExpression) {
      ...
      if (initializer instanceof CAssignment) {
        ...
        return new CInitializerExpression(getLocation(e),
            ((CAssignment) initializer).getLeftHandSide());
      ...
      if (!(initializer instanceof CExpression)) {
        throw new CFAGenerationRuntimeException(
            "Initializer is not free of side-effects, it is a " + initializer.getClass()
                .getSimpleName(), e, niceFileNameFunction);
      }
  ...
  private List<CParameterDeclaration> convert(IASTDeclaration[] ps) {
    List<CParameterDeclaration> paramsList = new ArrayList<>(ps.length);
    for (IASTDeclaration c : ps) {
      if (!c.getRawSignature().equals("void")) {
        paramsList.addAll(convert(c));
      } else {
        // there may be a function declaration f(void), which is equal to f()
        // we don't want this dummy parameter "void"
        assert ps.length == 1;
  ...
  private IASTDeclSpecifier getSpecifier(IASTDeclaration d) {
    if (d instanceof IASTSimpleDeclaration) {
      return ((IASTSimpleDeclaration) d).getDeclSpecifier();
    } else if (d instanceof CASTFunctionDefinition) {
      return ((CASTFunctionDefinition) d).getDeclSpecifier();
    } else {
      throw new CFAGenerationRuntimeException("Can not get specifier!", d, niceFileNameFunction);
    }
  }

  private IASTDeclarator[] getDeclarator(IASTDeclaration d) {
    if (d instanceof IASTSimpleDeclaration) {
      return ((IASTSimpleDeclaration) d).getDeclarators();
    } else {
      throw new CFAGenerationRuntimeException("Can not get declarator!", d, niceFileNameFunction);
    }
  }

  private List<CParameterDeclaration> convert(IASTDeclaration p) {
    Pair<CStorageClass, ? extends CType> specifier = convert(getSpecifier(p));
    if (specifier.getFirst() != CStorageClass.AUTO) {
      throw new CFAGenerationRuntimeException("Unsupported storage class for parameters", p,
          niceFileNameFunction);
    }

    IASTDeclarator[] declarators = getDeclarator(p);
    ...
    for (IASTDeclarator declarator1 : declarators) {
      Triple<CType, IASTInitializer, String> declarator = convert(declarator1, specifier
          .getSecond());
      parameterDeclarations.add(getCParamDecla(getLocation(declarator1), p, declarator));
    }

    return parameterDeclarations;
  }

  private CParameterDeclaration getCParamDecla(
      FileLocation loc, IASTDeclaration p, Triple<CType,
      IASTInitializer, String> declarator) {
    ...
    return new CParameterDeclaration(loc, type, declarator.getThird());
  }
  ...
  FileLocation getLocation(final IASTNode n) {
    ...
    startingLineInOrigin = startingInOrigin.getSecondNotNull();
