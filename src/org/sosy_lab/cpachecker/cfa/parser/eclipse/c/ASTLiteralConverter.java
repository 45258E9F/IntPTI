...
class ASTLiteralConverter {
  ...
  private void check(boolean assertion, String msg, IASTNode astNode)
    ...
    if (!assertion) {
      throw new CFAGenerationRuntimeException(msg, astNode, niceFileNameFunction);
    }
  }
  ...
  CLiteralExpression convert(
    ...
    switch (e.getKind()) {
      ...
      case IASTLiteralExpression.lk_integer_constant:
        ...
        BigInteger integerValue = parseIntegerLiteral(suffix,
            valueStr.substring(0, valueStr.length() - suffix.getLength()), e);
  ...
  private CImaginaryLiteralExpression handleImaginaryNumber(
    ...
    type = new CSimpleType(type.isConst(), type.isVolatile(), type.getType(), type.isLong(),
        type.isShort(), type.isSigned(), type.isUnsigned(), type.isComplex(), true,
        type.isLongLong());
    switch (exp.getKind()) {
      case IASTLiteralExpression.lk_char_constant:
      ...
      case IASTLiteralExpression.lk_integer_constant:
        ...
        BigInteger integerValue = parseIntegerLiteral(suffix,
            valueStr.substring(0, valueStr.length() - suffix.getLength()), exp);
        CSimpleType actualType = suffix.getTypeForValue(integerValue, machine);
        return new CImaginaryLiteralExpression(fileLoc,
            actualType,
  ...
  char parseCharacterLiteral(String s, final IASTNode e) {
    if (s.length() > 0 && s.charAt(0) == 'L') {
      Logger.getGlobal().log(Level.WARNING, "treat wide char as normal char (droping 'L')");
      // TODO: this is a dirty hack of parsing wide char
      s = s.substring(1);
    }
  ...
  Suffix determineSuffix(String pIntegerLiteral, IASTNode pExpression) {
    ...
    if (pIntegerLiteral.charAt(last) == 'U' || pIntegerLiteral.charAt(last) == 'u') {
      if (!suffix.isSigned()) {
        throw new CFAGenerationRuntimeException("invalid duplicate modifier U in integer literal",
            pExpression, niceFileNameFunction);
      }
      last--;
  ...
  static enum Suffix {
    ...
    protected CSimpleType getType(
        BigInteger pValue,
        MachineModel pMachineModel,
        CSimpleType... pCandidates) {
