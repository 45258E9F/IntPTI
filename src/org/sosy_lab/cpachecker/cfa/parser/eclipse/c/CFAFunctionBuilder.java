...
class CFAFunctionBuilder extends ASTVisitor {
  ...
  // ArrayDeque not possible because it does not allow null
  ...
  public CFAFunctionBuilder(
      Configuration config, LogManagerWithoutDuplicates pLogger, FunctionScope pScope,
    ...
    scope = pScope;
    astCreator =
        new ASTConverter(config, pScope, pLogger, pNiceFileNameFunction, pSourceOriginMapping,
            pMachine, staticVariablePrefix, pSideAssignmentStack);
  ...
  private IASTFunctionDeclarator getFunctionDeclarator(IASTSimpleDeclaration sd) {
    IASTNode parent = sd.getParent();
    while (!(parent instanceof CASTFunctionDefinition)) {
      parent = parent.getParent();
    }
    return ((CASTFunctionDefinition) parent).getDeclarator();
  }
  ...
  private int handleSimpleDeclaration(final IASTSimpleDeclaration sd) {
    ...
    //YX, params of knr function as param declaration
    IASTFunctionDeclarator functionDeclarator = getFunctionDeclarator(sd);
    if (functionDeclarator instanceof CASTKnRFunctionDeclarator) {
      for (IASTDeclaration declaration :
          ((CASTKnRFunctionDeclarator) functionDeclarator).getParameterDeclarations()) {
        if (declaration.equals(sd)) {
          return PROCESS_SKIP;
        }
      }
    }

    // if current declaration is the children of a typeId initializer expression (i.e. compound
    // literal), we do not create a new edge for certain declaration
    IASTNode currentNode = sd;
    while (currentNode != null) {
      if (currentNode instanceof IASTTypeIdInitializerExpression) {
        break;
      }
  ...
  private CFANode createEdgeForDeclaration(
      final IASTSimpleDeclaration sd,
    ...
    for (CDeclaration newD : declList) {
      ...
      if (newD instanceof CVariableDeclaration) {
        ...
        if (init != null) {
          ...
          prevNode = nextNode;
          prevNode = createEdgesForSideEffects(prevNode,
              sideAssignmentStack.getAndResetPostSideAssignments(), rawSignature, fileLocation);
    ...
    prevNode =
        createEdgesForSideEffects(prevNode, sideAssignmentStack.getAndResetPostSideAssignments(),
            rawSignature, fileLocation);
  ...
  private void handleExpressionStatement(
      IASTExpressionStatement exprStatement,
    ...
    if (exprStatement.getExpression() instanceof IASTExpressionList) {
      for (IASTExpression exp : ((IASTExpressionList) exprStatement.getExpression())
          .getExpressions()) {
  ...
  private CFANode createIASTExpressionStatementEdges(
      String rawSignature, FileLocation fileloc,
    ...
    if (sideAssignmentStack.hasConditionalExpression()
      ...
      if ((tempVars.size() == 1) && (tempVars.get(0).getSecond()
   ...
   * @category statements
   */
  private void handleLabelStatement(
      IASTLabelStatement labelStatement,
      FileLocation fileloc) {
   ...
   * @category statements
   */
  private void handleGotoStatement(
  ...
  private CBinaryExpression buildBinaryExpression(
      FileLocation loc, CExpression operand1, CExpression operand2, BinaryOperator op) {
    try {
      return binExprBuilder.buildBinaryExpression(loc, operand1, operand2, op);
  ...
  private Optional<CExpression> createConditionEdges(
    ...
    return buildConditionTree(condition, fileLocation, rootNode, thenNode, elseNode, thenNode,
        elseNode, true, true, false);
  }
  ...
  private Optional<CExpression> buildConditionTree(
      IASTExpression condition, final FileLocation fileLocation,
    ...
    if (condition instanceof IASTUnaryExpression
      ...
      return buildConditionTree(((IASTUnaryExpression) condition).getOperand(), fileLocation,
          rootNode, thenNode, elseNode, thenNode, elseNode, true, true, flippedThenElse);

      // !a --> switch branches
    } else if (condition instanceof IASTUnaryExpression
        && ((IASTUnaryExpression) condition).getOperator() == IASTUnaryExpression.op_not) {
      buildConditionTree(((IASTUnaryExpression) condition).getOperand(), fileLocation, rootNode,
          elseNode, thenNode, elseNode, thenNode, true, true, !flippedThenElse);
      ...
      buildConditionTree(((IASTBinaryExpression) condition).getOperand1(), fileLocation, rootNode,
          innerNode, elseNode, thenNodeForLastThen, elseNode, true, false, flippedThenElse);
      buildConditionTree(((IASTBinaryExpression) condition).getOperand2(), fileLocation, innerNode,
          thenNode, elseNode, thenNodeForLastThen, elseNodeForLastElse, true, true,
          flippedThenElse);
      return Optional.absent();
      ...
      CFANode innerNode = newCFANode();
      buildConditionTree(((IASTBinaryExpression) condition).getOperand1(), fileLocation, rootNode,
          thenNode, innerNode, thenNodeForLastThen, elseNodeForLastElse, false, true,
          flippedThenElse);
      buildConditionTree(((IASTBinaryExpression) condition).getOperand2(), fileLocation, innerNode,
          thenNode, elseNode, thenNodeForLastThen, elseNodeForLastElse, true, true,
          flippedThenElse);
      ...
      switch (kind) {
        case ALWAYS_FALSE:
          ...
          //store always false expression
          GlobalInfo gInfoF = GlobalInfo.getInstance();
          gInfoF.getPreInfoManager().addAlwaysFalse(falseEdge);
        ...
        case ALWAYS_TRUE:
          ...
          //store always true expression
          GlobalInfo gInfoT = GlobalInfo.getInstance();
          gInfoT.getPreInfoManager().addAlwaysTrue(trueEdge);
      ...
      if (flippedThenElse && !allowBranchSwapping) {
        expression = buildBinaryExpression(loc, expression, CIntegerLiteralExpression.ZERO,
      ...
      if (ASTOperatorConverter.isBooleanExpression(expression)) {
        ...
        CExpression conv =
            buildBinaryExpression(loc, exp, CIntegerLiteralExpression.ZERO, BinaryOperator.EQUALS);
        ...
        CExpression conv = buildBinaryExpression(loc,
            buildBinaryExpression(FileLocation.DUMMY, expression, CIntegerLiteralExpression.ZERO,
                BinaryOperator.EQUALS),
  ...
  private void addConditionEdges(
      CExpression condition, CFANode rootNode,
  ...
  private void handleBreakStatement(
      IASTBreakStatement breakStatement,
      FileLocation fileloc) {
  ...
  private void handleContinueStatement(
      IASTContinueStatement continueStatement,
      FileLocation fileloc) {
  ...
  private int handleForStatement(
      final IASTForStatement forStatement,
  ...
  private CFANode createInitEdgeForForLoop(
    ...
    if (statement instanceof IASTDeclarationStatement) {
      ...
      if (!(decl instanceof IASTSimpleDeclaration)) {
        throw new CFAGenerationRuntimeException("Unexpected declaration in header of for loop",
            decl, niceFileNameFunction);
      }
      ...
      throw new CFAGenerationRuntimeException("Unexpected statement type in header of for loop",
          statement, niceFileNameFunction);
    }
  }
  ...
  private void createConditionEdgesForForLoop(
    ...
    if (condition == null) {
      ...
      createConditionEdges(expl[expl.length - 1], fileLocation, loopStart, firstLoopNode,
          postLoopNode);
    } else {
   ...
   */
  private int handleSwitchStatement(
      final IASTSwitchStatement statement,
      FileLocation fileloc) {
    ...
    prevNode = handleAllSideEffects(prevNode, switchExpression.getFileLocation(),
        statement.getRawSignature(), true);
   ...
   * @category switch statement build condition edges, to caseNode with "a==2", to notCaseNode with
   * "!(a==2)"
   */
  private CFANode handleCaseSingleStatement(
      final IASTExpression right, final CExpression switchExpr,
    ...
    final CBinaryExpression binExp = buildBinaryExpression(FileLocation.DUMMY,
    ...
    switch (astCreator.getConditionKind(exp)) {
      ...
      case ALWAYS_TRUE:
        final BlankEdge trueEdge = new BlankEdge("", fileLocation, rootNode, caseNode,
  ...
  private CFANode handleCaseRangeStatement(
      final IASTBinaryExpression range, final CExpression switchExpr,
      final CFANode rootNode, final CFANode caseNode, final CFANode notCaseNode,
      final FileLocation fileLocation) {
    ...
    final CBinaryExpression firstPart = buildBinaryExpression(FileLocation.DUMMY,
        smallEnd, switchExpr, CBinaryExpression.BinaryOperator.LESS_EQUAL);
    final CBinaryExpression secondPart = buildBinaryExpression(FileLocation.DUMMY,
  ...
  private void handleDefaultStatement(
      final IASTDefaultStatement statement,
  ...
  private CFANode handleAllSideEffects(
      CFANode prevNode, final FileLocation fileLocation,
    ...
    if (sideAssignmentStack.hasConditionalExpression() && !resultIsUsed) {
      ...
      prevNode =
          createEdgesForSideEffects(prevNode, sideAssignmentStack.getAndResetPreSideAssignments(),
              rawSignature, fileLocation);

      // handle ternary operator or && or || or { }
      for (Pair<IASTExpression, CIdExpression> cond : sideAssignmentStack
          .getAndResetConditionalExpressions()) {
  ...
  private CFANode handleConditionalExpression(
    ...
    if (condExp instanceof IASTConditionalExpression) {
      ...
      return handleCompoundStatementExpression((IGNUASTCompoundStatementExpression) condExp,
          prevNode, tempVar);
  ...
  private CFANode handleExpressionList(
    ...
    //tempVar is generated when exp is an instance of CRightHandSide
    if (tempVar != null && exp instanceof CRightHandSide) {
  ...
  private CFANode handleCompoundStatementExpression(
    ...
    if (!(lastStatement instanceof IASTExpressionStatement)) {
      if (tempVar == null) {
        CFANode lastNode =
            handleAllSideEffects(middleNode, fileLocation, lastStatement.getRawSignature(), true);
        scope.leaveBlock();
        return lastNode;
      }

      throw new CFAGenerationRuntimeException(
          "Unsupported statement type " + lastStatement.getClass().getSimpleName()
              + " at end of compound-statement expression", lastStatement, niceFileNameFunction);
    }

    CAstNode exp = astCreator.convertExpressionWithSideEffects(
  ...
  private CFANode handleShortcuttingOperators(
    ...
    switch (binExp.getOperator()) {
      case IASTBinaryExpression.op_logicalAnd:
        createConditionEdges(binExp.getOperand1(), fileLocation, rootNode, intermediateNode,
            elseNode);
        break;
      case IASTBinaryExpression.op_logicalOr:
        createConditionEdges(binExp.getOperand1(), fileLocation, rootNode, thenNode,
            intermediateNode);
        break;
      default:
        throw new AssertionError();
    ...
    if (tempVar != null) {
      ...
      CFAEdge trueEdge =
          new CStatementEdge(binExp.getRawSignature(), assignOne, FileLocation.DUMMY, thenNode,
              lastNode);
      addToCFA(trueEdge);
      ...
      CFAEdge falseEdge =
          new CStatementEdge(binExp.getRawSignature(), assignZero, FileLocation.DUMMY, elseNode,
              lastNode);
      addToCFA(falseEdge);
  ...
  private CFANode handleTernaryOperator(
      IASTConditionalExpression condExp,
      CFANode rootNode, CIdExpression tempVar) {
    ...
    Optional<CExpression> condition =
        createConditionEdges(condExp.getLogicalConditionExpression(), fileLocation, rootNode,
            thenNode, elseNode);
    ...
    if (condExp.getPositiveResultExpression() == null) {
      ...
      if (condition.isPresent()) {
        createEdgesForTernaryOperatorBranch(condition.get(),
            condExp.getLogicalConditionExpression(), lastNode, fileLocation, thenNode, tempVar);
      } else {
        createEdgesForTernaryOperatorBranch(condExp.getLogicalConditionExpression(), lastNode,
            fileLocation, thenNode, tempVar);
      }
    } else {
      createEdgesForTernaryOperatorBranch(condExp.getPositiveResultExpression(), lastNode,
          fileLocation, thenNode, tempVar);
    }

    createEdgesForTernaryOperatorBranch(condExp.getNegativeResultExpression(), lastNode,
        fileLocation, elseNode, tempVar);
  ...
  private void createEdgesForTernaryOperatorBranch(
    ...
    if (!sideAssignmentStack.hasConditionalExpression()) {
      ...
      if (exp instanceof CStatement) {
        ...
        CFAEdge edge =
            new CStatementEdge(condExp.getRawSignature(), (CStatement) exp, fileLocation, prevNode,
                middle);
        addToCFA(edge);
  ...
  private CFANode createEdgesForSideEffects(
    ...
    for (CAstNode sideeffect : sideeffects) {
      ...
      if (sideeffect instanceof CExpression) {
        sideeffect =
            new CExpressionStatement(sideeffect.getFileLocation(), (CExpression) sideeffect);
      }

      CFAEdge edge;
      if (sideeffect instanceof CStatement) {
        ((CStatement) sideeffect).accept(checkBinding);
        edge = new CStatementEdge(rawSignature, (CStatement) sideeffect, fileLocation, prevNode,
            nextNode);
        ...
        edge = new CDeclarationEdge(rawSignature, fileLocation, prevNode, nextNode,
            (CDeclaration) sideeffect);
      } else {
        throw new AssertionError();
      }
      addToCFA(edge);
  ...
  private CStatement createStatement(
    ...
    if (leftHandSide != null) {
      ...
      if (rightHandSide instanceof CExpression) {
        return new CExpressionAssignmentStatement(fileLoc, leftHandSide,
            (CExpression) rightHandSide);
      } else if (rightHandSide instanceof CFunctionCallExpression) {
        return new CFunctionCallAssignmentStatement(fileLoc, leftHandSide,
