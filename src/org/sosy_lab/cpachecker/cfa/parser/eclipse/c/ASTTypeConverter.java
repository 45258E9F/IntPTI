...
public class ASTTypeConverter {
  ...
  ASTTypeConverter(
      Scope pScope, ASTConverter pConverter, String pFilePrefix,
  ...
  private CType convert0(IType t) {
    if (t instanceof IBasicType) {
      ...
      if (oldType != null) {
        return new CElaboratedType(false, false, kind, oldType.getName(), oldType.getOrigName(),
            oldType);
      }
      ...
      //       to false
      CCompositeType compType = new CCompositeType(false, false, kind,
          ImmutableList.<CCompositeTypeMemberDeclaration>of(), name, name);
      ...
      typeConversions.get(filePrefix)
  ...
  public static CType conv(final IBasicType t) {
  ...
  private CTypedefType conv(final ITypedef t) {
    ...
    if (oldType != null) {
      ...
      return new CTypedefType(false, false, scope.getFileSpecificTypeName(name),
          convert(t.getType()));
    }
  }
  ...
  private CArrayType conv(final ICArrayType t) {
    ...
    if (v != null && v.numericalValue() != null) {
      length = new CIntegerLiteralExpression(FileLocation.DUMMY, CNumericTypes.INT,
          BigInteger.valueOf(v.numericalValue()));
    } else {
      try {
    ...
    switch (dd.getType()) {
      ...
      case IASTSimpleDeclSpecifier.t_void:
        if (dd.isComplex() || dd.isImaginary()
          ...
          throw new CFAGenerationRuntimeException("Void type with illegal modifier", dd,
              niceFileNameFunction);
        }
