...
public class ProgramDeclarations {
  ...
  public void registerTypeDefDeclaration(CTypeDefDeclaration declaration) {
    ...
    verify(shouldBeNull == null, "There is already a typedeftype registered with the name: %s",
        name);
  }
  ...
  public void registerVariableDeclaration(CVariableDeclaration declaration) {
    ...
    verify(shouldBeNull == null, "There is already a global variable registered with the name: %s",
        name);
  }
  ...
  public void completeUncompletedElaboratedTypes() {
    ...
    for (CElaboratedType type : elaboratedTypes.values()) {
      ...
      CElaboratedType newType =
          new CElaboratedType(type.isConst(), type.isVolatile(), type.getKind(), type.getOrigName(),
              type.getOrigName(), null);
  ...
  private static boolean areEqualTypes(CType type1, CType type2) {
    return areEqualTypes(type1, type2, new HashMap<Pair<CType, CType>, Boolean>(),
        new HashSet<Pair<CType, CType>>());
  }

  /**
   * To compare recursive types, we add induction hypothesis indicating that two composite types
   * T1 and T2 are assumed to be equal.
   */
  private static boolean areEqualTypes(
      CType type1,
      CType type2,
      Map<Pair<CType, CType>, Boolean> foundTypes,
      Set<Pair<CType, CType>> hypoTypes) {
    ...
    if (type1 instanceof CComplexType) {
      ...
      if (isOuterTypeEqual) {
        if (type1 instanceof CCompositeType) {
          if (!foundTypes.containsKey(typePair)) {
            boolean areEqual;
            if (hypoTypes.contains(typePair)) {
              areEqual = true;
            } else {
              hypoTypes.add(typePair);
              areEqual = areEqualCompositeTypes((CCompositeType) type1, (CCompositeType) type2,
                  foundTypes, hypoTypes);
              foundTypes.put(typePair, areEqual);
              hypoTypes.remove(typePair);
            }
            return areEqual;
      ...
      if (a1.getLength() instanceof CIntegerLiteralExpression && a2
          .getLength() instanceof CIntegerLiteralExpression) {
        if (!((CIntegerLiteralExpression) a1.getLength()).getValue()
  ...
  private static boolean areEqualCompositeTypes(
      CCompositeType type1,
      CCompositeType type2,
      Map<Pair<CType, CType>, Boolean> foundTypes,
      Set<Pair<CType, CType>> hypoTypes) {
    ...
    for (int i = 0; i < members1.size() && areEqual; i++) {
      ...
      areEqual = areEqual && areEqualTypes(typeM1.getCanonicalType(), typeM2.getCanonicalType(),
          foundTypes, hypoTypes);
    }
  ...
  private static boolean areEqualEnumTypes(CEnumType type1, CEnumType type2) {
    ...
    for (int i = 0; i < members1.size() && areEqual; i++) {
      ...
      areEqual = areEqual && (
          (member1.hasValue() && member2.hasValue() && member1.getValue() == member2.getValue())
  ...
  private static boolean areEqualFunctionTypes(
    ...
    boolean areEqual = (type1 instanceof CFunctionTypeWithNames
        ...
        && !((CFunctionTypeWithNames) type1).getName()
        .equals(((CFunctionTypeWithNames) type2).getName()))
    ...
    areEqual = areEqual && areEqualTypes(type1.getReturnType().getCanonicalType(),
        type2.getReturnType().getCanonicalType(), foundTypes, hypoTypes);
    ...
    for (int i = 0; areEqual && i < params1.size(); i++) {
      areEqual = areEqualTypes(params1.get(i).getCanonicalType(), params2.get(i).getCanonicalType(),
          foundTypes, hypoTypes);
    }
