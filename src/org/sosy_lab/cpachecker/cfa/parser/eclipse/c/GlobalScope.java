...
class GlobalScope extends AbstractScope {
  ...
  public boolean registerTypeDeclaration(CComplexTypeDeclaration declaration) {
    ...
    // type, as this would have been captured in the if clause above, thus it wasgit
    ...
    if (types.containsKey(getFileSpecificTypeName(name))) {
      ...
      if (!(oldType.getCanonicalType() instanceof CElaboratedType)
          && (oldType instanceof CElaboratedType && ((CElaboratedType) oldType).getRealType() !=
          type)) {
    ...
    if (oldType instanceof CCompositeType) {
      ...
      for (CCompositeTypeMemberDeclaration decl : oldCompositeType.getMembers()) {
        ...
        if (decl.getType() instanceof CPointerType) {
          newMembers.add(new CCompositeTypeMemberDeclaration(
              createPointerField((CPointerType) decl.getType(), oldType,
      ...
      for (CEnumerator c : ((CEnumType) oldType).getEnumerators()) {
        CEnumerator newC = new CEnumerator(c.getFileLocation(), c.getName(), c.getQualifiedName(),
            c.hasValue() ? c.getValue() : null);
        list.add(newC);
      }

      CEnumType renamedEnumType =
          new CEnumType(oldType.isConst(), oldType.isVolatile(), list, newName,
              oldType.getOrigName());
      ...
      return new CElaboratedType(oldType.isConst(), oldType.isVolatile(), oldType.getKind(),
          newName, oldType.getOrigName(), renamedRealType);
  ...
  private CType createPointerField(CPointerType oldType, CType eqType, CType newType) {
    if (oldType.getType() instanceof CPointerType) {
      return new CPointerType(oldType.isConst(), oldType.isVolatile(),
  ...
  public boolean registerTypeDeclaration(CTypeDefDeclaration declaration) {
    ...
    if (typedefs.containsKey(name)) {
      ...
      if (oldType.getCanonicalType() instanceof CElaboratedType && type
          .getCanonicalType() instanceof CCompositeType
