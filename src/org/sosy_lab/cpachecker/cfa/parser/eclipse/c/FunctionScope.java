...
class FunctionScope extends AbstractScope {
  ...
  public void enterFunction(CFunctionDeclaration pFuncDef) {
    ...
    checkArgument(globalFunctions.containsKey(getCurrentFunctionName()) || localFunctions
        .containsKey(getCurrentFunctionName()));
  ...
  public boolean registerTypeDeclaration(CComplexTypeDeclaration declaration) {
    ...
    if (lookupType(typeName) != null) {
      throw new CFAGenerationRuntimeException("Shadowing types are currently not supported",
          declaration);
    }
  ...
  public String toString() {
    return "Functions: " + Joiner.on(' ').join(globalFunctions.keySet()) + Joiner.on(' ')
        .join(localFunctions.keySet());
  }
}
