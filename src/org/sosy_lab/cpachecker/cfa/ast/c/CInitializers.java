...
public final class CInitializers {
  ...
  private static List<CExpressionAssignmentStatement> handleInitializerList(
    ...
    { // For starting, we go to the first very subobject of the "current object".
      ...
      if (currentType instanceof CCompositeType
        ...
        throw new UnrecognizedCCodeException(
            "Unexpected initializer for " + currentType + " that is not fully defined", edge,
            initializerList);
      } else {
        return ImmutableList.of();
        //TODO, statement like return __extension__ (__m128d){ __F, __F } should be handled
        //throw new UnrecognizedCCodeException("Unexpected initializer list for " + currentObject
        //+ " with type " + currentType, edge, initializerList);
      }

      if (!successful) {
        // struct or array was empty, no initializing needed
        if (!initializerList.getInitializers().isEmpty()) {
          throw new UnrecognizedCCodeException("Too many values in initializer list", edge,
              initializerList);
        }
        return ImmutableList.of();
      }
    ...
    List<CExpressionAssignmentStatement> result =
        new ArrayList<>(initializerList.getInitializers().size());
    for (CInitializer init : initializerList.getInitializers()) {
      ...
      if (init instanceof CDesignatedInitializer) {
        ...
        if (init instanceof CDesignatedInitializer) {
          throw new UnrecognizedCCodeException("Too complex struct initializer", edge,
              initializerList);
        }
      }
    ...
    for (CDesignator designator : designators) {
      ...
      if (designator instanceof CFieldDesignator) {
        ...
        if (!(indexExp instanceof CIntegerLiteralExpression)) {
          throw new UnrecognizedCCodeException("Cannot evaluate expression as array designator",
              edge, designator);
        }
        ...
        successful = true;
        for (long index = indexBottom.longValue(); index < indexTop.longValue() && successful;
             index++) {
        ...
        throw new UnrecognizedCCodeException("Unrecognized initializer designator", edge,
            designator);
      }

      if (!successful) {
        throw new UnrecognizedCCodeException("Empty struct or array is not supported as field",
            edge, currentSubobject);
      }
    }
  }
  ...
  private static boolean handleInitializerForArray(
    ...
    if (arrayType.getLength() instanceof CIntegerLiteralExpression) {
      ...
      if (!BigInteger.valueOf(size.longValue()).equals(size)) {
        throw new UnrecognizedCCodeException(
            "Size of type " + arrayType + " is too large to initialize explicitly", edge,
            designator);
      }
      ...
      if (edge instanceof CDeclarationEdge) {
        ...
        if (declaration instanceof CVariableDeclaration) {
          AInitializer initializer = ((CVariableDeclaration) declaration).getInitializer();
          if (initializer instanceof CInitializerList && ((CInitializerList) initializer)
      ...
      throw new UnrecognizedCCodeException(
          "Cannot initialize arrays with variable modified type like " + arrayType, edge,
          designator);
    }
