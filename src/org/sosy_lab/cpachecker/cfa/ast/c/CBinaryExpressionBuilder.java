...
public class CBinaryExpressionBuilder {
   ...
   * @param pLogger       logging, can be null
   */
  ...
  public CBinaryExpression buildBinaryExpression(
      FileLocation loc, CExpression op1, CExpression op2,
      BinaryOperator op)
    ...
    return new CBinaryExpression(loc, resultType, calculationType, op1, op2, op);
  }

  /**
   * This method does the same as
   * {@link #buildBinaryExpression(FileLocation, CExpression, CExpression, BinaryOperator)},
   * but does not throw a checked exception.
   * Use this whenever the operation is guaranteed to be valid.
   */
  public CBinaryExpression buildBinaryExpressionUnchecked(
      FileLocation loc, CExpression op1,
      CExpression op2, BinaryOperator op) {
    try {
      return buildBinaryExpression(loc, op1, op2, op);
  ...
  CType getResultTypeForBinaryOperation(
      final CType pType1, final CType pType2,
  ...
  CType getCalculationTypeForBinaryOperation(
    ...
    if (pType1 instanceof CSimpleType && pType2 instanceof CSimpleType) {
      ...
      final CType commonType = getCommonSimpleTypeForBinaryOperation(machineModel, (CSimpleType)
          pType1, (CSimpleType) pType2);
    ...
    if (pType1 instanceof CSimpleType) {
      return getSecondTypeToSimpleType(pType2, pBinOperator, op1, op2);
    }
    if (pType2 instanceof CSimpleType) {
      return getSecondTypeToSimpleType(pType1, pBinOperator, op1, op2);
    }


    // both are pointer or function-pointer
    if ((pType1 instanceof CPointerType || pType1 instanceof CFunctionType
        || pType1 instanceof CArrayType)
        && (pType2 instanceof CPointerType || pType2 instanceof CFunctionType
        || pType2 instanceof CArrayType)) {

      if (pBinOperator == BinaryOperator.MINUS) {
        return machineModel.getPointerDiffType();
      }
   ...
   */
  private CType getSecondTypeToSimpleType(
      final CType pType,
      final BinaryOperator pBinOperator, final CExpression op1, final CExpression op2)
      throws UnrecognizedCCodeException {
    ...
    if (pType instanceof CPointerType) {
      if (!additiveOperators.contains(pBinOperator) && !relationalOperators
          .contains(pBinOperator)) {
        throw new UnrecognizedCCodeException(
   ...
   */
  public static CSimpleType getCommonSimpleTypeForBinaryOperation(
      MachineModel model,
      CSimpleType t1, CSimpleType t2) {
    ...
    if (t1.getType() == DOUBLE) {
      return t1;
    }
    if (t2.getType() == DOUBLE) {
      return t2;
    }
    ...
    return getLongestIntegerPromotion(model, t1.getCanonicalType(), t2.getCanonicalType());
  }


  private static CSimpleType getLongestIntegerPromotion(
      MachineModel model, CSimpleType t1,
      CSimpleType t2) {
    ...
    t1 = model.getPromotedCType(t1);
    t2 = model.getPromotedCType(t2);
    ...
    final int size1 = model.getSizeof(t1);
    final int size2 = model.getSizeof(t2);
    ...
    if (t1.isSigned()) {
      return new CSimpleType(false, false, INT,
