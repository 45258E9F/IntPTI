...
public class CFADeclarationMover {
  ...
  private void handleDeclarationsInFunction(FunctionEntryNode startNode, MutableCFA cfa) {
    ...
    if (!declarations.isEmpty()) {
      ...
      cfa.addNode(tmpNode);
      CFAEdge declEndEdge =
          new BlankEdge("End of Declarations", FileLocation.DUMMY, actNode, tmpNode,
              "End of Declarations");
  ...
  private void moveDeclEdgeToNewLocation(CDeclarationEdge edge, CFANode pred, CFANode succ) {
    ...
    if (init instanceof CInitializerExpression) {
      ...
      CExpressionAssignmentStatement stmt =
          new CExpressionAssignmentStatement(varDecl.getFileLocation(),
              new CIdExpression(varDecl.getFileLocation(), varDecl),
              ((CInitializerExpression) init).getExpression());
      CStatementEdge midEdge =
          new CStatementEdge(edge.getRawStatement(), stmt, edge.getFileLocation(), actPred,
              actSucc);
      ...
      actSucc.removeEnteringEdge(edge);
      BlankEdge midEdge =
          new BlankEdge(edge.getRawStatement(), edge.getFileLocation(), actPred, actSucc,
    ...
    CVariableDeclaration declWithoutInitializer =
        new CVariableDeclaration(varDecl.getFileLocation(), varDecl.isGlobal(),
            varDecl.getCStorageClass(), varDecl.getType(), varDecl.getName(), varDecl.getOrigName(),
            varDecl.getQualifiedName(), null);
    CDeclarationEdge newEdge =
        new CDeclarationEdge(edge.getRawStatement(), edge.getFileLocation(), pred, succ,
            declWithoutInitializer);
