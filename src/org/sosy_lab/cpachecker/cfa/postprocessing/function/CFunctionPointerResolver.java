...
public class CFunctionPointerResolver {
  ...
  private enum FunctionSet {
    ...
    ALL,
    //all defined functions considered (Warning: some CPAs require at least EQ_PARAM_SIZES)
    USED_IN_CODE,
    //includes only functions which address is taken in the code
    EQ_PARAM_COUNT,
    //all functions with matching number of parameters considered
    EQ_PARAM_SIZES,
    //all functions with parameters with matching sizes
    EQ_PARAM_TYPES,
    //all functions with matching number and types of parameters considered (implies EQ_PARAM_SIZES)
  ...
  private Set<FunctionSet> functionSets = ImmutableSet
      .of(FunctionSet.USED_IN_CODE, FunctionSet.EQ_PARAM_SIZES, FunctionSet.RETURN_VALUE);
  ...
  public CFunctionPointerResolver(
      MutableCFA pCfa, List<Pair<ADeclaration, String>> pGlobalVars,
  ...
  private void replaceFunctionPointerCall(CFunctionCall functionCall, CStatementEdge statement) {
    ...
    for (FunctionEntryNode fNode : funcs) {
      ...
      CBinaryExpression condition = binExprBuilder.buildBinaryExpressionUnchecked(FileLocation
          .DUMMY, nameExp, amper, BinaryOperator.EQUALS);
  ...
  private CIdExpression createIdExpression(
      CExpression nameExp,
  ...
  private CFunctionCall createRegularCall(CFunctionCall functionCall, FunctionEntryNode fNode) {
    ...
    CFunctionCallExpression newCallExpr =
        new CFunctionCallExpression(oldCallExpr.getFileLocation(), oldCallExpr.getExpressionType(),
  ...
  private void createCallEdge(
      FileLocation fileLocation, String pRawStatement,
  ...
  private boolean checkReturnAndParamSizes(
    ...
    try {
      ...
      if (machine.getSizeof(declRet) != machine.getSizeof(actRet)) {
        logger.log(Level.FINEST, "Function call", functionCallExpression.toASTString(), "with type",
            actRet,
      ...
      logger.logUserException(Level.INFO, e,
          functionType.toASTString("") + " " + functionCallExpression);
      return true;
    }
