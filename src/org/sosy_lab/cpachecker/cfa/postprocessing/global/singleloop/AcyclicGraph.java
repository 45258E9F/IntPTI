...
class AcyclicGraph {
      ...
      new Function<CFANode, Iterable<CFAEdge>>() {
        ...
        public Iterable<CFAEdge> apply(@Nullable CFANode pArg0) {
          ...
          return getLeavingEdges(pArg0).filter(CONTAINS_EDGE)
              .transform(new Function<CFAEdge, CFAEdge>() {
   ...
   * @throws InterruptedException     if a shutdown has been requested by the given shutdown
   *                                  notifier.
  ...
  public boolean introducesLoop(CFAEdge pEdge, ShutdownNotifier pShutdownNotifier)
      throws InterruptedException {
    return CFAUtils
        .existsPath(pEdge.getSuccessor(), pEdge.getPredecessor(), GET_CONTAINED_LEAVING_EDGES,
            pShutdownNotifier);
  }
