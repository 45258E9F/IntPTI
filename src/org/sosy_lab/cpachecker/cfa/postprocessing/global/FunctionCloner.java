...
class FunctionCloner implements CFAVisitor {
    ...
    switch (edge.getEdgeType()) {
      ...
      case AssumeEdge: {
        if (edge instanceof CAssumeEdge) {
          ...
          newEdge = new CAssumeEdge(rawStatement, loc, start, end, cloneAst(e.getExpression()),
      ...
      case StatementEdge: {
        if (edge instanceof CFunctionSummaryStatementEdge) {
          ...
          newEdge =
              new CStatementEdge(rawStatement, cloneAst(((CStatementEdge) edge).getStatement()),
                  loc, start, end);
        } else {
          throw new AssertionError(ONLY_C_SUPPORTED);
        }
      ...
      case DeclarationEdge: {
        if (edge instanceof CDeclarationEdge) {
          newEdge = new CDeclarationEdge(rawStatement, loc, start, end,
      ...
      case ReturnStatementEdge: {
        ...
        if (edge instanceof CReturnStatementEdge) {
          newEdge = new CReturnStatementEdge(rawStatement,
              cloneAst(((CReturnStatementEdge) edge).getRawAST().get()),
  ...
  private <T extends CFANode> T cloneNode(@Nonnull final T node, final boolean addToMapping) {
    ...
    if (node instanceof CLabelNode) {
      ...
      final CFunctionEntryNode entryNode =
          new CFunctionEntryNode(n.getFileLocation(), cloneAst(n.getFunctionDefinition()),
              newExitNode, n.getFunctionParameterNames(), returnVariable);
      newExitNode.setEntryNode(entryNode); // this must not change hashvalue!
      newNode = entryNode;

    } else {
      assert node.getClass() == CFANode.class : "unhandled subclass for CFANode: " + node
          .getClass();
  ...
  private AAstNode cloneAstDirect(AAstNode ast) {
    ...
    if (ast instanceof CRightHandSide) {
      ...
      if (ast instanceof CInitializerExpression) {
        return new CInitializerExpression(loc,
            cloneAst(((CInitializerExpression) ast).getExpression()));
        ...
        return new CDesignatedInitializer(loc, cloneAstList(di.getDesignators()),
            cloneAst(di.getRightHandSide()));
      }
      ...
      if (ast instanceof CVariableDeclaration) {
        ...
        CVariableDeclaration newDecl =
            new CVariableDeclaration(loc, decl.isGlobal(), decl.getCStorageClass(),
        ...
        return new CFunctionDeclaration(loc, cloneType(decl.getType()), changeName(decl.getName()),
            l);
        ...
        return new CEnumType.CEnumerator(loc, decl.getName(),
            changeQualifiedName(decl.getQualifiedName()), decl.getValue());
      }
      ...
      if (ast instanceof CFunctionCallAssignmentStatement) {
        ...
        return new CFunctionCallAssignmentStatement(loc, cloneAst(stat.getLeftHandSide()),
            cloneAst(stat.getRightHandSide()));
        ...
        return new CExpressionAssignmentStatement(loc, cloneAst(stat.getLeftHandSide()),
            cloneAst(stat.getRightHandSide()));

      } else if (ast instanceof CFunctionCallStatement) {
        return new CFunctionCallStatement(loc,
            cloneAst(((CFunctionCallStatement) ast).getFunctionCallExpression()));

      } else if (ast instanceof CExpressionStatement) {
        return new CExpressionStatement(loc,
            cloneAst(((CExpressionStatement) ast).getExpression()));
      }
      ...
      if (ast instanceof CArrayDesignator) {
        return new CArrayDesignator(loc,
            cloneAst(((CArrayDesignator) ast).getSubscriptExpression()));

      } else if (ast instanceof CArrayRangeDesignator) {
        return new CArrayRangeDesignator(loc,
  ...
  private class CExpressionCloner extends DefaultCExpressionVisitor<CExpression, RuntimeException> {
    ...
    public CExpression visit(CCastExpression exp) {
      return new CCastExpression(exp.getFileLocation(), cloneType(exp.getExpressionType()),
          exp.getOperand().accept(this));
    }

    @Override
    public CExpression visit(CUnaryExpression exp) {
      return new CUnaryExpression(exp.getFileLocation(), cloneType(exp.getExpressionType()),
          exp.getOperand().accept(this), exp.getOperator());
    }

    @Override
    public CExpression visit(CArraySubscriptExpression exp) {
      return new CArraySubscriptExpression(exp.getFileLocation(),
    ...
    public CExpression visit(CPointerExpression exp) {
      return new CPointerExpression(exp.getFileLocation(), cloneType(exp.getExpressionType()),
          exp.getOperand().accept(this));
    }
  ...
  private class CTypeCloner extends DefaultCTypeVisitor<CType, RuntimeException> {
    ...
    public CType visit(CArrayType type) {
      return new CArrayType(type.isConst(), type.isVolatile(), type.getType().accept(this),
          type.getLength());
    }
    ...
    public CType visit(CElaboratedType type) {
      return new CElaboratedType(type.isConst(), type.isVolatile(), type.getKind(), type.getName(),
          type.getOrigName(), cloneType(type.getRealType()));
    }
    ...
    public CType visit(CFunctionType type) {
      ...
      if (type instanceof CFunctionTypeWithNames) {
        ...
        for (CParameterDeclaration param : ((CFunctionTypeWithNames) type)
            .getParameterDeclarations()) {
          l.add(cloneAst(param));
        }
        funcType =
            new CFunctionTypeWithNames(type.isConst(), type.isVolatile(), type.getReturnType(), l,
                type.takesVarArgs());
      } else {
        ...
        funcType = new CFunctionType(type.isConst(), type.isVolatile(), type.getReturnType(), l,
            type.takesVarArgs());
      }
    ...
    public CType visit(CTypedefType type) {
      return new CTypedefType(type.isConst(), type.isVolatile(), type.getName(),
