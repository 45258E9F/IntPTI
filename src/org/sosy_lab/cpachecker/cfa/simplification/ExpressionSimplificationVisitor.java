...
public class ExpressionSimplificationVisitor extends DefaultCExpressionVisitor
      ...
      if (type.getType().isIntegerType()) {
        ...
        try {
          ...
          logger.logf(Level.FINE,
              "Cannot simplify expression to numeric value %s, keeping original expression %s instead",
              numericResult, expr.toASTString());
          return expr;
        }
      }
    }
    ...
    if (value1 == null || value2 == null) {
      ...
      if (op1 == expr.getOperand1() && op2 == expr.getOperand2()) {
        ...
        newExpr = binExprBuilder.buildBinaryExpressionUnchecked(
            expr.getFileLocation(), op1, op2, binaryOperator);
      }
      return newExpr;
    }
  ...
  public CExpression visit(final CTypeIdExpression expr) {
    ...
    switch (idOperator) {
      ...
      case ALIGNOF:
        int align = machineModel.getAlignof(innerType);
        return new CIntegerLiteralExpression(expr.getFileLocation(), expr.getExpressionType(),
            BigInteger.valueOf(align));
  ...
  public CExpression visit(final CUnaryExpression expr) {
    ...
    if (unaryOperator == UnaryOperator.SIZEOF) {
      return new CIntegerLiteralExpression(loc, exprType,
          BigInteger.valueOf(machineModel.getSizeof(operandType)));
    } else if (unaryOperator == UnaryOperator.ALIGNOF) {
      return new CIntegerLiteralExpression(loc, exprType,
          BigInteger.valueOf(machineModel.getAlignof(operandType)));
    }
    ...
    if (value != null && operandType instanceof CSimpleType) {
      if (unaryOperator == UnaryOperator.MINUS) {
        ...
        switch (((CSimpleType) operandType).getType()) {
          ...
          case INT:
            return new CIntegerLiteralExpression(loc, exprType,
                BigInteger.valueOf(negatedValue.longValue()));
          case FLOAT:
          case DOUBLE:
            return new CFloatLiteralExpression(loc, exprType,
                BigDecimal.valueOf(negatedValue.doubleValue()));
          default:
            // fall-through and return the original expression
        }

      } else if (unaryOperator == UnaryOperator.TILDE && ((CSimpleType) operandType).getType()
          .isIntegerType()) {
        ...
        final NumericValue complementValue =
            (NumericValue) AbstractExpressionValueVisitor.castCValue(
                new NumericValue(~value.longValue()), exprType, machineModel, logger, loc);
        return new CIntegerLiteralExpression(loc, exprType,
            BigInteger.valueOf(complementValue.longValue()));
      }
    }
