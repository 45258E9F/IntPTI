...
public class CFASecondPassBuilder {
  ...
  public CFASecondPassBuilder(
      final MutableCFA pCfa, final Language pLanguage, final LogManager pLogger,
  ...
  public void updateLoopStructure(
      String pFunctionName, CFANode Pre, CFANode oldSuc, CFANode
      newSuc) {
    ...
    for (String functionName : cfa.getAllFunctionNames()) {
      if (!functionName.equals(pFunctionName)) {
        loops.putAll(functionName, loopStructure.getLoopsForFunction(functionName));
      }
    }

    ImmutableCollection<Loop> loopsForFunc = loopStructure.getLoopsForFunction(pFunctionName);
    for (Loop loop : loopsForFunc) {
      if (loop.getLoopNodes().contains(Pre) &&
          loop.getLoopNodes().contains(oldSuc)) {
        Set<CFANode> nodes = new HashSet<>();
        nodes.addAll(loop.getLoopNodes());
        nodes.add(newSuc);
        loops.put(pFunctionName, new Loop(loop.getLoopHeads(), nodes));
      } else {
        loops.put(pFunctionName, loop);
      }
    }
    LoopStructure newLoopStructure = LoopStructure.getNewLoopStructure(loops.build());
    cfa.setLoopStructure(Optional.of(newLoopStructure));
  }
   ...
   *                     function call expression, e.g. if statement is a = call(b); then
   *                     functionCall is call(b).
   */
  private void createCallAndReturnEdges(AStatementEdge edge, AFunctionCall functionCall)
    ...
    if (successorNode.getEnteringSummaryEdge() != null) {
      ...
      cfa.addNode(tmp);
      updateLoopStructure(successorNode.getFunctionName(), predecessorNode, successorNode, tmp);
    ...
    switch (language) {
      case C:
        ...
        callEdge = new CFunctionCallEdge(edge.getRawStatement(),
            ...
            (CFunctionEntryNode) fDefNode, (CFunctionCall) functionCall,
      ...
      case JAVA:
        ...
        callEdge = new JMethodCallEdge(edge.getRawStatement(),
            fileLocation, predecessorNode,
            (JMethodEntryNode) fDefNode, (JMethodOrConstructorInvocation) functionCall,
    ...
    if (fExitNode.getNumEnteringEdges() == 0) {
      ...
      switch (language) {
        case C:
          returnEdge = new CFunctionReturnEdge(fileLocation, fExitNode, successorNode,
              (CFunctionSummaryEdge) calltoReturnEdge);
          break;
        case JAVA:
          returnEdge = new JMethodReturnEdge(fileLocation, fExitNode, successorNode,
              (JMethodSummaryEdge) calltoReturnEdge);
          break;
        default:
          throw new AssertionError();
  ...
  private void replaceBuiltinFunction(AStatementEdge edge, AFunctionCall call) {
    ...
    if (!(assumeExp instanceof CBinaryExpression && ((CBinaryExpression) assumeExp).getOperator()
        .isLogicalOperator())) {
      assumeExp = new CBinaryExpressionBuilder(cfa.getMachineModel(), logger)
          .buildBinaryExpressionUnchecked(assumeExp.getFileLocation(),
              assumeExp,
