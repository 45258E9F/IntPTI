...
public class BMCAlgorithm extends AbstractBMCAlgorithm implements Algorithm {
  ...
  protected void analyzeCounterexample(
    ...
    try {
      ...
      ARGPath targetPath;
      try {
        targetPath = ARGUtils.getPathFromBranchingInformation(root, pReachedSet.asCollection(),
      ...
      try {
        ...
        if (solver.getVersion().toLowerCase().contains("smtinterpol")) {
          ...
          pmgr = new PathFormulaManagerImpl(formulaManager, config, logger, shutdownNotifier, cfa,
        ...
        logger.logUserException(Level.WARNING, e,
            "Could not replay error path to get a more precise model");
        return;
      }
      ...
      ((ARGCPA) cpa)
          .addCounterexample(counterexample.getTargetPath().getLastState(), counterexample);
