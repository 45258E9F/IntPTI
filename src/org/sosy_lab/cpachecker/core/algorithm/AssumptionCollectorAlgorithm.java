...
public class AssumptionCollectorAlgorithm implements Algorithm, StatisticsProvider {
  ...
  public AssumptionCollectorAlgorithm(
      Algorithm algo, ConfigurableProgramAnalysis pCpa,
      CFA pCFA,
    ...
    if (this.cpa == null) {
      throw new InvalidConfigurationException(
          "AssumptionStorageCPA needed for AssumptionCollectionAlgorithm");
    }
...
//        logger.log(Level.FINER, "Dumping assumptions due to: " + e.toString());
  ...
  private AssumptionWithLocation collectLocationAssumptions(
      ReachedSet reached,
      AssumptionWithLocation exceptionAssumptions) {
  ...
  private void addAssumption(
      ...
      AbstractState state) {
    BooleanFormula dataRegion =
        AbstractStates.extractReportedFormulas(formulaManager, state, pfmgr);
  ...
  private void produceAssumptionAutomaton(Appendable output, ReachedSet reached)
      throws IOException {
   ...
   * @param relevantStates        A set with all states with non-trivial assumptions (all others
   *                              will have assumption TRUE).
  ...
  public static int writeAutomaton(
    ...
    for (final ARGState s : relevantStates) {
      ...
      if ((branchingThreshold > 0) && (s.getChildren().size() > 1)) {
        branching = true;
        sb.append("    branchingCount == branchingThreshold -> " + actionOnFinalEdges
            + "GOTO __FALSE;\n");
      }
          ...
          for (CFAEdge innerEdge : from(((MultiEdge) edge).getEdges()).skip(1)) {
            ...
            if (!first) {
              ...
              descriptionForInnerMultiEdges
                  .append("GOTO ARG" + s.getStateId() + "M" + multiEdgeID + ";\n");
              descriptionForInnerMultiEdges
                  .append("    TRUE -> " + actionOnFinalEdges + "GOTO __TRUE;\n\n");
            } else {
              first = false;
            }

            descriptionForInnerMultiEdges
                .append("STATE USEFIRST ARG" + s.getStateId() + "M" + multiEdgeID + " :\n");
            numProducedStates++;
  ...
  private static void addAssumption(
      final Appendable writer, final AssumptionStorageState assumptionState,
      boolean ignoreAssumptions) throws IOException {
    if (!ignoreAssumptions) {
  ...
  private class AssumptionCollectionStatistics extends AbstractStatistics {
    ...
    public void printStatistics(PrintStream out, Result pResult, ReachedSet pReached) {
      AssumptionWithLocation resultAssumption =
          collectLocationAssumptions(pReached, exceptionAssumptions);
