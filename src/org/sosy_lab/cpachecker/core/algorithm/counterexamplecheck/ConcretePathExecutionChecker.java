...
public class ConcretePathExecutionChecker implements CounterexampleChecker, Statistics {
  ...
  public ConcretePathExecutionChecker(Configuration config, LogManager logger, CFA cfa, ARGCPA cpa)
  ...
  public boolean checkCounterexample(
      ARGState pRootState, ARGState pErrorState,
    ...
    if (dumpFile != null) {
      ...
      try (DeleteOnCloseFile tempFile = Files.createTempFile("concretePath", ".c")) {
        ...
        throw new CounterexampleAnalysisFailed("Could not create temporary file " + e.getMessage(),
            e);
      }
    }
  }
  ...
  private void compilePathProgram(String absFilePath)
    ...
    String[] cmdLine =
        {pathToCompiler.getAbsolutePath(), absFilePath, "-o", absFilePath + ".exe", "-w"};
    ...
    switch (exitCode) {
      case 0: // Verification successful (Path is infeasible)
        logger.log(Level.FINER,
            "Concrete path program was executed, the error location was infeasible.");
        return false;
      case 1: // Verification failed (Path is feasible)
        logger.log(Level.FINER,
            "Concrete path program was executed, the error location was reached.");
        return true;
      default:
  ...
  private boolean checkCounterexample(
    ...
    Appender pathProgram = PathToConcreteProgramTranslator
        .translatePaths(pRootState, pErrorPathStates, ceInfo.getCFAPathWithAssignments());
