...
public class AutomaticCPAFactory implements CPAFactory {
  ...
  public static CPAFactory forType(String className, ClassLoader cl) {
    ...
    checkArgument(ConfigurableProgramAnalysis.class.isAssignableFrom(cls),
        "Class %s does not implement the CPA interface and cannot be used with AutomaticCPAFactory",
        className);
  ...
  public ConfigurableProgramAnalysis createInstance()
    ...
    for (int i = 0; i < formalParameters.length; i++) {
      ...
      if (!optional) {
        Preconditions.checkNotNull(actualParam,
            "%s instance needed to create %s-CPA!", formalParam.getSimpleName(),
            type.getSimpleName());
      }
    ...
    String exception = Classes
        .verifyDeclaredExceptions(cons, InvalidConfigurationException.class, CPAException.class);
    if (exception != null) {
      throw new UnsupportedOperationException(
          "Cannot automatically create CPAs if the constructor declares the unsupported checked exception "
              + exception);
    }

    if (!childCpaInjected && injects.containsKey(ConfigurableProgramAnalysis.class)) {
      throw new InvalidConfigurationException(
          "Child CPA configured for " + type.getSimpleName() + ", but this is not a wrapper CPA.");
    }
  ...
  public <T> AutomaticCPAFactory withOptions(final Class<T> optionsClass) {
    ...
    // verify types of declared exceptions
    String exception = Classes
        .verifyDeclaredExceptions(constructor, InvalidConfigurationException.class,
            CPAException.class);
    if (exception != null) {
      throw new IllegalArgumentException(
          "Constructor of options holder class declares illegal checked exception: " + exception);
    }
  ...
  private static final class AutomaticCPAFactoryWithOptions<T> extends AutomaticCPAFactory {
    ...
    public ConfigurableProgramAnalysis createInstance()
      ...
      try {
        ...
        throw new UnsupportedOperationException(
            "Cannot automatically create CPAs without an accessible constructor for their options class!",
            e);

      } catch (InstantiationException e) {
        throw new UnsupportedOperationException(
            "Cannot automatically create CPAs with an abstract options class!", e);
      }
