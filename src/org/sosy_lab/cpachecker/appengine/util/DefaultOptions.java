...
public class DefaultOptions {
  ...
  public boolean setOption(String key, String value) {
    ...
    if (key.equals("limits.time.wall")) {
      if (!getOptions().containsKey("gae.instanceType")
        ...
        try {
          defaultValue =
              Integer.parseInt(getDefault("limits.time.wall").replaceAll("[^0-9]*$", ""));
    ...
    if (key.equals("gae.instanceType") && value.equals("BACKEND")
        && originalWalltimeLimit != null) {
  ...
  public static Map<String, String> getImmutableOptions() throws IOException {
    ...
    try (InputStream in = Paths.get("WEB-INF", "default-options.properties").asByteSource()
        .openStream()) {
      properties.load(in);
    }
