...
public class TaskDAO {
   ...
   * @param limit  The maximum number of tasks to return. If negative or zero, no limit will be
   *               applied
   ...
   * @param limit   The maximum number of {@link Task}s to retrieve. If -1 then no limit will be
   *                applied.
   * @return A {@link List} of {@link Task}s
   */
    ...
    if (task.getRequestID() != null) {
      ...
      for (RequestLogs record : LogServiceFactory.getLogService().fetch(query)) {
        ...
        if (task.getStatus() == Status.PENDING || task.getStatus() == Status.RUNNING) {
          try {
            if (TaskFileDAO
                .loadByName(DefaultOptions.getImmutableOptions().get("statistics.file"), task)
                != null) {
    ...
    if (task.getRequestID() == null) {
      ...
      if (amountOfDetectedRecords > GAETaskQueueTaskExecutor.MAX_RETRIES
        ...
        task.setStatusMessage(
            "The task's request has finished, the task's status did not reflect this and no retries are left");
