...
public class TaskExecutorServerResource extends WadlServerResource implements TaskExecutorResource {
  ...
  public void executeTask(Representation entity) throws Exception {
    ...
    try {
      if (TaskFileDAO.loadByName(ERROR_FILE_NAME, task) != null
          ||
          TaskFileDAO.loadByName(DefaultOptions.getImmutableOptions().get("statistics.file"), task)
              != null) {
        return;
      }
    } catch (IOException e) {
    ...
    Series<Header> headers =
        (Series<Header>) getRequestAttributes().get("org.restlet.http.headers");
    ...
    ShutdownHook shutdownHook = new ShutdownHook() {
      ...
      public void shutdown() {
        shutdownManager.requestShutdown("The backend is shutting down.");
      }
    };
    LifecycleManager.getInstance().setShutdownHook(shutdownHook);

    final CPAchecker cpaChecker = new CPAchecker(config, logManager, shutdownManager);
    ...
    ResourceLimitChecker limits =
        ResourceLimitChecker.fromConfiguration(config, logManager, shutdownManager);
    limits.start();
  ...
  private void dumpConfiguration() throws IOException {
    ...
    if (config != null && config.getProperty("configuration.dumpFile") != null && !config
        .getProperty("configuration.dumpFile").equals("")) {
  ...
  private void dumpStatistics() throws IOException {
    ...
    if (config == null || config.getProperty("statistics.export").equals("false")
        || result == null) {
      return;
    }
