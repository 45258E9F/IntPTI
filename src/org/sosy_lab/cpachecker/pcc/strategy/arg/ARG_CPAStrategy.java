...
public class ARG_CPAStrategy extends AbstractARGStrategy {
  ...
  public ARG_CPAStrategy(
    ...
    super(pConfig, pLogger, pCpa == null ? new DefaultPropertyChecker() : pCpa.getPropChecker(),
        pShutdownNotifier);
    pConfig.inject(this);
    if (pCpa == null) {
      stop = null;
  ...
  protected boolean checkCovering(
    ...
    return checkCoverWithStopOp(pCovered.getWrappedState(),
        Collections.singleton(pCovering.getWrappedState()), pPrecision);
  }
  ...
  protected boolean checkSuccessors(
    ...
    Collection<? extends AbstractState> computedSuccessors =
        transfer.getAbstractSuccessors(pPredecessor.getWrappedState(),
            Lists.<AbstractState>newArrayList(), pPrecision);
  ...
  protected boolean addSuccessors(
      final Collection<ARGState> pSuccessors, final ReachedSet pReachedSet,
  ...
  protected boolean treatStateIfCoveredByUnkownState(
      final ARGState pCovered, final ARGState pCoveringState,
  ...
  private boolean checkCoverWithStopOp(
      final AbstractState pCovered, final Collection<AbstractState> pCoverElems,
      final Precision pPrecision) throws CPAException, InterruptedException {
