...
public class CMCPartitionChecker {
  ...
  public CMCPartitionChecker(
      final ConfigurableProgramAnalysis pCpa, final AtomicBoolean pCheckResult,
  ...
  public void checkPartition(
      final AbstractState[] partitionNodes, final AbstractState[] externalNodes,
      final int[][] partitionEdgesAdjacencyList, final int maxProofSize)
    ...
    Multimap<CFANode, AbstractState> statesPerLocation =
        computeMappingStatesToLocation(partitionNodes, externalNodes);
    ...
    try {
      ...
      for (int i = 0; i < partitionNodes.length; i++) {
        ...
        if (successorEdges == null) {
          // standard case
          checkSuccessorsStandard(transfer.getAbstractSuccessors(current,
              Lists.<AbstractState>newArrayList(), initPrec), statesPerLocation, maxProofSize);
          ...
          if (successorEdges.length == 0) {
            ...
            checkSuccessorsInRecomputedARG(transfer.getAbstractSuccessors(current,
                Lists.<AbstractState>newArrayList(), initPrec),
                getARGSuccessors(successorEdges, partitionNodes, externalNodes), maxProofSize,
  ...
  private void addStatesToMapping(
      final AbstractState[] stateSet,
      final Multimap<CFANode, AbstractState> mapping) {
  ...
  private void checkSuccessorsStandard(
      ...
      final Multimap<CFANode, AbstractState> pStatesPerLocation, final int maxSize)
    ...
    while (!successorsToInspect.isEmpty()) {
      ...
      if (!stopOp.stop(successor, pStatesPerLocation.get(AbstractStates.extractLocation(successor)),
          initPrec)) {
        // recomputed state
        ...
        // compute successors
        successorsToInspect.addAll(transfer.getAbstractSuccessors(successor,
            Lists.<AbstractState>newArrayList(), initPrec));
      }
    }
  }
  ...
  private void checkSuccessorsInRecomputedARG(
    ...
    if (pAbstractSuccessors.size() != 1 || pArgSuccessors.size() > 1) {
      ...
      while (!stopOp.stop(singleState, pArgSuccessors, initPrec)) {
        ...
        successors = transfer.getAbstractSuccessors(singleState,
            Lists.<AbstractState>newArrayList(), initPrec);
  ...
  private void prepareCoverageInspectionOfExternalNodes(
      AbstractState[] pPartitionNodes,
      AbstractState[] pExternalNodes) {
  ...
  public boolean checkCoverageOfExternalsAndInitialState()
      throws CPAException, InterruptedException {
    return stopOp.stop(cpa.getInitialState(AbstractStates.extractLocation(root),
        StateSpacePartition.getDefaultPartition()),
        Collections.singleton(root.getWrappedState()), initPrec)
        && PartitioningUtils
        .areElementsCoveredByPartitionElement(externalNodes, partitionNodes, stopOp, initPrec);
  }
}
