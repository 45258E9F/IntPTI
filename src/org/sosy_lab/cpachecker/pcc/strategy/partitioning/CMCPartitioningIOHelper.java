...
public class CMCPartitioningIOHelper extends PartitioningIOHelper {
  ...
  public CMCPartitioningIOHelper(
      ...
      final Set<ARGState> pUnexploredStates, final @Nullable ARGState pRoot)
  ...
  public CMCPartitioningIOHelper(
      ...
      throws InvalidConfigurationException {
    this(pConfig, pLogger, pShutdownNotifier, Collections.<ARGState>emptySet(),
        Collections.<ARGState>emptySet(), null);
  }

  public
  @Nullable
  int[][] getEdgesForPartition(final int pIndex) {
        ...
        if (isUnexplored(node)) {
          ...
          for (int i = 0; i < successorLinks[count].length; i++) {
            successorLinks[count][i] =
                findSuccessorIndex(pPartialReachedSetDirectedGraph.getNode(successors.get(i)),
                    partitionNodes,
                    externalNodes);
