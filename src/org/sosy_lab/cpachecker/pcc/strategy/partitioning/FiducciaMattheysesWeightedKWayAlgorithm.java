...
public class FiducciaMattheysesWeightedKWayAlgorithm {
  ...
  public FiducciaMattheysesWeightedKWayAlgorithm(
      List<Set<Integer>> pInitPartitioning,
  ...
  /**
   * Refines a partition with a greedy KL/FM-algorithm Visit each node in a randomized order once;
   * check if it can be moved resulting in a better partition
   *
   ...
   * @return true if fromPartition!=toPartition and weight of fromPartition added to node's weight
   * holds balancing constraints
   */
  private boolean isNodeMovable(int node, int fromPartition, int toPartition) {
    ...
    int nodeWeight = wGraph.getNode(node).getWeight();
    if (partWeight + nodeWeight > balanceCriterion * maxLoad) {
      return false;
    }
