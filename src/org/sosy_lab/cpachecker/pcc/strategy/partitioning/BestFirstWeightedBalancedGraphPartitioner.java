...
/**
 * Compute a greedy graph partitioning in best-first-manner Exploration order according to chosen
 * evaluation function Mainly based on the GGGP algorithm described in
 * http://www.cc.gatech.edu/~bader/COURSES/GATECH/CSE6140-Fall2007/papers/KK95a.pdf, with a modified
 * best_improvment evaluation function
 */
...
public class BestFirstWeightedBalancedGraphPartitioner implements WeightedBalancedGraphPartitioner {
   ...
   */
  public BestFirstWeightedBalancedGraphPartitioner(
  ...
  public List<Set<Integer>> computePartitioning(
    ...
    BitSet inPartition =
        new BitSet(wGraph.getNumNodes()); //Indicates whether node already in a partition
    ...
    List<Integer> partitionWeights =
        new ArrayList<>(pNumPartitions); //take nodes' weight into account
    ...
    while (nextUnpartitionedNode < wGraph.getNumNodes() && nextUnpartitionedNode >= 0) {
      ...
      while (!waitlist.isEmpty()) {
        ...
        if (!inPartition.get(nextNode.getNodeNumber())) { //duplicate nodes in waitlist possible
          ...
          if (partitionWeights.get(partition) + nodeWeight
              > balancePrecision * partitionSize) { // next partition
            partition++;
            if (partition
                >= pNumPartitions) { //Due to weighted nodes, pNumPartitions maybe to small
              result.add(Sets.<Integer>newHashSetWithExpectedSize(partitionSize));
              partitionWeights.add(0);
            }
          }
          result.get(partition).add(nextNode.getNodeNumber());
          partitionWeights.set(partition, partitionWeights.get(partition)
              + nodeWeight); //nodeWeight added to the partition weight
      ...
      nextUnpartitionedNode =
          inPartition.nextClearBit(nextUnpartitionedNode); //The next node without partition so far
    }
