...
public class PartitioningIOHelper {
  ...
  protected PartitioningIOHelper(
      final Configuration pConfig, final LogManager pLogger,
      final ShutdownNotifier pShutdownNotifier, final boolean withCMC)
    ...
    partitioner = GraphPartitionerFactory
        .createPartitioner(logger, partitioningStrategy, pShutdownNotifier, pConfig);
  }
  ...
  public
  @Nullable
  Pair<AbstractState[], AbstractState[]> getPartition(int pIndex) {
  ...
  protected void saveInternalProof(
    ...
    for (Set<Integer> partition : pPartitionDescription.getSecond()) {
      partitions.add(Pair.of(pPartitionDescription.getFirst().getSetNodes(partition, false),
          pPartitionDescription
              .getFirst()
      ...
      return Pair.of(graph,
          partitioner.computePartitioning(
              maxNumElemsPerPartition <= 0 ? 1
                                           : (int) Math.ceil(graph.getNumNodes()
                                               / (double) maxNumElemsPerPartition), graph));
    } else {
      return Pair.of(graph,
          partitioner.computePartitioning(
              maxNumElemsPerPartition <= 0 ? 1 : (int) Math
                  .ceil(pReached.size() / (double) maxNumElemsPerPartition),
              graph));
    }
  }
  ...
  public void readPartition(
      final ObjectInputStream pIn,
      final PCStrategyStatistics pStats,
      final Lock pLock)
      throws ClassNotFoundException, IOException {
    if (pLock == null) {
      throw new IllegalArgumentException("Cannot protect against parallel access");
    }
    Pair<AbstractState[], AbstractState[]> result = readPartitionContent(pIn);
  ...
  public void writeMetadata(
      ...
      final int pReachedSetSize,
      final int pNumPartitions)
      throws IOException {
  ...
  public void writePartition(
      final ObjectOutputStream pOut, final Set<Integer> pPartition,
  ...
  public void writePartition(
      ...
      Pair<AbstractState[], AbstractState[]> pPartition)
      throws IOException {
    writePartition(pOut, pPartition.getFirst(), pPartition.getSecond());
  }

  private void writePartition(
      final ObjectOutputStream pOut, final AbstractState[] pPartitionNodes,
  ...
  private class PartitioningStatistics implements Statistics {
    ...
    private void computeAndPrintDetailedPartitioningStats(PrintStream pOut) {
      int maxP = 0, maxO = 0, minP = Integer.MAX_VALUE, minO = Integer.MAX_VALUE, totalO = 0,
          totalS = 0, current;
      ...
      pOut.format(Locale.ENGLISH, "Avg. partition size:  %.2f%n",
          ((double) totalS) / numPartitions);
      pOut.format(Locale.ENGLISH, "Avg. partition overhead: %.2f%n",
