...
public class ARGProofCheckerParallelStrategy extends SequentialReadStrategy {
    ...
    try {
      ...
      for (int i = 0; i < args.length - 2; i++) {
        ...
        for (int j = helper.length * numElems; j < argStates.size(); j++) {
          if (!checkInnerElement(propChecker, checker, argStates.get(j), block,
              partialReturnNodes)) {
            return false;
          }
        }
        ...
        // add ARG as checked
        ((BAMCPA) checker).getTransferRelation()
            .setCorrectARG(Pair.of(args[i], block), returnNodes);
      }
  ...
  private static boolean checkElement(
    ...
    try {
      ...
      if (toCheck.isCovered()) {
        ...
        if (!checker.areAbstractSuccessors(toCheck, Lists.<AbstractState>newArrayList(), null,
            successors)) {
          return false;
        }
      }
  ...
  private static class StateCheckingHelper implements Runnable {
    ...
    public StateCheckingHelper(
        CyclicBarrier pBarrier, CommonResult pResult, PropertyChecker pPropCheck,
