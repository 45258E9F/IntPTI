...
public class PartialReachedSetParallelIOCheckingInterleavedStrategy extends AbstractStrategy {
  ...
  public boolean checkCertificate(final ReachedSet pReachedSet)
    ...
    try {
      if (numReadThreads == 0) {
        ...
        startCheckingThreads(numThreads, executor, checkResult, partitionsRead, partitionChecked,
            certificate,
        ...
        startCheckingThreads(numThreads - numReadThreads, checkExecutor, checkResult,
            partitionsRead, partitionChecked,
      ...
      if (!PartitioningUtils.areElementsCoveredByPartitionElement(inOtherPartition, partitionNodes,
          cpa.getStopOperator(),
          initPrec)) {
  ...
  private void startReadingThreads(
      final int threads, final ExecutorService pReadingExecutor, final AtomicBoolean pCheckResult,
      final Semaphore partitionsRead) {
    AtomicInteger nextPartitionId = new AtomicInteger(0);
    for (int i = 0; i < threads; i++) {
      pReadingExecutor
          .execute(new ParallelPartitionReader(pCheckResult, partitionsRead, nextPartitionId, this,
  ...
  private void startCheckingThreads(
    ...
    for (int i = 0; i < threads; i++) {
      pCheckingExecutor.execute(
          new ParallelPartitionChecker(availablePartitions, nextId, pCheckResult, pPartitionsRead,
              pPartitionChecked, pLock, ioHelper, pInPartition, pCertificate, pInOtherPartition,
              pInitialPrecision, cpa
