...
public class BAMTransferRelation implements TransferRelation {
  ...
  public BAMTransferRelation(
      Configuration pConfig, LogManager pLogger, BAMCPA bamCpa,
      ProofChecker wrappedChecker,
      BAMDataManager pData, ShutdownNotifier pShutdownNotifier)
      throws InvalidConfigurationException {
    logger = pLogger;
    algorithmFactory = new CPAAlgorithmFactory(bamCpa, logger, pConfig, pShutdownNotifier, null);
    callstackTransfer = (CallstackTransferRelation) (CPAs.retrieveCPA(bamCpa, CallstackCPA.class))
        .getTransferRelation();
  ...
  public Collection<? extends AbstractState> getAbstractSuccessors(
      final AbstractState pState, List<AbstractState> otherStates, final Precision pPrecision)
  ...
  protected Collection<? extends AbstractState> getAbstractSuccessorsWithoutWrapping(
    ...
    if (startNewBlockAnalysis(pState, node)) {
      ...
      Collection<? extends AbstractState> resultStates =
          doRecursiveAnalysis(pState, pPrecision, node);
    ...
    final Collection<? extends AbstractState> result = wrappedTransfer.getAbstractSuccessors
        (pState, Lists.<AbstractState>newArrayList(), pPrecision);
    if (foundRecursion) {
  ...
  protected boolean startNewBlockAnalysis(final AbstractState pState, final CFANode node) {
    return partitioning.isCallNode(node) && !partitioning.getBlockForCallNode(node)
        .equals(currentBlock);
  }
  ...
  protected Collection<? extends AbstractState> doRecursiveAnalysis(
    ...
    final AbstractState reducedInitialState =
        wrappedReducer.getVariableReducedState(initialState, currentBlock, node);
    final Precision reducedInitialPrecision =
        wrappedReducer.getVariableReducedPrecision(pPrecision, currentBlock);
  ...
  private Collection<Pair<AbstractState, Precision>> getReducedResult(
    ...
    assert cachedReturnStates == null || reached != null
    ...
    if (cachedReturnStates != null && !reached.hasWaitingState()) {
      ...
      logger.log(Level.FINEST, "Cache hit with finished reached-set with root",
      ...
      // cache hit, return element from cache
      logger.log(Level.FINEST, "Cache hit with target-state in reached-set with root",
          reached.getFirstState());
      reducedResult = cachedReturnStates;
    ...
    data.bamCache
        .put(reducedInitialState, reached.getPrecision(reached.getFirstState()), currentBlock,
            reducedResult, rootOfBlock);
  ...
  private Collection<AbstractState> performCompositeAnalysisWithCPAAlgorithm(
    ...
    if (isTargetState(lastState)) {
      ...
      for (AbstractState returnState : AbstractStates
          .filterLocations(reached, currentBlock.getReturnNodes())) {
  ...
  protected AbstractState attachAdditionalInfoToCallNode(AbstractState pElem) {
    if (!(pElem instanceof BAMARGBlockStartState) && PCCInformation.isPCCEnabled()
  ...
  private ARGState createAdditionalInfo(ARGState pElem) {
    ...
    if (partitioning.isCallNode(node) && !partitioning.getBlockForCallNode(node)
        .equals(currentBlock)) {
  ...
  protected void addBlockAnalysisInfo(AbstractState pElement) throws CPATransferException {
    if (PCCInformation.isPCCEnabled()) {
      if (data.bamCache.getLastAnalyzedBlock() == null
  ...
  public Collection<? extends AbstractState> strengthen(
    ...
    return attachAdditionalInfoToCallNodes(
        wrappedTransfer.strengthen(pElement, pOtherElements, pCfaEdge, pPrecision));
  }

  public boolean areAbstractSuccessors(
      AbstractState pState, List<AbstractState>
      pOtherStates, CFAEdge pCfaEdge, Collection<? extends AbstractState> pSuccessors)
      throws CPATransferException, InterruptedException {
    if (pCfaEdge != null) {
      return wrappedProofChecker.areAbstractSuccessors(pState, pOtherStates,
          pCfaEdge, pSuccessors);
    }
    return areAbstractSuccessors0(pState, pOtherStates, pSuccessors, partitioning.getMainBlock());
  }
    ...
    if (partitioning.isCallNode(node)
      ...
      try {
        ...
        for (ARGState leaveB : endOfBlock) {
          ...
          expandedState = wrappedReducer
              .getVariableExpandedStateForProofChecking(pState, analyzedBlock, leaveB);
        ...
        throw new CPATransferException(
            "Checking ARG with root " + ((BAMARGBlockStartState) pState).getAnalyzedBlock()
      ...
      for (CFAEdge leavingEdge : CFAUtils.leavingEdges(node)) {
        ...
        if (!wrappedProofChecker
            .areAbstractSuccessors(pState, pOtherStates, leavingEdge, pSuccessors)) {
          return false;
        }
      }
    }
  ...
  private Pair<Boolean, Collection<ARGState>> checkARGBlock(
      ARGState rootNode,
    ...
    while (!waitlist.isEmpty()) {
      ...
      if (!areAbstractSuccessors0(current, Lists.<AbstractState>newArrayList(), current
          .getChildren(), currentBlock)) {
        returnNodes = Collections.emptyList();
        return Pair.of(false, returnNodes);
      }
