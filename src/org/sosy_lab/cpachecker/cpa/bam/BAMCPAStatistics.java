...
class BAMCPAStatistics implements Statistics {
  ...
  private final Predicate<Pair<ARGState, ARGState>> highlightSummaryEdge =
      new Predicate<Pair<ARGState, ARGState>>() {
        @Override
  ...
  public void printStatistics(PrintStream out, Result result, ReachedSet reached) {
    ...
    int sumCalls =
        data.bamCache.cacheMisses + data.bamCache.partialCacheHits + data.bamCache.fullCacheHits;
    ...
    out.println("  Number of cache misses:                                       "
        + data.bamCache.cacheMisses + " (" + toPercent(data.bamCache.cacheMisses, sumCalls)
        + " of all calls)");
    out.println("  Number of partial cache hits:                                 "
        + data.bamCache.partialCacheHits + " (" + toPercent(data.bamCache.partialCacheHits,
        sumCalls) + " of all calls)");
    out.println("  Number of full cache hits:                                    "
        + data.bamCache.fullCacheHits + " (" + toPercent(data.bamCache.fullCacheHits, sumCalls)
        + " of all calls)");
    if (data.bamCache.gatherCacheMissStatistics) {
      ...
      out.println("  Number of abstraction caused misses:                          "
          + data.bamCache.abstractionCausedMisses + " (" + toPercent(
          data.bamCache.abstractionCausedMisses, data.bamCache.cacheMisses) + " of all misses)");
      out.println("  Number of precision caused misses:                            "
          + data.bamCache.precisionCausedMisses + " (" + toPercent(
          data.bamCache.precisionCausedMisses, data.bamCache.cacheMisses) + " of all misses)");
      out.println("  Number of misses with no similar elements:                    "
          + data.bamCache.noSimilarCausedMisses + " (" + toPercent(
          data.bamCache.noSimilarCausedMisses, data.bamCache.cacheMisses) + " of all misses)");
    }
    out.println(
        "Time for reducing abstract states:                            " + reducer.reduceTime
            + " (Calls: " + reducer.reduceTime.getNumberOfIntervals() + ")");
    out.println(
        "Time for expanding abstract states:                           " + reducer.expandTime
            + " (Calls: " + reducer.expandTime.getNumberOfIntervals() + ")");
    out.println(
        "Time for checking equality of abstract states:                " + data.bamCache.equalsTimer
            + " (Calls: " + data.bamCache.equalsTimer.getNumberOfIntervals() + ")");
    out.println("Time for computing the hashCode of abstract states:           "
        + data.bamCache.hashingTimer + " (Calls: " + data.bamCache.hashingTimer
        .getNumberOfIntervals() + ")");
    out.println("Time for searching for similar cache entries:                   "
        + data.bamCache.searchingTimer + " (Calls: " + data.bamCache.searchingTimer
        .getNumberOfIntervals() + ")");
    out.println("Time for reducing precisions:                                   "
        + reducer.reducePrecisionTime + " (Calls: " + reducer.reducePrecisionTime
        .getNumberOfIntervals() + ")");
    out.println("Time for expanding precisions:                                  "
        + reducer.expandPrecisionTime + " (Calls: " + reducer.expandPrecisionTime
        .getNumberOfIntervals() + ")");
    ...
    for (BAMBasedRefiner refiner : refiners) {
      ...
      out.println("  Compute path for refinement:                                  "
          + refiner.computePathTimer);
      ...
      out.println("  Removing cached subtrees:                                     "
          + refiner.removeCachedSubtreeTimer);
    }
  ...
  protected void exportAllReachedSets(
      final Path superArgFile, final PathTemplate indexedFile,
  ...
  private void writeArg(
    ...
    try (Writer w = Files.openOutputFile(file)) {
      ...
      logger.logUserException(Level.WARNING, e,
          String.format("Could not write ARG to file: %s", file));
    }
  }
