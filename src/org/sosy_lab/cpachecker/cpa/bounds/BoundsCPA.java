...
public class BoundsCPA extends AbstractCPA
  ...
  public BoundsCPA(Configuration config, CFA pCfa, LogManager pLogger)
  ...
  private BoundsCPA(
    ...
    if (!pCfa.getLoopStructure().isPresent()) {
      throw new CPAException(
          "UnifiedLoopstackCPA cannot work without loop-structure information in CFA.");
    }
    ...
    for (AbstractState state : pReached) {
      ...
      if (loopstackState != null) {
        maximumLoopIterationReached =
            Math.max(maximumLoopIterationReached, loopstackState.getDeepestIteration());
      }
    }
