...
public class BoundsState implements AbstractState, Partitionable, AvoidanceReportingState {
  ...
  public BoundsState(boolean pStopIt, boolean pStopRec) {
    this(pStopIt, pStopRec, PathCopyingPersistentTreeMap.<ComparableLoop, Integer>of(), 0, 1, null,
        0);
  }
  ...
  private BoundsState(
      boolean pStopIt,
      boolean pStopRec,
      PersistentSortedMap<ComparableLoop, Integer> pIterations,
      int pDeepestIteration,
      int pDeepestRecursion,
      String pCurrentFunction,
      int pReturnFromCounter) {
    Preconditions.checkArgument(pDeepestIteration >= 0);
    Preconditions.checkArgument(
        pDeepestIteration == 0 && pIterations.isEmpty() || pDeepestIteration > 0 && !pIterations
            .isEmpty());
  ...
  public BoundsState stopIt() {
    return new BoundsState(true, stopRec, iterations, deepestIteration, deepestRecursion,
        currentFunction, returnFromCounter);
  }
  ...
  public BoundsState setDeepestRecursion(int pDeepestRecursion) {
    ...
    return new BoundsState(stopIt, stopRec, iterations, deepestIteration, pDeepestRecursion,
        currentFunction, returnFromCounter);
  }
  ...
  public BoundsState returnFromFunction() {
    return new BoundsState(stopIt, stopRec, iterations, deepestIteration, deepestRecursion,
        currentFunction, returnFromCounter + 1);
  }
  ...
  public Set<Loop> getDeepestIterationLoops() {
    return FluentIterable.from(iterations.entrySet())
        .filter(new Predicate<Entry<ComparableLoop, Integer>>() {
  ...
  public int hashCode() {
    if (hashCache == 0) {
      hashCache = Objects
          .hash(deepestRecursion, returnFromCounter, stopIt, stopRec, currentFunction, iterations);
    }
    return hashCache;
  }
  ...
  public BooleanFormula getReasonFormula(FormulaManagerView manager) {
    ...
    if (stopIt) {
      reasonFormula = bfmgr.and(reasonFormula,
          PreventingHeuristic.LOOPITERATIONS.getFormula(manager, getDeepestIteration()));
    }
    if (stopRec) {
      reasonFormula = bfmgr.and(reasonFormula,
          PreventingHeuristic.RECURSIONDEPTH.getFormula(manager, getDeepestRecursion()));
    }
