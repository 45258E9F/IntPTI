...
public class AbstractARGBasedRefiner implements Refiner, StatisticsProvider {
  ...
  public final boolean performRefinement(ReachedSet pReached)
    ...
    if (!counterexample.isSpurious()) {
      ...
      assert targetPath.getFirstState() == path.getFirstState()
          : "Target path from refiner does not contain root node";
      assert targetPath.getLastState() == path.getLastState()
    ...
    logger
        .log(Level.FINEST, "ARG based refinement finished, result is", counterexample.isSpurious());
