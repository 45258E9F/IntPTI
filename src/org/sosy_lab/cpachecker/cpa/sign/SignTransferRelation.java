...
public class SignTransferRelation
  ...
  protected SignState handleReturnStatementEdge(CReturnStatementEdge pCfaEdge)
    ...
    CExpression expression =
        pCfaEdge.getExpression().or(CIntegerLiteralExpression.ZERO); // 0 is the default in C
  ...
  protected SignState handleFunctionCallEdge(
    ...
    for (int i = 0; i < pParameters.size(); i++) {
      ...
      String scopedVarId = getScopedVariableNameForNonGlobalVariable(pParameters.get(i).getName(),
          pCalledFunctionName);
      mapBuilder.put(scopedVarId,
          ((CRightHandSide) exp).accept(new SignCExpressionVisitor(edge, state, this)));
    }
    ImmutableMap<String, SIGN> argumentMap = mapBuilder.build();
    logger.log(Level.FINE,
        "Entering function " + pCalledFunctionName + " with arguments " + argumentMap);
    return state.enterFunction(argumentMap);
  }

  @Override
  protected SignState handleFunctionReturnEdge(
      FunctionReturnEdge pCfaEdge, FunctionSummaryEdge pFnkCall,
    ...
    if (pSummaryExpr instanceof AFunctionCallAssignmentStatement) {
      ...
      if (!(leftSide instanceof AIdExpression)) {
        throw new UnrecognizedCodeException("Unsupported code found",
            pCfaEdge);
      }
      ...
      logger.log(Level.FINE,
          "Leave function " + functionName + " with return assignment: " + assignedVarName + " = "
    ...
    if (pSummaryExpr instanceof AFunctionCallStatement) {
      ...
      return state.removeSignAssumptionOfVariable(
  ...
  private Optional<IdentifierValuePair> evaluateAssumption(
    ...
    BinaryOperator resultOp = !truthAssumption ? negateComparisonOperator(pAssumeExp.getOperator())
    ...
    return evaluateAssumption(strongestId, resultOp, resultSign,
        isLeftOperand(strongestId, pAssumeExp));
  }
      ...
      case GREATER_THAN:
        if (pIdentIsLeft) {
          ...
          if (SIGN.MINUS0.covers(pResultSign)) { // (0)- > x
            return Optional
                .of(new IdentifierValuePair(pIdExp, equalZero ? SIGN.MINUS0 : SIGN.MINUS));
          }
        }
        break;
      case LESS_EQUAL:
  ...
  private CExpression getRefinementExpression(
      CExpression pStrongestIdent,
      CBinaryExpression pBinExp) {
  ...
  private List<CExpression> filterIdentifier(CBinaryExpression pAssumeExp) {
    ...
    if ((pAssumeExp.getOperand1() instanceof CIdExpression) || (pAssumeExp
        .getOperand1() instanceof CFieldReference)) {
      result.add(pAssumeExp.getOperand1());
    }
    if ((pAssumeExp.getOperand2() instanceof CIdExpression) || (pAssumeExp
        .getOperand2() instanceof CFieldReference)) {
  ...
  protected SignState handleAssumption(
    ...
    if (result.isPresent()) {
      logger.log(Level.FINE,
          "Assumption: " + (pTruthAssumption ? pExpression : "!(" + pExpression + ")") + " --> "
              + result.get().identifier + " = " + result.get().value);
      ...
      if (!result.get().value.intersects(state.getSignForVariable(
          getScopedVariableName(result.get().identifier)))) {
        return null;
      }
    }
    return state;
  ...
  protected SignState handleDeclarationEdge(ADeclarationEdge pCfaEdge, ADeclaration pDecl)
    ...
    if (init instanceof AInitializerExpression) {
      return handleAssignmentToVariable(state, scopedId,
          ((AInitializerExpression) init).getExpression());
    }
    // type x;
  ...
  private SignState handleAssignment(AAssignment pAssignExpr)
    ...
    if (left instanceof AIdExpression) {// TODO also consider arrays, pointer, etc.?
      if (!((left.getExpressionType() instanceof CSimpleType) || (left
          .getExpressionType() instanceof CTypedefType))) {
        return state;
      }
    ...
    if (left instanceof CArraySubscriptExpression) {
      ...
      return state.assignSignToVariable(
          getScopedVariableName(((CArraySubscriptExpression) left).getArrayExpression(),
              functionName), SIGN.ALL);
    }
