...
public final class ApronCPA
    implements ConfigurableProgramAnalysis, ProofChecker, StatisticsProvider {
  ...
  private ApronCPA(
      Configuration config, LogManager log,
    ...
    this.transferRelation =
        new ApronTransferRelation(logger, cfa.getLoopStructure().get(), splitDisequalities);
    ...
    if (initialPrecisionFile != null || precisionType.equals("REFINEABLE_EMPTY")) {
      ...
      tempPrecision = VariableTrackingPrecision
          .createStaticPrecision(config, cfa.getVarClassification(), getClass());
    }
    ...
    try {
      Collection<? extends AbstractState> computedSuccessors =
          transferRelation.getAbstractSuccessorsForEdge(
              pState, otherStates, precision, pCfaEdge);
      boolean found;
