...
class OctagonArgBasedDelegatingRefiner implements ARGBasedRefiner, Statistics, StatisticsProvider {
  ...
  @Option(secure = true, description =
      "Timelimit for the backup feasibility check with the octagon analysis."
  ...
  private boolean performValueAnalysisRefinement(
      ...
      final ARGPath errorPath) throws CPAException, InterruptedException {
    numberOfValueAnalysisRefinements++;

    UnmodifiableReachedSet reachedSet = reached.asReachedSet();
    Precision precision = reachedSet.getPrecision(reachedSet.getLastState());
    VariableTrackingPrecision octPrecision =
        (VariableTrackingPrecision) Precisions.asIterable(precision)
  ...
  private boolean performOctagonAnalysisRefinement(
      final ARGReachedSet reached,
      final OctagonAnalysisFeasabilityChecker checker) {
    UnmodifiableReachedSet reachedSet = reached.asReachedSet();
    Precision precision = reachedSet.getPrecision(reachedSet.getLastState());
    VariableTrackingPrecision octPrecision =
        (VariableTrackingPrecision) Precisions.asIterable(precision)
   ...
   */
  private boolean isRepeatedRefinement(
      Multimap<CFANode, MemoryLocation> increment,
      Pair<ARGState, CFAEdge> refinementRoot) {
   ...
   *
   * Repeated counterexamples might occur when combining the analysis with thresholding, or when
  ...
  private boolean valueAnalysisRefinementWasSuccessful(
      ARGPath errorPath, VariableTrackingPrecision valueAnalysisPrecision,
  ...
  public void printStatistics(PrintStream out, Result result, ReachedSet reached) {
    out.println("  number of value analysis refinements:                "
        + numberOfValueAnalysisRefinements);
   ...
   */
  private OctagonAnalysisFeasabilityChecker createOctagonFeasibilityChecker(ARGPath path)
      throws CPAException, InterruptedException {
    try {
