...
public class OctagonState implements AbstractState {
  ...
  public OctagonState(
      Octagon oct,
      BiMap<MemoryLocation, Integer> map,
  ...
  protected int isLessOrEquals(OctagonState state) {
    ...
    if (variableToIndexMap.equals(state.variableToIndexMap)) {
      ...
      if (variableToIndexMap.entrySet().containsAll(state.variableToIndexMap.entrySet())) {
        ...
        return octagon.getManager()
            .isIncludedInLazy(checkStates.getFirst().octagon, checkStates.getSecond().octagon);
      } else {
        return 2;
      }
    }
   ...
   *
   * @param oct the octagon which has the preferred size, instead of just using an int, the OctState
   *            is the parameter, so we can check if the variables are matching if not an Exception
   *            is thrown
   */
  public Pair<OctagonState, OctagonState> shrinkToFittingSize(OctagonState oct) {
    ...
    if (variableToIndexMap.size() != maxEqualIndex + 1) {
      BiMap<MemoryLocation, Integer> newMap1 =
          HashBiMap.<MemoryLocation, Integer>create(variableToIndexMap);
      ...
      Octagon newOct1 =
          octagonManager.removeDimension(octagon, variableToIndexMap.size() - (maxEqualIndex + 1));
    ...
    if (oct.variableToIndexMap.size() != maxEqualIndex + 1) {
      BiMap<MemoryLocation, Integer> newMap2 =
          HashBiMap.<MemoryLocation, Integer>create(oct.variableToIndexMap);
      ...
      Octagon newOct2 = octagonManager
          .removeDimension(oct.octagon, oct.variableToIndexMap.size() - (maxEqualIndex + 1));
   ...
   */
  private OctagonState addConstraint(
      BinaryConstraints cons,
      int leftIndex,
      int rightIndex,
   ...
   */
  public OctagonState addSmallerEqConstraint(
      MemoryLocation pLeftVarName,
      MemoryLocation pRightVarName) {
    // use 0 as constant value, we don't need it
    return addSmallerEqConstraint0(pLeftVarName, pRightVarName, OctagonIntValue.ZERO);
  }

  private OctagonState addSmallerEqConstraint0(
      MemoryLocation pRightVariableName,
      MemoryLocation pLeftVariableName,
      OctagonNumericValue pConstant) {
   ...
   */
  public OctagonState addSmallerEqConstraint(
      MemoryLocation pVariableName,
      OctagonNumericValue pValueOfLiteral) {
    int varIdx = getVariableIndexFor(pVariableName);
  ...
  public OctagonState addSmallerEqConstraint(
    ...
    oct = oct.add(new OctagonIntervalCoefficients(oct.size(),
        new OctagonInterval(Double.NEGATIVE_INFINITY, 0), this));
   ...
   * Note that this only works with integers!
   */
  public OctagonState addSmallerConstraint(
      MemoryLocation pLeftVarName,
      MemoryLocation pRightVarName) {
  ...
  public OctagonState addSmallerConstraint(
    ...
    return addConstraint(BinaryConstraints.PX, varIdx, -1,
        pValueOfLiteral.subtract(OctagonIntValue.ONE));
  }
  ...
  public OctagonState addSmallerConstraint(MemoryLocation pVariableName, IOctagonCoefficients oct) {
    ...
    // the octagon library can only handle <= and >= constraints on floats
    if (variableToTypeMap.get(pVariableName) == Type.FLOAT) {
      return addSmallerEqConstraint(pVariableName,
          oct.sub(new OctagonSimpleCoefficients(oct.size(), ASSUMPTION_DELTA, this)));
    }
    ...
    oct = oct.add(new OctagonIntervalCoefficients(oct.size(),
        new OctagonInterval(Double.NEGATIVE_INFINITY, -1), this));
    OctagonState assignedState = makeAssignment(pVariableName, oct);
    return assignedState.intersect(this);
  }
   ...
   * Note that this only works with integers!
   */
  public OctagonState addGreaterEqConstraint(
      MemoryLocation pLeftVarName,
      MemoryLocation pRightVarName) {
    // use 0 as constant value, we don't need it
  ...
  private OctagonState addGreaterEqConstraint0(
      MemoryLocation pRightVariableName,
      MemoryLocation pLeftVariableName,
      OctagonNumericValue pConstant) {
    int rVarIdx = getVariableIndexFor(pRightVariableName);
  ...
  public OctagonState addGreaterEqConstraint(
    ...
    oct = oct.add(new OctagonIntervalCoefficients(oct.size(),
        new OctagonInterval(0, Double.POSITIVE_INFINITY), this));
    OctagonState assignedState = makeAssignment(pVariableName, oct);
    return assignedState.intersect(this);
  }
   ...
   * Note that this only works with integers!
   */
  public OctagonState addGreaterConstraint(
      MemoryLocation pLeftVarName,
      MemoryLocation pRightVarName) {
    int rVarIdx = getVariableIndexFor(pLeftVarName);
    int lVarIdx = getVariableIndexFor(pRightVarName);
  ...
  public OctagonState addGreaterConstraint(
    ...
    // set right index to -1 as it is not used
    return addConstraint(BinaryConstraints.MX, varIdx, -1,
        pValueOfLiteral.add(OctagonIntValue.ONE).mul(-1));
  }
  ...
  public OctagonState addGreaterConstraint(MemoryLocation pVariableName, IOctagonCoefficients oct) {
    ...
    // the octagon library can only handle <= and >= constraints on floats
    if (variableToTypeMap.get(pVariableName) == Type.FLOAT) {
      return addGreaterEqConstraint(pVariableName,
          oct.add(new OctagonSimpleCoefficients(oct.size(), ASSUMPTION_DELTA, this)));
    }
    ...
    oct = oct.add(new OctagonIntervalCoefficients(oct.size(),
        new OctagonInterval(1, Double.POSITIVE_INFINITY), this));
    OctagonState assignedState = makeAssignment(pVariableName, oct);
   ...
   * Note that this only works with integers!
   */
  public OctagonState addEqConstraint(
      MemoryLocation pVariableName,
      OctagonNumericValue constantValue) {
   ...
   */
  public Set<OctagonState> addIneqConstraint(
      MemoryLocation pLeftVarName,
      MemoryLocation pRightVarName) {
  ...
  private OctagonState removeVars(String varPrefix) {
    ...
    OctagonState newState =
        new OctagonState(octagonManager.removeDimension(octagon, keysToRemove.size()),
            HashBiMap.create(variableToIndexMap),
            new HashMap<>(variableToTypeMap),
            logger);
