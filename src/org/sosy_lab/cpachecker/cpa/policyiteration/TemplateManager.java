...
public class TemplateManager {
  ...
  PolicyPrecision precisionForNode(CFANode node) {
  ...
  public Formula toFormula(
    ...
    for (Entry<CIdExpression, Rational> entry : template.linearExpression) {
      ...
      if (Objects.equals(coeff, Rational.ZERO)) {
        continue;
      } else if (Objects.equals(coeff, Rational.NEG_ONE)) {
        multipliedItem = fmgr.makeNegate(item);
      } else if (!Objects.equals(coeff, Rational.ONE)) {
  ...
  private boolean shouldUseRationals(Template template) {
  ...
  void addGeneratedTemplates(Set<Template> templates) {
  ...
  boolean isOverflowing(Template template, Rational v) {
  ...
  private CSimpleType getTemplateType(Template t) {
    ...
    for (Entry<CIdExpression, Rational> e : t.getLinearExpression()) {
      ...
      if (sum == null) {
        ...
        sum = expressionBuilder.buildBinaryExpressionUnchecked(FileLocation.DUMMY,
  ...
  private Iterable<ASimpleDeclaration> getVarsForNode(CFANode node) {
    if (varFiltering == VarFilteringStrategy.ALL_LIVE) {
  ...
  private Formula normalizeLength(
      Formula f, int maxBitvectorSize,
      FormulaManagerView fmgr) {
  ...
  private int getBitvectorSize(
      Template t, PathFormulaManager pfmgr,
