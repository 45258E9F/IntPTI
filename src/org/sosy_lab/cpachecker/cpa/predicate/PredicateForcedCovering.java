  ...
  public boolean tryForcedCovering(
    ...
    for (final AbstractState coveringCandidate : pReached.getReached(pState)) {
      ...
      if (stop.isForcedCoveringPossible(pState, coveringCandidate, pPrecision)) {
        ...
        {
          formulas
              .add(getPredicateState(commonParent).getAbstractionFormula().asInstantiatedFormula());
          ...
          assert !bfmgr.isTrue(stateFormula)
              : "Existing state with abstraction true would cover anyway, no forced covering needed";
