...
public class ImpactGlobalRefiner implements Refiner, StatisticsProvider {
  ...
  private void printStatistics(PrintStream out) {
    if (refinementCalls > 0) {
      out.println("Avg. number of iterations per refinement:   " + div(refinementIterations,
          refinementCalls));
      out.println("Avg. number of target states per iteration: " + div(totalNumberOfTargetStates,
          refinementIterations));
      out.println(
          "Avg. number of refined paths per iteration: " + div(pathsRefined, refinementIterations));
      out.println(
          "Avg. number of sat checks per iteration:    " + div(satCheckTime.getNumberOfIntervals(),
              refinementIterations));
      out.println(
          "Avg. length of refined path (in blocks):    " + div(totalPathLengthToInfeasibility,
              pathsRefined));
      out.println("Avg. number of affected states per path:    " + div(totalNumberOfAffectedStates,
          pathsRefined));
      out.println();
   ...
   *
   * The strategy is to first build the predecessor/successor relations for all abstraction states
   * on the paths to the target states, and then call {@link #performRefinementOnPath(List,
  ...
  private boolean performRefinement0(ReachedSet pReached, List<AbstractState> targets)
    ...
    try (InterpolatingProverEnvironment<?> itpProver = solver
  ...
  private <T> boolean performRefinement0(
      ARGState current, SetMultimap<ARGState, ARGState> successors,
   ...
   *
   * Note that the successor and predecessor relation contains only states that belong to paths to a
   * target state, so we refine only such paths, and not all paths in the ARG.
   *
  ...
  private <T> boolean step(
      ARGState current, List<T> itpStack, SetMultimap<ARGState, ARGState> successors,
      Map<ARGState, ARGState> predecessors, ReachedSet pReached, List<AbstractState> targets,
      InterpolatingProverEnvironment<T> itpProver)
      throws InterruptedException, SolverException, CPAException {
  ...
  private <T> void performRefinementOnPath(
      List<T> itpStack, final ARGState unreachableState,
   ...
   */
  private boolean performRefinementForState(
   ...
   *                         top of ARG to target state).
   * @param reached          The reached set.
   */
  private void finishRefinementOfPath(
      final ARGState unreachableState, List<ARGState> affectedStates,
