...
public class PredicateStaticRefiner extends StaticRefiner
  ...
  private boolean isAssumeOnLoopVariable(AssumeEdge e) {
    ...
    Collection<String> referenced =
        CIdExpressionCollectorVisitor.getVariablesOfExpression((CExpression) e.getExpression());
  ...
  private Collection<AbstractionPredicate> assumeEdgeToPredicates(
      ...
      AssumeEdge assume) throws CPATransferException, InterruptedException {
  ...
  private void dumpAssumePredicate(Path target) {
    try (Writer w = Files.openOutputFile(target)) {
      ...
      logger.logUserException(Level.WARNING, e,
          "Interrupted, could not write assume predicates to file!");
      Thread.currentThread().interrupt();
    } catch (IOException e) {
      logger.logUserException(Level.WARNING, e,
          "IO exception! Could not write assume predicates to file!");
    } catch (CPATransferException e) {
      logger.logUserException(Level.WARNING, e,
          "Transfer exception! Could not write assume predicates to file!");
    }
  }
