...
public class BAMPredicateReducer implements Reducer {
  ...
  public AbstractState getVariableReducedState(
    ...
    PersistentMap<CFANode, Integer> abstractionLocations =
        predicateElement.getAbstractionLocationsOnPath()
            .empty();
  ...
  public Object getHashCodeForState(AbstractState pElementKey, Precision pPrecisionKey) {
    ...
    */
    ...
    return Triple.of(element.getAbstractionFormula().asRegion(), precision,
        cpa.getRelevantPredicatesComputer());
  }
  ...
  public Precision getVariableExpandedPrecision(
    ...
    if (rootPrecision == reducedPrecision) {
  ...
  public Precision getVariableReducedPrecision(Precision pPrecision, Block context) {
    ...
    final ImmutableSet<AbstractionPredicate> globalPredicates =
        ImmutableSet.copyOf(cpa.getRelevantPredicatesComputer().getRelevantPredicates(
            context, expandedPredicatePrecision.getGlobalPredicates()));

    // we only need function predicates with used variables
    final ImmutableSetMultimap.Builder<String, AbstractionPredicate> functionPredicatesBuilder =
    ...
    for (String functionname : expandedPredicatePrecision.getFunctionPredicates().keySet()) {
      ...
      functionPredicatesBuilder
          .putAll(functionname, cpa.getRelevantPredicatesComputer().getRelevantPredicates(
              context, expandedPredicatePrecision.getFunctionPredicates().get(functionname)));
    }
    final ImmutableSetMultimap<String, AbstractionPredicate> functionPredicates =
        functionPredicatesBuilder.build();

    // we only need local predicates with used variables and with nodes from the block
    final ImmutableSetMultimap.Builder<CFANode, AbstractionPredicate> localPredicatesBuilder =
        ImmutableSetMultimap.builder();
    for (CFANode node : expandedPredicatePrecision.getLocalPredicates().keySet()) {
      if (context.getNodes().contains(node)) {
        ...
        localPredicatesBuilder
            .putAll(node, cpa.getRelevantPredicatesComputer().getRelevantPredicates(
                context, expandedPredicatePrecision.getPredicates(node, 0)));
      }
    }
    final ImmutableSetMultimap<CFANode, AbstractionPredicate> localPredicates =
        localPredicatesBuilder.build();
  ...
  private static class ReducedPredicatePrecision extends PredicatePrecision {
    ...
    public boolean equals(Object pObj) {
      return super.equals(pObj)
          ...
          && rootPredicatePrecision
          .equals(((ReducedPredicatePrecision) pObj).getRootPredicatePrecision());
    }
   ...
   * lets build a new SSA from: - local variables from rootSSA,                  -> update indices
   * (their indices will have "holes") - local variables from expandedSSA,              -> ignore
   * indices (their indices are the "holes") - global variables from expandedSSA,             ->
   * update indices (we have to keep them) - the local return variables from expandedState. ->
   * update indices (we have to keep them, there can be several ret-vars from distinct functions,
   * ignore them, they are created new, if needed) we copy expandedState and override all local
   * values.
   *
  ...
  protected static SSAMap updateIndices(
      final SSAMap rootSSA, final SSAMap expandedSSA,
    ...
    for (String var : expandedSSA.allVariables()) {
      ...
      if (expandedSSA.containsVariable(var)) { // var was used and maybe overridden inside the block
        final CType type = expandedSSA.getType(var);
        if (var.contains("::") && !isReturnVar(var,
            functionExitNode)) { // var is scoped -> not global

          if (!rootSSA.containsVariable(
              var)) { // inner local variable, never seen before, use fresh index as basis for further assignments
  ...
  private static void setFreshValueBasis(SSAMapBuilder ssa, String name, int idx) {
    Preconditions
        .checkArgument(idx > 0, "Indices need to be positive for this SSAMap implementation:", name,
            idx);
    int oldIdx = ssa.getIndex(name);
    Preconditions
        .checkArgument(idx >= oldIdx, "SSAMap updates need to be strictly monotone:", name, idx,
            "vs", oldIdx);
