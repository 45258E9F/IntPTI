...
public abstract class RefinementStrategy {
  ...
  public void performRefinement(
      ARGReachedSet pReached, List<ARGState> abstractionStatesTrace,
  ...
  private Pair<ARGState, List<ARGState>> evaluateInterpolantsOnPath(
    ...
    // Traverse the path
    for (Pair<BooleanFormula, ARGState> interpolationPoint : Pair
        .zipList(pInterpolants, abstractionStatesTrace)) {
   ...
   *
   * @param unreachableState       The first state in the path which is infeasible (this identifies
   *                               the path).
   * @param affectedStates         The list of states that were affected by the refinement (ordered
   *                               from root to target state).
