...
public class LDDAbstractionTransferRelation extends SingleEdgeTransferRelation {
  ...
  public LDDAbstractionTransferRelation(
      LDDRegionManager regionManager,
  ...
  public Collection<? extends LDDAbstractState> getAbstractSuccessorsForEdge(
      AbstractState element, List<AbstractState> otherStates, Precision precision, CFAEdge edge)
  ...
  /**
   * Tries to create an LDD as the successor state of the given state over the given control flow
   * edge.
   *
   ...
   * @return the LDD representing the condition or <code>null</code> if the expression is not
   * supported.
   */
    ...
    if (pExpression instanceof CBinaryExpression) {
      ...
      switch (operator) {
        ...
        case GREATER_THAN:
          ...
          right = left;
          left = temp;
          operator = operator == BinaryOperator.GREATER_EQUAL ? BinaryOperator.LESS_EQUAL
        ...
        case EQUALS:
          ...
          return this.regionManager
              .makeAnd(makeNode(leftTerm, rightTerm, true), makeNode(rightTerm, leftTerm, true));
        case NOT_EQUALS:
          ...
          return this.regionManager.makeNot(this.regionManager
              .makeAnd(makeNode(leftTerm, rightTerm, true), makeNode(rightTerm, leftTerm, true)));
        default:
          return null;
      }
  ...
  private LDDRegion makeNode(
      Map<String, Integer> leftTerm,
      Map<String, Integer> rightTerm,
      boolean leq) {
  ...
  /**
   * Tries to create an LDD for an assignment of the given expression to the variable with the given
   * name. If the expression is not supported, the previous LDD is returned. If the expression is
   * supported, the conjunction of the previous LDD with the LDD obtained from the assignment is
   * returned.
   *
  ...
  private LDDRegion assign(String variable, CExpression expression, LDDRegion previousRegion) {
    ...
    if (reduced != null) {
      ...
      return this.regionManager
          .makeAnd(previousRegion, toConstantAssignmentRegion(Collections.singleton(variable),
              reduced));
    }
    ...
    if (term != null) {
      ...
      if (this.usedVars.contains(variable)) {
        ...
        region =
            this.regionManager.makeAnd(previousRegion, toConstantAssignmentRegion(term, constant));
      }
      return region;
    }
    return previousRegion;
   ...
   */
  private LDDRegion substituteByTerm(
      String pVariable,
      Map<String, Integer> pTerm,
      int constant,
      LDDRegion region) {
    return this.regionManager
        .replace(this.variables.get(pVariable), toIndexCoefficients(pTerm), this.variables.size(),
            constant, region);
  }
  ...
  /**
   * Removes the integer coefficient for the given variable from the given integer linear term and
   * returns it. If the coefficient does not exist, it is treated as zero.
   *
  ...
  /**
   * Removes the rational coefficient for the given variable from the given rational linear term and
   * returns it. If the coefficient does not exist, 0/1 is returned, so the coefficient is treated
   * as zero.
   *
   ...
   */
  private Pair<Integer, Integer> removeRationalCoefficient(
      Map<String, Pair<Integer, Integer>> coefficients,
      String variable) {
   ...
   * @return the integer linear term the expression was converted to or <code>null</code> if the
   * conversion failed.
   */
  ...
  private Map<String, Pair<Integer, Integer>> reduceToRationalTerm(CExpression expression) {
    ...
    if (expression instanceof CBinaryExpression) {
      ...
      if (firstAsTerm != null && secondAsTerm != null && operator == BinaryOperator.DIVIDE) {
        return null;
      }
      ...
      switch (operator) {
        ...
        case PLUS:
          variableCoeffs = term;
          Pair<Integer, Integer> previousConstVar =
              removeRationalCoefficient(variableCoeffs, "const");
   ...
   * @return the integer constant represented by the expression or <code>null</code> if the
   * expression did not represent an integer constant.
   */
   ...
   * @param leq      the flag indicating whether to use a "less or equal" or a "less than"
   *                 operator.
  ...
  private LDDRegion toNode(Map<String, Integer> coeffs, boolean leq, int constant) {
    return this.regionManager
        .makeNode(toIndexCoefficients(coeffs), this.variables.size(), leq, constant);
  }
   ...
   * constant-
   *
   * @param variables the integer linear term containing all the variable coefficients of the
  ...
  private LDDRegion toConstantAssignmentRegion(Collection<String> variables, int constant) {
    return this.regionManager
        .makeConstantAssignment(toIndexCoefficients(variables), this.variables.size(),
            constant);
  }
   ...
   * constant-
   *
   * @param coeffs   the integer linear term containing all the variable coefficients of the
   *                 assignment expression.
   * @param constant the constant part of the assignment expression.
   * @return the LDDRegion created for the assignment.
   */
  private LDDRegion toConstantAssignmentRegion(Map<String, Integer> coeffs, int constant) {
    return this.regionManager
        .makeConstantAssignment(toIndexCoefficients(coeffs), this.variables.size(),
            constant);
  }
   ...
   * @return a collection of pairs of variable indices and their coefficients, all of which are
   * treated as 1.
   */
  ...
  @Override
  public Collection<? extends LDDAbstractState> strengthen(
