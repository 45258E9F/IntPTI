...
public class ExpressionValueVisitor extends AbstractExpressionValueVisitor {
  ...
  final ValueAnalysisState readableState;
  ...
  public ExpressionValueVisitor(
      ValueAnalysisState pState, String pFunctionName,
  ...
  public Value evaluateCFunctionCallExpression(
      final CFunctionCallExpression callExpression,
      final CType pTargetType)
      throws UnrecognizedCCodeException {
    CExpression functionName = callExpression.getFunctionNameExpression();
    if (functionName instanceof CIdExpression) {
      String name = ((CIdExpression) functionName).getName();
      if (BuiltinFunctions.isBuiltinFunction(name)) {
        return castCValue(callExpression.accept(this), pTargetType, getMachineModel(), getLogger(),
            callExpression.getFileLocation());
      } else {
        // use function adapter here
        ...
        Value value = ValueFunctionAdapter.instance
            (getFunctionName(), getLogger()).evaluateFunctionCallExpression(callExpression,
            argumentValues, readableState, null).getResult();
        return castCValue(value, pTargetType, getMachineModel(), getLogger(), callExpression
            .getFileLocation());
      }
    }
  ...
  public MemoryLocation evaluateMemLocForArraySlot(
    ...
    return locationEvaluator
        .getArraySlotLocationFromArrayStart(pArrayStartLocation, pSlotNumber, pArrayType);
  }
