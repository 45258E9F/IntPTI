...
public class ValueAnalysisPrecisionAdjustment implements PrecisionAdjustment, StatisticsProvider {
  ...
  public ValueAnalysisPrecisionAdjustment(
      ...
      final ValueAnalysisTransferRelation pTransfer,
      final CFA pCfa)
  ...
  public Optional<PrecisionAdjustmentResult> prec(
      AbstractState pState, Precision pPrecision, UnmodifiableReachedSet pStates,
  ...
  private boolean performPrecisionBasedAbstraction() {
    ...
    if (performPrecisionBasedAbstraction == null) {
      performPrecisionBasedAbstraction =
          (transfer.getCurrentLevelOfDeterminism() < determinismThreshold)
          ? true
          : false;
    }
  ...
  private void enforcePrecision(
    ...
    if (abstractAtEachLocation()
      ...
      for (MemoryLocation memoryLocation : state.getTrackedMemoryLocations()) {
        if (location != null && !precision
            .isTracking(memoryLocation, state.getTypeForMemoryLocation(memoryLocation),
