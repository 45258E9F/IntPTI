...
public abstract class AbstractExpressionValueVisitor
   ...
   */
  public AbstractExpressionValueVisitor(
...
//      logger.logf(Level.FINE, "Parameters to binary operation '%s %s %s' are no numeric values.", lVal, binaryOperator, rVal);
    ...
    switch (binaryOperator) {
      ...
      case BINARY_OR:
      case BINARY_XOR: {
        result = arithmeticOperation((NumericValue) lVal, (NumericValue) rVal, binaryOperator,
            calculationType, machineModel, logger);
  ...
  private static SymbolicValue createSymbolicExpression(
    ...
    switch (pOperator) {
      ...
      case SHIFT_RIGHT:
        return factory
            .shiftRightSigned(leftOperand, rightOperand, pExpressionType, pCalculationType);
      case BINARY_AND:
      ...
      case LESS_EQUAL:
        return factory
            .lessThanOrEqual(leftOperand, rightOperand, pExpressionType, pCalculationType);
      case GREATER_THAN:
        return factory.greaterThan(leftOperand, rightOperand, pExpressionType, pCalculationType);
      case GREATER_EQUAL:
        return factory
            .greaterThanOrEqual(leftOperand, rightOperand, pExpressionType, pCalculationType);
      default:
...
//        logger.logf(Level.SEVERE, "Division by Zero (%d / %d)", l, r);
      ...
      case MODULO:
        if (r == 0) {
//        logger.logf(Level.SEVERE, "Division by Zero (%d mod %d)", l, r);
          return 0;
        }
        return l % r;
      case MULTIPLY:
   ...
   * @return the resulting value
   */
  private static float arithmeticOperation(
  ...
  private static Value arithmeticOperation(
      final NumericValue lNum, final NumericValue rNum,
      final BinaryOperator op, final CType calculationType,
      final MachineModel machineModel, final LogManager logger) {
...
//        logger.logf(Level.FINE, "unsupported type for result of binary operation %s", type.toString());
  ...
  private static Value booleanOperation(
      final NumericValue l, final NumericValue r,
      final BinaryOperator op, final CType calculationType,
      final MachineModel machineModel, final LogManager logger) {
...
//        logger.logf(Level.FINE, "unsupported type %s for result of binary operation %s", type.toString(), op);
        ...
        if (BuiltinFloatFunctions.matchesAbsolute(functionName)) {
          ...
          if (parameterValues.size() == 1) {
            ...
            if (value.isExplicitlyKnown()) {
              ...
              switch (paramType.getType()) {
                case FLOAT:
                  return Float.isNaN(numericValue.floatValue()) ? new NumericValue(1)
                                                                : new NumericValue(0);
                case DOUBLE:
                  return Double.isNaN(numericValue.doubleValue()) ? new NumericValue(1)
                                                                  : new NumericValue(0);
                default:
                  break;
              }
          ...
          if (parameterValues.size() == 1) {
            ...
            if (value.isExplicitlyKnown()) {
              ...
              switch (paramType.getType()) {
                case FLOAT:
                  return Float.isInfinite(numericValue.floatValue()) ? new NumericValue(1)
                                                                     : new NumericValue(0);
                case DOUBLE:
                  return Double.isInfinite(numericValue.doubleValue()) ? new NumericValue(1)
                                                                       : new NumericValue(0);
                default:
                  break;
              }
          ...
          if (parameterValues.size() == 1) {
            ...
            if (value.isExplicitlyKnown()) {
              ...
              switch (paramType.getType()) {
                case FLOAT:
                  return Float.isInfinite(numericValue.floatValue()) ? new NumericValue(0)
                                                                     : new NumericValue(1);
                case DOUBLE:
                  return Double.isInfinite(numericValue.doubleValue()) ? new NumericValue(0)
                                                                       : new NumericValue(1);
                default:
                  break;
              }
  ...
  public static Value createSymbolicExpression(
      Value pValue, CType pOperandType, UnaryOperator
      pUnaryOperator, CType pExpressionType) {
    final SymbolicValueFactory factory = SymbolicValueFactory.getInstance();
  ...
  private Value createSymbolicExpression(
    ...
    switch (pOperator) {
      ...
      case SHIFT_RIGHT_UNSIGNED:
        return factory
            .shiftRightUnsigned(leftOperand, rightOperand, pExpressionType, pCalculationType);
      case BINARY_AND:
        return factory.binaryAnd(leftOperand, rightOperand, pExpressionType, pCalculationType);
      case LOGICAL_AND:
  ...
  private Value calculateIntegerOperation(
      NumericValue pLeftValue, NumericValue pRightValue,
  ...
  private Value calculateComparison(
      Value pLeftValue, Value pRightValue,
      JBinaryExpression.BinaryOperator pOperator) {
  ...
  private Value createSymbolicExpression(
      Value pValue, JType pOperandType,
  ...
  public Value visit(JArraySubscriptExpression pJArraySubscriptExpression) {
    NumericValue subscriptValue =
        (NumericValue) pJArraySubscriptExpression.getSubscriptExpression().accept(this);
  ...
  public static Value castCValue(
      @Nonnull final Value value, final CType targetType,
      final MachineModel machineModel, final LogManagerWithoutDuplicates logger,
    ...
    if (!value.isNumericValue()) {
      // logger.logf(Level.FINE, "Can not cast C value %s to %s", value.toString(), targetType.
      //     toString());
      return value;
    }
  ...
  public static Value castJValue(
      @Nonnull final Value value, JType sourceType,
   ...
   * @return A numeric type that can be used to perform arithmetics on an instance of the type
   * directly, or null if none.
   *
   * Most notably, CPointerType will be converted to the unsigned integer type of correct size.
   */
