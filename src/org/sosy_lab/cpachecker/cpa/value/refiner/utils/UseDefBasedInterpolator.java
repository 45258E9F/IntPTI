...
public class UseDefBasedInterpolator {
  ...
  private ValueAnalysisInterpolant createInterpolant(Collection<ASimpleDeclaration> uses) {
    ...
    return new ValueAnalysisInterpolant(useDefInterpolant,
        Collections.<MemoryLocation, Type>emptyMap());
  }
   ...
   *
   * This class has one mutable field {@link MemoryLocationCreator#currentOffset}, so throw away
   * after use.
   */
  private static class MemoryLocationCreator
      implements CTypeVisitor<List<MemoryLocation>, IllegalArgumentException> {
    ...
    @Override
    public List<MemoryLocation> visit(final CCompositeType pCompositeType)
    ...
    @Override
    public List<MemoryLocation> visit(final CElaboratedType pElaboratedType)
        throws IllegalArgumentException {
      withinComplexType = true;
    ...
    @Override
    public List<MemoryLocation> visit(final CFunctionType pFunctionType)
        throws IllegalArgumentException {
    ...
    @Override
    public List<MemoryLocation> visit(final CPointerType pPointerType)
        throws IllegalArgumentException {
      return createSingleMemoryLocation(model.getSizeofPtr());
    }

    @Override
    public List<MemoryLocation> visit(final CProblemType pProblemType)
        throws IllegalArgumentException {
      throw new IllegalArgumentException("Unknown C-Type: " + pProblemType.getClass().toString());
    }

    @Override
    public List<MemoryLocation> visit(final CSimpleType pSimpleType)
        throws IllegalArgumentException {
      return createSingleMemoryLocation(model.getSizeof(pSimpleType));
    }

    @Override
    public List<MemoryLocation> visit(final CTypedefType pTypedefType)
        throws IllegalArgumentException {
    ...
    private List<MemoryLocation> createSingleMemoryLocation(final int pSize) {
      if (withinComplexType) {
        List<MemoryLocation> memory =
            Collections.singletonList(MemoryLocation.valueOf(qualifiedName, currentOffset));
    ...
    private List<MemoryLocation> createMemoryLocationsForArray(
        final int pLength,
        final CType pType) {
