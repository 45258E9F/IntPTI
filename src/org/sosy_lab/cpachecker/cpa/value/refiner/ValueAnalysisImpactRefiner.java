...
public class ValueAnalysisImpactRefiner
  ...
  protected void refineUsingInterpolants(
    ...
    for (Map.Entry<ARGState, ValueAnalysisInterpolant> itp : pInterpolationTree
      ...
      if (strengthenedStates.contains(currentState)) {
        ...
        for (MemoryLocation memoryLocation : pInterpolationTree.getInterpolantForState(currentState)
            .getMemoryLocations()) {
  ...
  protected void printAdditionalStatistics(PrintStream pOut, Result pResult, ReachedSet pReached) {
    pOut.println(
        "Total number of restarts:      " + String.format(Locale.US, "%9d", restartCounter));
