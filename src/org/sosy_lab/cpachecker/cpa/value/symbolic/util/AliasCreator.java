...
/**
 * Classes implementing this interface allow the creation of aliases for two sets of symbolic
 * values, so that the first set of symbolic values equals the second set of symbolic values, if
 * each symbolic identifier is replaced with its alias.
 */
    ...
    Environment join(final Environment pOther) {
      ...
      for (Map.Entry<SymbolicIdentifier, SymbolicIdentifier> entry : pOther.aliases.entrySet()) {
        ...
        if (newEnv.aliases.containsKey(key)
            && !SymbolicValues
            .representSameSymbolicMeaning(newEnv.aliases.get(key), entry.getValue())) {
          return null;
        }
      ...
      for (Map.Entry<SymbolicValue, SymbolicValue> entry : pOther.counterparts.entrySet()) {
        ...
        if (newEnv.counterparts.containsKey(key)
            && !SymbolicValues
            .representSameSymbolicMeaning(newEnv.counterparts.get(key), entry.getValue())) {
          return null;
        }
