...
public class BDDTransferRelation
  ...
  public BDDTransferRelation(
      NamedRegionManager manager, BitvectorManager bvmgr,
  ...
  private BDDState handleAssignment(CAssignment assignment, CFANode successor) {
    ...
    if (rhs instanceof CExpression) {
      ...
      if (isUsedInExpression(varName, exp)) {
        ...
        final Region[] tmp = predmgr
            .createPredicateWithoutPrecisionCheck(tmpVarName, getBitsize(partition, targetType));
        ...
        final Region[] var = predmgr.createPredicate(scopeVar(lhs), targetType, successor,
            getBitsize(partition, targetType), precision);
      ...
      final Region[] var = predmgr.createPredicate(scopeVar(lhs), targetType, successor, bitsize,
  ...
  private BDDState handleExternalFunctionCall(
    ...
    for (final CExpression param : params) {
      ...
      if (param instanceof CUnaryExpression &&
        ...
        final Region[] var = predmgr
            .createPredicate(scopeVar(id), id.getExpressionType(), successor, bitsize,
                precision); // is default bitsize enough?
  ...
  protected BDDState handleDeclarationEdge(CDeclarationEdge cfaEdge, CDeclaration decl) {
    ...
    if (decl instanceof CVariableDeclaration) {
      ...
      Region[] var = predmgr
          .createPredicate(vdecl.getQualifiedName(), vdecl.getType(), cfaEdge.getSuccessor(),
              getBitsize(partition, vdecl.getType()), precision);
      BDDState newState = state.forget(var);

      // initializer on RIGHT SIDE available, make region for it
      if (init != null) {
        final Region[] rhs =
            evaluateVectorExpression(partition, init, vdecl.getType(), cfaEdge.getSuccessor());
  ...
  protected BDDState handleFunctionCallEdge(
    ...
    for (int i = 0; i < params.size(); i++) {
      ...
      final Region[] var = predmgr.createPredicate(varName, targetType, cfaEdge.getSuccessor(),
          getBitsize(partition, targetType), precision);
      final Region[] arg =
          evaluateVectorExpression(partition, args.get(i), targetType, cfaEdge.getSuccessor());
  ...
  protected BDDState handleFunctionReturnEdge(
    ...
    if (summaryExpr instanceof CFunctionCallAssignmentStatement) {
      ...
      final Region[] var = predmgr
          .createPredicate(scopeVar(lhs), lhs.getExpressionType(), cfaEdge.getSuccessor(), size,
              precision);
      newState = newState.forget(var);

      // make region (predicate) for RIGHT SIDE
      final Region[] retVar = predmgr
          .createPredicate(returnVar, summaryExpr.getFunctionCallExpression().getExpressionType(),
              cfaEdge.getSuccessor(), size, precision);
  ...
  protected BDDState handleReturnStatementEdge(CReturnStatementEdge cfaEdge) {
    ...
    if (cfaEdge.getExpression().isPresent()) {
      returnVar = ((CIdExpression) cfaEdge.asAssignment().get().getLeftHandSide()).getDeclaration()
          .getQualifiedName();
      final Partition partition = varClass.getPartitionForEdge(cfaEdge);
      ...
      final Region[] regRHS =
          evaluateVectorExpression(partition, cfaEdge.getExpression().get(), functionReturnType,
              cfaEdge.getSuccessor());

      // make variable (predicate) for returnStatement,
      // delete variable, if it was used before, this is done with an existential operator
      final Region[] retvar =
          predmgr.createPredicate(returnVar, functionReturnType, cfaEdge.getSuccessor(),
    ...
    for (String var : predmgr.getTrackedVars().keySet()) {
      if (isLocalVariableForFunction(var, functionName) && !returnVar.equals(var)) {
        newState = newState.forget(
            predmgr.createPredicateWithoutPrecisionCheck(var, predmgr.getTrackedVars().get(var)));
      }
    }
  ...
  protected BDDState handleAssumption(
    ...
    final Region[] operand =
        evaluateVectorExpression(partition, expression, CNumericTypes.INT, cfaEdge.getSuccessor());
    if (operand == null) {
      return state;
  ...
  Region[] evaluateVectorExpression(
    ...
    if (varClass.getIntBoolPartitions().contains(partition)) {
      ...
      return exp.accept(
          new BDDCompressExpressionVisitor(predmgr, precision, getBitsize(partition, null),
              location, bvmgr, partition));
    } else {
      Region[] value = exp.accept(
    ...
    if (dumpfile != null) { // option -noout
      try (Writer w = Files.openOutputFile(dumpfile)) {
        ...
        logger.logUserException(Level.WARNING, e,
            "Could not write tracked variables for BDDCPA to file");
      }
    }

    out.println(String.format("Number of boolean vars:           %d (of %d)", trackedIntBool.size(),
        numOfBooleans));
    out.println(String.format("Number of intEqual vars:          %d (of %d)", trackedIntEq.size(),
        numOfIntEquals));
    out.println(String.format("Number of intAdd vars:            %d (of %d)", trackedIntAdd.size(),
        numOfIntAdds));
