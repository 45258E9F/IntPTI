...
class AutomatonTransferRelation extends SingleEdgeTransferRelation {
   ...
   *
   * If the state is a NonDet-State multiple following states may be returned. If the only following
   * state is BOTTOM an empty set is returned.
   */
  private Collection<AutomatonState> getFollowStates(
      AutomatonState state,
      List<AbstractState> otherElements,
      CFAEdge edge,
      boolean failOnUnknownMatch) throws CPATransferException {
        ...
        if (match.getValue()) {
          ...
          if (assertionsHold.canNotEvaluate()) {
            ...
            AutomatonSafetyProperty prop =
                new AutomatonSafetyProperty(state.getOwningAutomaton(), t);
    ...
    if (edgeMatched) {
      ...
      AutomatonState stateNewCounters = AutomatonState
          .automatonStateFactory(state.getVars(), state.getInternalState(), cpa, state.getMatches(),
              state.getFailedMatches() + failedMatches, null);
  ...
  public Collection<? extends AbstractState> strengthen(
    ...
    if (!(pElement instanceof AutomatonUnknownState)) {
          ...
          for (AbstractState otherState : otherStates) {
            ...
            if (otherState instanceof AutomatonUnknownState) {
              ...
              Collection<? extends AbstractState> successors =
                  getFollowStates(unknownState.getPreviousState(), statesOtherToCurrent, pCfaEdge,
                      true);
      ...
      for (List<AbstractState> otherStates : strengtheningCombinations) {
        successors
            .addAll(getFollowStates(lUnknownState.getPreviousState(), otherStates, pCfaEdge, true));
      }
