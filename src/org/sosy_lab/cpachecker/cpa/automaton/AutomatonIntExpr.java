...
/**
 * Implements a integer expression that evaluates and returns a <code>int</code> value when
    ...
    public ResultValue<Integer> eval(AutomatonExpressionArguments pArgs) {
      if (TRANSITION_VARS_PATTERN.matcher(varId).matches()) { // $1  AutomatonTransitionVariables
        ...
        if (val == null) {
          ...
          return new ResultValue<>("could not find the transition variable $" + key + ".",
        ...
        if (variable != null) {
          ...
          return new ResultValue<>("could not find the automaton variable " + varId + ".",
              "AutomatonIntExpr.VarAccess");
        }
      }
    }
