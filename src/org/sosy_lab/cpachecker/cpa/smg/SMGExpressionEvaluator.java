...
public class SMGExpressionEvaluator {
   ...
   *
   * @param edge       The cfa edge which determines the location of the program.
   ...
   */
  public int getSizeof(CFAEdge edge, CType pType, SMGState pState, CExpression expression)
   ...
   * @return The size of the given type in bytes.
   */
  public int getSizeof(CFAEdge edge, CType pType, SMGState pState)
      throws UnrecognizedCCodeException {
  ...
  public class LValueAssignmentVisitor extends AddressVisitor {
    ...
    public List<SMGAddressAndState> visit(CUnaryExpression lValue) throws CPATransferException {
      ...
      throw new UnrecognizedCCodeException(lValue.toASTString() + " is not an lValue", getCfaEdge(),
          lValue);
    }
  ...
  private List<SMGAddressAndState> getAddressOfField(
      SMGState pSmgState, CFAEdge cfaEdge,
    ...
    */
    ...
    SMGAddressValueAndStateList fieldOwnerAddressAndStates =
        evaluateAddress(pSmgState, cfaEdge, fieldOwner);

    for (SMGAddressValueAndState fieldOwnerAddressAndState : fieldOwnerAddressAndStates
  ...
  public SMGValueAndState readValue(
    ...
    SMGSymbolicValue value = pSmgState.readValue(pObject, fieldOffset, pType, exp).getObject();

    return SMGValueAndState.of(pSmgState, value);
  }

  private SMGField getField(
      CFAEdge edge, CType ownerType, String fieldName, SMGState pState,
      CExpression exp) throws UnrecognizedCCodeException {
  ...
  private SMGField getField(
      CFAEdge pEdge, CCompositeType ownerType, String fieldName,
      SMGState pState, CExpression expression) throws UnrecognizedCCodeException {
  ...
  protected List<SMGExplicitValueAndState> evaluateExplicitValue(
    ...
    ExplicitValueVisitor visitor =
        new ExplicitValueVisitor(smgState, null, machineModel, logger, cfaEdge);
  ...
  private SMGExplicitValueAndState deriveExplicitValueFromSymbolicValue(
    ...
    if (!symbolicValue.isUnknown()) {
      ...
      if (symbolicValue instanceof SMGAddressValue) {
        ...
        if (address.getObject() == SMGObject
            .getNullObject()) {
          return SMGExplicitValueAndState.of(newState,
  ...
  public SMGSymbolicValue evaluateExpressionValueV2(
      SMGState smgState,
      CFAEdge cfaEdge, CRightHandSide rValue) throws CPATransferException {
  ...
  protected SMGValueAndStateList evaluateNonAddressValue(
  ...
  protected SMGValueAndStateList evaluateAssumptionValue(
      SMGState newState,
  ...
  private abstract class AddressVisitor
    ...
    @Override
    public List<SMGAddressAndState> visit(CArraySubscriptExpression exp)
        throws CPATransferException {
  ...
  class PointerVisitor extends ExpressionValueVisitor {
    ...
    // is it possible that an integer literal has pointer type??
    @Override
    public SMGAddressValueAndStateList visit(CIntegerLiteralExpression exp)
        throws CPATransferException {
      return getAddressFromSymbolicValues(super.visit(exp));
    }

    @Override
    public SMGAddressValueAndStateList visit(CCharLiteralExpression exp)
        throws CPATransferException {
      return getAddressFromSymbolicValues(super.visit(exp));
    }

    @Override
    public SMGAddressValueAndStateList visit(CFloatLiteralExpression pExp)
        throws CPATransferException {
    ...
    @Override
    public SMGAddressValueAndStateList visit(CUnaryExpression unaryExpression)
        throws CPATransferException {
    ...
    private SMGAddressValueAndStateList handleAmper(CRightHandSide amperOperand)
        throws CPATransferException {

      if (getRealExpressionType(amperOperand) instanceof CFunctionType
          && amperOperand instanceof CIdExpression) {
        // function type &foo
        SMGAddressValueAndStateList addressValueAndStates =
            createAddressOfFunction((CIdExpression) amperOperand);
        return addressValueAndStates;
      } else if (amperOperand instanceof CIdExpression) {
        // &a
        SMGAddressValueAndStateList addressValueAndState =
            createAddressOfVariable((CIdExpression) amperOperand);
    ...
    protected SMGAddressValueAndStateList createAddressOfFunction(
        CIdExpression idFunctionExpression)
    ...
    private SMGAddressValueAndStateList createAddressOfArraySubscript(
        CArraySubscriptExpression lValue)
        throws CPATransferException {
      ...
      for (SMGAddressValueAndState arrayAddressAndState : arrayAddressAndStates
          .asAddressValueAndStateList()) {
        ...
        for (SMGExplicitValueAndState subscriptValueAndState : subscriptValueAndStates) {
          ...
          SMGAddressValueAndStateList resultAddressAndState =
              createAddress(newState, arrayAddress.getObject(), offset);
    ...
    private SMGAddressValueAndStateList createAddressOfField(CFieldReference lValue)
      ...
      for (SMGAddressAndState addressOfFieldAndState : addressOfFieldAndStates) {
        ...
        SMGAddressValueAndStateList resultAddressValueAndState =
            createAddress(addressOfFieldAndState.getSmgState(),
    ...
    private SMGAddressValueAndStateList createAddressOfVariable(CIdExpression idExpression)
        throws SMGInconsistentException {
    ...
    @Override
    public SMGAddressValueAndStateList visit(CPointerExpression pointerExpression)
        throws CPATransferException {
    ...
    @Override
    public SMGAddressValueAndStateList visit(CBinaryExpression binaryExp)
        throws CPATransferException {
      ...
      if (lVarIsAddress == rVarIsAddress) {
        return SMGAddressValueAndStateList
            .of(getInitialSmgState()); // If both or neither are Addresses,
    ...
    @Override
    public SMGAddressValueAndStateList visit(CArraySubscriptExpression exp)
        throws CPATransferException {
      return getAddressFromSymbolicValues(super.visit(exp));
    }
  ...
  private SMGAddressValueAndStateList handlePointerArithmetic(
        ...
        for (SMGAddressValueAndState addressValueAndState : addressValueAndStates
          ...
          for (SMGExplicitValueAndState offsetValueAndState : offsetValueAndStates) {
            ...
            SMGExplicitValue typeSize =
                SMGKnownExpValue.valueOf(getSizeof(cfaEdge, typeOfPointer, newState, address));
            ...
            switch (binaryOperator) {
              case PLUS:
                SMGAddressValueAndStateList resultAddressValueAndStateList =
                    createAddress(newState, target, addressOffset.add(pointerOffsetValue));
                result.addAll(resultAddressValueAndStateList.asAddressValueAndStateList());
                break;
              case MINUS:
                if (lVarIsAddress) {
                  resultAddressValueAndStateList =
                      createAddress(newState, target, addressOffset.subtract(pointerOffsetValue));
                  result.addAll(resultAddressValueAndStateList.asAddressValueAndStateList());
                  break;
                } else {
  ...
  protected SMGAddressValueAndStateList createAddresses(List<SMGAddressAndState> pAddresses)
      throws SMGInconsistentException {
  ...
  private SMGAddressValueAndStateList createAddress(SMGAddressAndState addressAndState)
      throws SMGInconsistentException {
   ...
   * @throws SMGInconsistentException thrown if the symbolic address is misinterpreted as a
   *                                  pointer.
   */
  protected SMGAddressValueAndStateList getAddressFromSymbolicValues(
      SMGValueAndStateList pAddressValueAndStateList)
      throws SMGInconsistentException {
    ...
    if (pAddressValueAndStateList instanceof SMGAddressValueAndStateList) {
      ...
      for (SMGValueAndState valueAndState : pAddressValueAndStateList.getValueAndStateList()) {
        addressAndStateList
            .addAll(getAddressFromSymbolicValue(valueAndState).asAddressValueAndStateList());
      }
   ...
   *                                  pointer.
   */
  SMGAddressValueAndStateList getAddressFromSymbolicValue(SMGValueAndState pAddressValueAndState)
      throws SMGInconsistentException {

    if (pAddressValueAndState instanceof SMGAddressValueAndState) {
      return SMGAddressValueAndStateList
          .of((SMGAddressValueAndState) pAddressValueAndState);
    }
    ...
    if (pAddressValue instanceof SMGAddressValue) {
      return SMGAddressValueAndStateList
          .of(SMGAddressValueAndState.of(smgState,
    ...
    if (!smgState
        .isPointer(pAddressValue.getAsInt())) {
      return SMGAddressValueAndStateList.of(smgState);
    }
  ...
  SMGAddressValueAndStateList createAddress(
      SMGState pSmgState, SMGObject pTarget,
  ...
  SMGAddressValueAndStateList getAddress(
      SMGState pSmgState, SMGObject pTarget,
      SMGExplicitValue pOffset) throws SMGInconsistentException {
        ...
        for (SMGAddressAndState addressAndState : addressAndStates) {
          ...
          SMGValueAndState pointerAndState =
              readValue(newState, address.getObject(),
                  address.getOffset(), type, getCfaEdge(), pVariableName);

          SMGAddressValueAndStateList trueAddressAndState =
              getAddressFromSymbolicValue(pointerAndState);
  ...
  class AssumeVisitor extends ExpressionValueVisitor {
      ...
      switch (binaryOperator) {
        ...
        case GREATER_THAN:
          ...
          for (SMGValueAndState leftSideValAndState : leftSideValAndStates.getValueAndStateList()) {
            ...
            for (SMGValueAndState rightSideValAndState : rightSideValAndStates
    ...
    private class BinaryRelationEvaluator {
       ...
       * @param pOp      the given binary operator, that describes an boolean expression between two
       *                 values.
       * @param pV1      the first operand.
       * @param pV2      the second operand
       */
      public BinaryRelationEvaluator(
          SMGState newState, BinaryOperator pOp,
      ...
      private boolean isUnequal(
          SMGSymbolicValue pValue1, SMGSymbolicValue pValue2,
    ...
    public SMGSymbolicValue evaluateBinaryAssumption(
        SMGState newState, BinaryOperator pOp,
        SMGSymbolicValue v1, SMGSymbolicValue v2) {
  ...
  class ExpressionValueVisitor
      extends DefaultCExpressionVisitor<SMGValueAndStateList, CPATransferException>
    ...
    public SMGValueAndStateList visit(CArraySubscriptExpression exp) throws CPATransferException {
      ...
      for (SMGAddressAndState addressAndState : addressAndStateList) {
        ...
        SMGValueAndState symbolicValueResultAndState = readValue(newState, address.getObject(),
            address.getOffset(), getRealExpressionType(exp), cfaEdge, exp);
    ...
    public SMGValueAndStateList visit(CFieldReference fieldReference) throws CPATransferException {
      ...
      for (SMGAddressAndState addressOfFieldAndState : addressOfFieldAndStates) {
        ...
        result.add(readValue(newState, addressOfField.getObject(), addressOfField.getOffset(),
            fieldType, cfaEdge, fieldReference));
      }
    ...
    public SMGValueAndStateList visit(CIdExpression idExpression)
      ...
      if (decl instanceof CEnumerator) {
        ...
        SMGValueAndState result = readValue(smgState, variableObject, SMGKnownExpValue.ZERO,
            getRealExpressionType(idExpression), cfaEdge, idExpression);
    ...
    @Override
    public SMGValueAndStateList visit(CUnaryExpression unaryExpression)
        throws CPATransferException {
      ...
      switch (unaryOperator) {
        ...
        case AMPER:
          // Admittedly, an ampersand expression is usually handled by address visitor. But if
          // such case actually occurs, we do nothing instead of making the analysis crash.
          return SMGValueAndStateList.of(getInitialSmgState());
        ...
        case SIZEOF:
          int size = getSizeof(cfaEdge, getRealExpressionType(unaryOperand), getInitialSmgState(),
              unaryOperand);
          SMGSymbolicValue val =
              (size == 0) ? SMGKnownSymValue.ZERO : SMGUnknownValue.getInstance();
          return SMGValueAndStateList.of(getInitialSmgState(), val);
        case TILDE:
    ...
    @Override
    public SMGValueAndStateList visit(CPointerExpression pointerExpression)
        throws CPATransferException {
    ...
    @Override
    public SMGValueAndStateList visit(CTypeIdExpression typeIdExp)
        throws UnrecognizedCCodeException {
      ...
      switch (typeOperator) {
        case SIZEOF:
          SMGSymbolicValue val =
              getSizeof(cfaEdge, type, getInitialSmgState(), typeIdExp) == 0 ? SMGKnownSymValue.ZERO
                                                                             : SMGUnknownValue
                  .getInstance();
          return SMGValueAndStateList.of(getInitialSmgState(), val);
        default:
    ...
    public SMGValueAndStateList visit(CBinaryExpression exp) throws CPATransferException {
      ...
      SMGValueAndStateList lValAndStates =
          evaluateExpressionValue(getInitialSmgState(), getCfaEdge(), lVarInBinaryExp);
      ...
      for (SMGValueAndState lValAndState : lValAndStates.getValueAndStateList()) {
        ...
        SMGValueAndStateList rValAndStates =
            evaluateExpressionValue(newState, getCfaEdge(), rVarInBinaryExp);
    ...
    private SMGValueAndState evaluateBinaryExpression(
        SMGSymbolicValue lVal, SMGSymbolicValue rVal,
        BinaryOperator binaryOperator, SMGState newState) {

      if (lVal.equals(SMGUnknownValue.getInstance())
          || rVal.equals(SMGUnknownValue.getInstance())) {
      ...
      switch (binaryOperator) {
        ...
        case BINARY_XOR: {
          ...
          switch (binaryOperator) {
            ...
            case SHIFT_RIGHT:
              ...
              SMGSymbolicValue val =
                  (isZero) ? SMGKnownSymValue.ZERO : SMGUnknownValue.getInstance();
        ...
        case LESS_EQUAL: {
          ...
          SMGSymbolicValue assumptionVal =
    ...
    protected SMGValueAndStateList dereferenceArray(CExpression exp, CType derefType)
        throws CPATransferException {
      ...
      for (SMGAddressAndState addressAndState : addressAndStates) {
        ...
        if (derefType instanceof CArrayType) {
          result.addAll(createAddress(newState, address.getObject(), address.getOffset())
              .asAddressValueAndStateList());
        } else {
          result.add(
              readValue(newState, address.getObject(), address.getOffset(), derefType, cfaEdge,
                  exp));
        }
      }
    ...
    protected final SMGValueAndStateList dereferencePointer(
        CExpression exp,
    ...
    public Value visit(CBinaryExpression binaryExp)
      ...
      if (value.isUnknown() && isPointerComparison(binaryExp)) {
        ...
        for (int c = 1; c < symValueAndStates.size(); c++) {
          smgStatesToBeProccessed
              .add(symValueAndStates.getValueAndStateList().get(c).getSmgState());
        }
  ...
  public static class SMGValueAndStateList {
    ...
    public List<SMGState> asSMGStateList() {
      ...
      return FluentIterable.from(valueAndStateList)
          .transform(new Function<SMGValueAndState, SMGState>() {
  ...
  public static class SMGAddressValueAndStateList extends SMGValueAndStateList {
      ...
      return FluentIterable.from(getValueAndStateList())
          .transform(new Function<SMGValueAndState, SMGAddressAndState>() {
            ...
            public SMGAddressAndState apply(SMGValueAndState valueAndState) {
              ...
              SMGAddressValueAndState addressValueAndState =
                  (SMGAddressValueAndState) valueAndState;
    ...
    public List<SMGAddressValueAndState> asAddressValueAndStateList() {
      return FluentIterable.from(getValueAndStateList())
          .transform(new Function<SMGValueAndState, SMGAddressValueAndState>() {
    ...
    public static SMGAddressValueAndStateList copyOfAddressValueList(
        List<SMGAddressValueAndState> pList) {
  ...
  public static class CSizeOfVisitor extends BaseSizeofVisitor {
    ...
    public CSizeOfVisitor(
        MachineModel pModel, CFAEdge pEdge, SMGState pState,
