...
final class SMGConsistencyVerifier {
   ...
   */
  static private boolean checkSingleFieldConsistency(
      LogManager pLogger,
      SMGObject pObject,
      SMG pSmg) {
    ...
    for (SMGEdgeHasValue hvEdge : pSmg.getHVEdges(filter)) {
      if ((hvEdge.getOffset() + hvEdge.getSizeInBytes(pSmg.getMachineModel())) > pObject
          .getSize()) {
   ...
   */
  static private boolean verifyEdgeConsistency(
      LogManager pLogger,
      SMG pSmg,
      Collection<? extends SMGEdge> pEdges) {
