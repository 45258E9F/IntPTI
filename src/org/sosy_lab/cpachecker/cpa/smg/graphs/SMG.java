...
public class SMG {
  ...
  protected Map<Integer, SMGEdgePointsTo> pt_edges = new HashMap<>();
  ...
  protected final MachineModel machine_model;
   ...
   */
  final public void addObject(
      final SMGObject pObj,
      final boolean pValidity,
      final boolean pExternal) {
    objects.add(pObj);
   ...
   *
   * @param pNewHV Keeps consistency: no
   */
  ...
  public void addPredicateRelation(
      SMGSymbolicValue pV1, SMGSymbolicValue pV2, BinaryOperator
      pOp, CFAEdge pEdge) {
    if (trackPredicates && pEdge instanceof CAssumeEdge) {
      ...
      if (assumeEdge.getTruthAssumption()) {
        ...
        symbolicRelations.addRelation(pV1, pV2, pOp.getOppositeLogicalOperator());
      }
    }
  }

  public void addPredicateRelation(
      SMGSymbolicValue pSymbolicValue, SMGExplicitValue pExplicitValue,
    ...
    if (trackPredicates && pEdge instanceof CAssumeEdge) {
      ...
      if (assumeEdge.getTruthAssumption()) {
        ...
        symbolicRelations
            .addExplicitRelation(pSymbolicValue, pExplicitValue, pOp.getOppositeLogicalOperator());
      }
    }
  }
   ...
   * to any object.
   *
   * Throws {@link IllegalArgumentException} if pValue is not present in the SMG.
   *
   * TODO: Test TODO: Consistency check: no value can point to more objects
   */
  final public SMGObject getObjectPointedBy(Integer pValue) {
    if (!values.contains(pValue)) {
      // FIX: no exception is thrown here, since this case is possible when user writes a
      // problematic program (such as allocating a negative size of memory)
      return null;
    }
   ...
   * @return A bitset. A bit has 1 value if the appropriate byte is guaranteed to be NULL (is
   * covered by a HasValue edge leading from an object to null value, 0 otherwise.
   */
  public BitSet getNullBytesForObject(SMGObject pObj) {
    BitSet bs = new BitSet(pObj.getSize());
    bs.clear();
    SMGEdgeHasValueFilter objectFilter =
        SMGEdgeHasValueFilter.objectFilter(pObj).filterHavingValue(getNullValue());
