...
public class SMGIsLessOrEqual {
  ...
  public static boolean isLessOrEqual(CLangSMG pSMG1, CLangSMG pSMG2) {
    ...
    BiMap<Integer, Integer> valueMap = HashBiMap.create();
    Set<Integer> unIncludeValue = new HashSet<>(pSMG1.getValues());
    unIncludeValue.removeAll(new HashSet<>(pSMG2.getValues()));
    ...
    for (Entry<String, SMGRegion> entry : globals_in_smg1.entrySet()) {
      ...
      if (!isLessOrEqualFields(pSMG1, pSMG2, globalInSMG1, globalInSMG2, valueMap,
          unIncludeValue)) {
        return false;
      }
    }
    ...
    while (smg1stackIterator.hasNext() && smg2stackIterator.hasNext()) {
      ...
      if (!(frameInSMG1.getFunctionDeclaration().getType().getReturnType()
          .getCanonicalType() instanceof CVoidType)
          && !isLessOrEqualFields(pSMG1, pSMG2, frameInSMG1.getReturnObject(),
          frameInSMG2.getReturnObject(), valueMap, unIncludeValue)) {
        return false;
      }
      ...
      for (String localVar : localVars) {
        ...
        if (!isLessOrEqualFields(pSMG1, pSMG2, localInSMG1, localInSMG2, valueMap,
            unIncludeValue)) {
          return false;
        }
      }
    ...
    for (SMGObject object_in_smg1 : heap_in_smg1) {
      ...
      if (!isLessOrEqualFields(pSMG1, pSMG2, object_in_smg1, object_in_smg1, valueMap,
          unIncludeValue)) {
        return false;
      }
  ...
  private static boolean isLessOrEqualFields(
      CLangSMG pSMG1, CLangSMG pSMG2, SMGObject pSMGObject1,
      SMGObject pSMGObject2, BiMap<Integer, Integer> valueMap, Set<Integer> unIncludeValue) {
    ...
    if (!(pSMG1.getObjects().contains(pSMGObject1)
        && pSMG2.getObjects().contains(pSMGObject2))) {
      throw new IllegalArgumentException(
    ...
    for (SMGEdgeHasValue edge1 : HVE1) {
      boolean isPointer = pSMG1.isPointer(edge1.getValue());

      filterForSMG2.filterAtOffset(edge1.getOffset()).filterByType(edge1.getType())
          .filterHavingValue(edge1.getValue());
      if (unIncludeValue.contains(edge1.getValue())) {
        for (SMGEdgeHasValue smgEdgeHasValue : HVE2) {
          if (valueMap.containsValue(smgEdgeHasValue.getValue()) || pSMG1.getValues()
              .contains(smgEdgeHasValue.getValue())) {
            return false;
          }
          valueMap.put(edge1.getValue(), smgEdgeHasValue.getValue());
          unIncludeValue.remove(edge1.getValue());
          break;
        }
      }
      ...
      if (isPointer) {
        if (!pSMG2.isPointer(edge1.getValue())) {
          return false;
        }
        SMGEdgePointsTo ptE1 = pSMG1.getPointer(edge1.getValue());
        SMGEdgePointsTo ptE2 = pSMG2.getPointer(edge1.getValue());
