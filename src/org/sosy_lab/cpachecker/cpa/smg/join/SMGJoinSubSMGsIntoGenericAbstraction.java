...
public class SMGJoinSubSMGsIntoGenericAbstraction {
  ...
  private Optional<GenericAbstractionCandidate> createGenericAbstractionCandidate(
    ...
    builder.getAbstractToConcretePointerMap()
        .putAll(pMatchResult.getAbstractToConcretePointerMap());

    for (Entry<Integer, Integer> entry : pMatchResult.getAbstractToConcretePointerMapInputSMG()
        .entrySet()) {
    ...
    for (SMGObject obj : pMatchResult2.getObjectsToBeRemovedForAbstractionInputSMG()) {
      if (mapping2.containsKey(obj) && !builder.getObjectsToBeRemovedForAbstraction()
          .contains(mapping2.get(obj))) {
        return Optional.absent();
      }
    }

    int score = pMatchResult.getScore() > pMatchResult2.getScore() ? pMatchResult.getScore()
                                                                   : pMatchResult2.getScore();
  ...
  private MatchResult subSMGmatchSpecificShape(
    ...
    if (pRootObject instanceof GenericAbstraction) {
      ...
      return subSMGmatchSpecificShape(pInputSMG, (SMGRegion) pRootObject, pMapping, pTemplate,
          pAlreadyVisited);
    } else {
  ...
  private MatchResult subSMGmatchStep(
    ...
    for (SMGObjectTemplate entryRegion : entryRegions) {
      ...
      result =
          subSMGmatchStep(pMatStep, pInputSMG, pRootObject, entryRegion, pMapping, alreadyVisited,
              pTemplate);
  ...
  private MatchResult subSMGmatchStep(
      MaterlisationStep pMatStep, SMG pInputSMG,
    ...
    while (!toBeMatchedLater.isEmpty()) {
      ...
      for (SMGNodeTemplateAndNode templateNodeAndConcreteNode : toBeMatched) {
        ...
        boolean match =
            matchNodeTemplateWithNode(node, nodeTemplate, pMatStep, templateToInputSmgMapping,
                pInputSMG, toBeMatchedLater, pAlreadyVisited, pMapping);
  ...
  private boolean matchNodeTemplateWithNode(
      SMGNode pNode, SMGNodeTemplate pNodeTemplate,
    ...
    if (pNode.isValue() && pNodeTemplate.isValueTemplate()) {
      return matchValueTemplateWithValue(pNode.getValue(), pNodeTemplate.getValueTemplate(),
          pMatStep, pTemplateToInputSmgMapping, pInputSMG, pToBeMatchedLater, pAlreadyVisited,
          pMapping);
  ...
  private boolean matchGenericAbstractionTemplateWithGenericAbstraction(
    ...
    Map<Integer, SMGEdgeHasValue> fieldsOfRegionMap =
        FluentIterable.from(fieldsOfRegion).uniqueIndex(new MapHasValueEdgeToOffset());
    ...
    for (int offset : fieldsOfRegionMap.keySet()) {
      if (fieldsOfRegionTemplateMap.containsKey(offset)) {
        ...
        if (pTemplateToInputSmgMapping.contains(pointerTemplate)) {
          ...
          pTemplateToInputSmgMapping.put(pointerTemplate, pointer);
          if (pMatStep.getAbstractFieldsToIPointer()
  ...
  private Set<SMGEdgePointsTo> getPointerToThisAbstraction(
      MatchResult pGenAbsMatchResult,
      SMG pInputSMG) {
    ...
    for (int offset : fieldsOfRegionMap.keySet()) {
      if (fieldsOfRegionTemplateMap.containsKey(offset)) {
        ...
        if (pMatStepToSubSMGMapping.contains(pointerTemplate)) {
          ...
          pMatStepToSubSMGMapping.put(pointerTemplate, pointer);
          if (matStep.getAbstractFieldsToIPointer()
              .contains(fieldsOfRegionTemplateMap.get(offset))) {
            pToBeMatchedLater.add(new SMGNodeTemplateAndNode(pointerTemplate, pointer));
          }
        }
  ...
  private MatchResult wasMatchedPreviously(
      SMG pInputSMG, SMGObject pRootObject,
  ...
  private static class MatchResult {
    ...
    public MatchResult(
        boolean pMatche, Set<SMGObject> pObjectsToBeRemovedForAbstraction,
    ...
    public static MatchResult valueOf(GenericAbstractionCandidate pAbstractionCandidate) {
      ...
      return new MatchResult(true, pAbstractionCandidate.getObjectsToBeRemoved(),
          ...
          pAbstractionCandidate.createTemplate());
    }
    ...
    public static MatchResult valueOf(GenericAbstraction pRootObject) {
      ...
      return new MatchResult(true, emptySet, 100, emptyMap,
          ...
          pRootObject.createCandidateTemplate());
    }
