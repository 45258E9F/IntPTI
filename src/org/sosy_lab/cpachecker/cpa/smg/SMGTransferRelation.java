...
import org.sosy_lab.cpachecker.cpa.smg.checker.NullPointerDereferenceChecker;
...
public class SMGTransferRelation extends SingleEdgeTransferRelation
  ...
  private static enum SMGExportLevel {
    NEVER,
    LEAF,
    INTERESTING,
    EVERY
  ...
  private static enum UnknownFunctionHandling {
    STRICT,
    ASSUME_SAFE
  }
  ...
  @Option(
      ...
      description = "Function which indicate on external allocated memory")
  private String externalAllocationFunction = "ext_allocation";

  private static final String tempVarTemplate = "__CPAchecker_TMP";
  ...
  private CheckerManager<SMGState> checkerManager;
  ...
  private class SMGBuiltins {
      ...
      if (exportSMGFilePattern != null && currentState != null) {
        if (name == null) {
          if (currentState.getPredecessorId() == 0) {
            ...
            name = String.format("%03d-%03d-%03d", currentState.getPredecessorId(),
                currentState.getId(), id_counter.getAndIncrement());
          }
        }
        name = name.replace("\"", "");
    ...
    public final SMGAddressValueAndStateList evaluateMemset(
      ...
      try {
        ...
        throw new UnrecognizedCCodeException("Memset buffer argument not found.", cfaEdge,
            functionCall);
      }
      ...
      try {
        ...
        logger.logDebugException(e);
        throw new UnrecognizedCCodeException("Memset ch argument not found.", cfaEdge,
            functionCall);
      }
      ...
      try {
        ...
        logger.logDebugException(e);
        throw new UnrecognizedCCodeException("Memset count argument not found.", cfaEdge,
            functionCall);
      }

      List<SMGAddressValueAndState> result = new ArrayList<>(4);

      SMGAddressValueAndStateList bufferAddressAndStates =
      ...
      for (SMGAddressValueAndState bufferAddressAndState : bufferAddressAndStates
        ...
        List<SMGExplicitValueAndState> countValueAndStates =
            evaluateExplicitValue(currentState, cfaEdge, countExpr);
        ...
        for (SMGExplicitValueAndState countValueAndState : countValueAndStates) {
          ...
          for (SMGValueAndState chAndState : chAndStates.getValueAndStateList()) {
            ...
            List<SMGExplicitValueAndState> expValueAndStates =
                evaluateExplicitValue(currentState, cfaEdge, chExpr);

            for (SMGExplicitValueAndState expValueAndState : expValueAndStates) {

              SMGAddressValueAndState memsetResult =
                  evaluateMemset(expValueAndState.getSmgState(), cfaEdge,
                      bufferAddressAndState.getObject(),
                      countValueAndState.getObject(), chAndState.getObject(),
    ...
    private final SMGAddressValueAndState evaluateMemset(
        SMGState currentState, CFAEdge cfaEdge,
        SMGAddressValue bufferAddress, SMGExplicitValue countValue, SMGSymbolicValue ch,
        SMGExplicitValue expValue)
        throws CPATransferException {

      if (bufferAddress.isUnknown()
          || countValue.isUnknown()) {
      ...
      if (ch.equals(SMGKnownSymValue.ZERO)) {
        ...
        for (int c = 0; c < count; c++) {
          currentState = writeValue(currentState, bufferMemory, offset + c,
    ...
    protected SMGValueAndStateList evaluateExpressionValue(
        SMGState smgState, CFAEdge cfaEdge,
        CExpression rValue)
        throws CPATransferException {
    ...
    protected SMGAddressValueAndStateList evaluateAddress(
        SMGState pState, CFAEdge pCfaEdge,
        CRightHandSide pRvalue) throws CPATransferException {
    ...
    public final SMGAddressValueAndStateList evaluateAlloca(
        CFunctionCallExpression functionCall,
    ...
    private SMGAddressValueAndState evaluateAlloca(
        SMGState currentState,
        SMGExplicitValue pSizeValue, CFAEdge cfaEdge, CRightHandSide sizeExpr)
        throws CPATransferException {
      ...
      SMGAddressValue addressValue =
          currentState.addNewStackAllocation(sizeValue.getAsInt(), allocation_label);
    ...
    private List<SMGExplicitValueAndState> getAllocateFunctionSize(
        SMGState pState, CFAEdge cfaEdge,
    ...
    public SMGAddressValueAndStateList evaluateConfigurableAllocationFunction(
      ...
      List<SMGExplicitValueAndState> sizeAndStates =
      ...
      for (SMGExplicitValueAndState sizeAndState : sizeAndStates) {
        ...
        if (zeroingMemoryAllocation.contains(functionName)) {
          currentState = writeValue(currentState, new_address.getObject(), 0,
              AnonymousTypes.createTypeWithLength(size),
    ...
    public SMGAddressValueAndStateList evaluateConfigurableAllocationFunction(
        CFunctionCallExpression functionCall,
        SMGState pState, CFAEdge cfaEdge, String name, CType type) throws CPATransferException {
      SMGState currentState = pState;
      ...
      for (SMGExplicitValueAndState sizeAndState : sizeAndStates) {
        ...
        // update type information in memory allocation
        if (type == null) {
          type = functionCall.getExpressionType();
        }
        currentState.addTypeInfo(allocation_label, type, TypeAndAlloca.DYNAMIC);
        // if the left name corresponds to temporary variable, we record this for further process
        if (name != null) {
          currentState.addTemporaryVar2MemorySpace(name, allocation_label);
        }
    ...
    public final List<SMGState> evaluateFree(
      ...
      try {
        ...
        logger.logDebugException(e);
        throw new UnrecognizedCCodeException("Built-in free(): No parameter passed", cfaEdge,
            pFunctionCall);
      }

      SMGAddressValueAndStateList addressAndStates =
      ...
      for (SMGAddressValueAndState addressAndState : addressAndStates
        ...
        if (address.getAsInt() == 0) {
          ...
          currentState = currentState.free(address.getAsInt(), address.getOffset().getAsInt(),
              address.getObject());
        }
    ...
    public boolean isConfigurableAllocationFunction(String functionName) {
      return memoryAllocationFunctions.contains(functionName)
          || arrayAllocationFunctions.contains(functionName);
    }
    ...
    public SMGAddressValueAndStateList evaluateMemcpy(
        CFunctionCallExpression pFunctionCall,
      ...
      try {
        ...
        logger.logDebugException(e);
        throw new UnrecognizedCCodeException("Memcpy target argument not found.", pCfaEdge,
            pFunctionCall);
      }
      ...
      try {
        ...
        logger.logDebugException(e);
        throw new UnrecognizedCCodeException("Memcpy source argument not found.", pCfaEdge,
            pFunctionCall);
      }
      ...
      try {
        ...
        logger.logDebugException(e);
        throw new UnrecognizedCCodeException("Memcpy count argument not found.", pCfaEdge,
            pFunctionCall);
      }

      List<SMGAddressValueAndState> result = new ArrayList<>(4);

      SMGAddressValueAndStateList targetStr1AndStates =
      ...
      for (SMGAddressValueAndState targetStr1AndState : targetStr1AndStates
        ...
        SMGAddressValueAndStateList sourceStr2AndStates =
        ...
        for (SMGAddressValueAndState sourceStr2AndState : sourceStr2AndStates
          ...
          List<SMGExplicitValueAndState> sizeValueAndStates =
              evaluateExplicitValue(currentState, pCfaEdge, sizeExpr);

          for (SMGExplicitValueAndState sizeValueAndState : sizeValueAndStates) {
            result
                .add(evaluateMemcpy(sizeValueAndState.getSmgState(), targetStr1AndState.getObject(),
                    sourceStr2AndState.getObject(), sizeValueAndState.getObject()));
          }
        }
      }
    ...
    private SMGAddressValueAndState evaluateMemcpy(
        ...
        SMGAddressValue sourceStr2Address, SMGExplicitValue sizeValue)
  ...
  private void plotWhenConfigured(
      SMGExportLevel level, String pName, SMGState pState,
      String pLocation) {
  ...
  public Collection<? extends AbstractState> getAbstractSuccessorsForEdge(
    ...
    if (cfaEdge instanceof MultiEdge) {
      ...
      for (CFAEdge edge : multiEdge) {
        ...
        while (!processQueue.isEmpty()) {
          ...
          Collection<? extends AbstractState> resultOfOneOp =
              getAbstractSuccessorsForEdge(next, edge);
    ...
    if (cfaEdge.getSuccessor().isLoopStart()) {
      HashSet<SMGState> abstractResult = new HashSet<>();
      for (AbstractState abstractState : results) {
        abstractResult.add(((SMGState) abstractState).abstraction());
      }
  ...
  public SMGTransferRelation(
    ...
    checkerManager = new CheckerManager<>(config, SMGState.class);
    checkerManager.register(NullPointerDereferenceChecker.class);
  }

  public static SMGTransferRelation createTransferRelationForRefinement(
      ...
      case FunctionReturnEdge:
        ...
        if (checkForMemLeaksAtEveryFrameDrop) {
          for (SMGState successor : successors) {
            String name = String.format("%03d-%03d-%03d", successor.getPredecessorId(),
                successor.getId(), id_counter.getAndIncrement());
            plotWhenConfigured(SMGExportLevel.INTERESTING, "beforePrune" + name, successor,
  ...
  private void plotWhenConfigured(
      SMGExportLevel pInteresting, String pName, List<SMGState> pStates,
      String pDescription) {
  ...
  private List<SMGState> handleExitFromFunction(
      SMGState smgState,
    ...
    if (tmpFieldMemory != null) {
      return handleAssignmentToField(smgState, returnEdge,
          tmpFieldMemory, 0, null, expType, returnExp);
    }
  ...
  private List<SMGState> handleFunctionReturn(
    ...
    if (exprOnSummary instanceof CFunctionCallAssignmentStatement) {
      ...
      CType rValueType = expressionEvaluator.getRealExpressionType(
      ...
      for (SMGAddressAndState addressAndValue : addressAndValues) {
        ...
        if (!address.isUnknown()) {
          ...
          SMGState resultState =
              assignFieldToState(newState, functionReturnEdge, object, offset, rValue, rValueType);
  ...
  private SMGSymbolicValue getFunctionReturnValue(SMGState smgState, CType type, CFAEdge pCFAEdge)
    ...
    return expressionEvaluator
        .readValue(smgState, tmpMemory, SMGKnownExpValue.ZERO, type, pCFAEdge, null)
        .getObject();//TODO: null should be replace by another expression
  }
  ...
  private List<SMGState> handleFunctionCall(SMGState smgState, CFunctionCallEdge callEdge)
    ...
    for (int i = 0; i < paramDecl.size(); i++) {
      ...
      for (SMGState newState : newStates) {
        ...
        SMGValueAndStateList stateValues =
            readValueToBeAssiged(newState, callEdge, paramObj, 0, exp);
        ...
        for (SMGValueAndState stateValue : stateValues.getValueAndStateList()) {
          ...
          List<SMGState> newStatesWithExpVal = assignExplicitValueToSymbolicValue(
    ...
    for (SMGState newState : newStates) {
      ...
      for (int i = 0; i < paramDecl.size(); i++) {
        ...
        if (rValueType instanceof CArrayType) {
          rValueType = new CPointerType(rValueType.isConst(), rValueType.isVolatile(),
              ((CArrayType) rValueType).getType());
        }

        if (cParamType instanceof CArrayType) {
          cParamType = new CPointerType(cParamType.isConst(), cParamType.isVolatile(),
  ...
  private List<SMGState> handleAssumption(
      SMGState pSmgState, CExpression expression,
      CFAEdge cfaEdge,
    ...
    for (SMGValueAndState valueAndState : valueAndStates.getValueAndStateList()) {
      ...
      if (!value.isUnknown()) {
        ...
        result.addAll(
            deriveFurtherInformationFromAssumption(smgState, visitor, cfaEdge, truthValue,
                expression,
  ...
  private List<SMGState> deriveFurtherInformationFromAssumption(
      ...
      CFAEdge cfaEdge, boolean truthValue, CExpression expression,
      boolean createNewStateIfNecessary) throws CPATransferException {
    ...
    List<SMGExplicitValueAndState> explicitValueAndStates =
    ...
    for (SMGExplicitValueAndState explicitValueAndState : explicitValueAndStates) {
      ...
      if (explicitValue.isUnknown()) {
        ...
        if (!val1ImpliesOn.isUnknown() && !val2ImpliesOn.isUnknown()) {
          if (impliesEqOn) {
            newState.identifyEqualValues((SMGKnownSymValue) val1ImpliesOn,
                (SMGKnownSymValue) val2ImpliesOn);
          } else if (impliesNeqOn) {
            newState.identifyNonEqualValues((SMGKnownSymValue) val1ImpliesOn,
                (SMGKnownSymValue) val2ImpliesOn);
          }
        }
  ...
  private CExpression eliminateOuterEquals(CExpression pExpression) {
    ...
    if (!(op1 instanceof CBinaryExpression && op2 instanceof CIntegerLiteralExpression
  ...
  private List<SMGState> handleStatement(SMGState pState, CStatementEdge pCfaEdge)
      throws CPATransferException {
    ...
    if (cStmt instanceof CAssignment) {
      ...
      if (builtins.isABuiltIn(functionName)) {
        ...
        if (builtins.isConfigurableAllocationFunction(functionName)) {
          ...
          newStates =
              builtins.evaluateConfigurableAllocationFunction(cFCExpression, newState, pCfaEdge)
                  .asSMGStateList();
        ...
        if (builtins.isExternalAllocationFunction(functionName)) {
          newStates = builtins.evaluateExternalAllocation(cFCExpression, newState, pCfaEdge)
              .asSMGStateList();
        }
        ...
        org.sosy_lab.cpachecker.cpa.smg.SMGExpressionEvaluator.ExpressionValueVisitor visitor =
            expressionEvaluator.getExpressionValueVisitor(pCfaEdge, pState);
        List<CExpression> parameters = cFCExpression.getParameterExpressions();
        for (CExpression cExpression : parameters) {
          List<SMGState> states = cExpression.accept(visitor).asSMGStateList();
          for (SMGState smgState : states) {
            if (smgState.checkProperty(SMGState.HAS_INVALID_READS)) {
              return ImmutableList
                  .of(pState.setInvalidRead(cExpression));
            }
          }
        }

        // TODO: migrate the implementation to a function adapter
        if (functionName.equals("fscanf")) {
          for (int i = 2; i < parameters.size(); i++) {
            CExpression arg = parameters.get(i);
            if (arg instanceof CUnaryExpression) {
              UnaryOperator operator = ((CUnaryExpression) arg).getOperator();
              if (operator == UnaryOperator.AMPER) {
                CExpression possibleNum = ((CUnaryExpression) arg).getOperand();
                CType type = expressionEvaluator.getRealExpressionType(possibleNum);
                if (Types.isNumericalType(type) && possibleNum instanceof CIdExpression) {
                  SMGObject variableObject = pState.getObjectForVisibleVariable(((CIdExpression)
                      possibleNum).getName());
                  Integer newValue = SMGValueFactory.getNewValue();
                  SMGStateEdgePair stateEdgePair = pState.writeValue(variableObject, 0, type,
                      newValue);
                  return ImmutableList.of(stateEdgePair.getState());
                }
              }
            }
          }
        ...
        switch (handleUnknownFunctions) {
          case STRICT:
            throw new CPATransferException("Unknown function '" + functionName
  ...
  private List<SMGState> handleAssignment(
      SMGState pState, CFAEdge cfaEdge, CExpression lValue,
    ...
    LValueAssignmentVisitor visitor =
        expressionEvaluator.getLValueAssignmentVisitor(cfaEdge, state);
    ...
    for (SMGAddressAndState addressOfFieldAndState : addressOfFieldAndStates) {
      ...
      if (lValue instanceof CIdExpression) {
        leftID = ((CIdExpression) lValue).getDeclaration().getQualifiedName();
        if (!leftID.contains(tempVarTemplate)) {
          leftID = null;
        }
      }
      ...
      if (addressOfField.isUnknown()) {
        ...
        List<SMGState> newStates =
            handleAssignmentToField(state, cfaEdge, addressOfField.getObject(),
                addressOfField.getOffset().getAsInt(), leftID, fieldType, rValue);
   ...
   *
   */
  private SMGValueAndStateList readValueToBeAssiged(
      SMGState pNewState, CFAEdge cfaEdge,
    ...
    for (SMGValueAndState valueAndState : valueAndStates.getValueAndStateList()) {
      ...
      if (value.isUnknown()) {
        ...
        if (expressionEvaluator.isMissingExplicitInformation()) {
          addMissingInformation(memoryOfField, fieldOffset, rValue,
              expressionEvaluator.isRequiered());
          expressionEvaluator.reset();
        }
  ...
  private SMGValueAndStateList readValueToBeAssiged(
      SMGState pNewState, CFAEdge cfaEdge, SMGObject memoryOfField, int fieldOffset,
      String leftName, CType leftType, CRightHandSide rValue) throws CPATransferException {

    SMGValueAndStateList valueAndStates =
        expressionEvaluator.evaluateExpressionValue(pNewState, cfaEdge, rValue, leftName, leftType);
  ...
  private List<SMGState> assignFieldToState(
      SMGState pNewState, CFAEdge cfaEdge,
      SMGObject memoryOfField, int fieldOffset,
      @Nullable String pLFieldName, CType pLFieldType,
    ...
    SMGValueAndStateList valueAndStates =
        readValueToBeAssiged(pNewState, cfaEdge, memoryOfField, fieldOffset,
            pLFieldName, pLFieldType, rValue);
    ...
    for (SMGValueAndState valueAndState : valueAndStates.getValueAndStateList()) {
      ...
      for (SMGState currentNewState : newStates) {
        newState = assignFieldToState(currentNewState, cfaEdge, memoryOfField, fieldOffset, value,
            rValueType);
        result.add(newState);
      }
    }
  ...
  private List<SMGState> assignExplicitValueToSymbolicValue(
      SMGState pNewState,
    ...
    List<SMGState> result =
        FluentIterable.from(expValueAndStates)
            .transform(new Function<SMGExplicitValueAndState, SMGState>() {
  ...
  private void addMissingInformation(
      SMGObject pMemoryOfField, int pFieldOffset,
  ...
  private SMGState assignFieldToState(
      SMGState newState, CFAEdge cfaEdge,
  ...
  private SMGState assignStruct(
      SMGState pNewState, SMGObject pMemoryOfField,
    ...
    if (pValue instanceof SMGKnownAddVal) {
      ...
      //FIXME Does not work with variable array length.
      int structSize =
          structOffset + expressionEvaluator.getSizeof(pCfaEdge, pRValueType, pNewState);
  ...
  private SMGState writeValue(
      SMGState pNewState, SMGObject pMemoryOfField, int pFieldOffset,
      long pSizeType,
      SMGSymbolicValue pValue, CFAEdge pEdge)
      throws UnrecognizedCCodeException, SMGInconsistentException {
    return writeValue(pNewState, pMemoryOfField, pFieldOffset,
        AnonymousTypes.createTypeWithLength(pSizeType), pValue, pEdge);
  }
  ...
  private SMGState writeValue(
    ...
    boolean doesNotFitIntoObject = pFieldOffset < 0
        || pFieldOffset
        + expressionEvaluator.getSizeof(pEdge, pRValueType, pNewState) > pMemoryOfField
        .getSize();
  ...
  private List<SMGState> handleAssignmentToField(
    ...
    List<SMGState> newStates =
        assignFieldToState(newState, cfaEdge, memoryOfField, fieldOffset, pLFieldName,
            pLFieldType, rValue);
  ...
  private List<SMGState> handleVariableDeclaration(
      SMGState pState, CVariableDeclaration pVarDecl,
      CDeclarationEdge pEdge) throws CPATransferException {
    ...
    */
    if (newObject == null) {
      ...
      if (pVarDecl.isGlobal()) {
        newObject = pState.addGlobalVariable(typeSize, varName, cType);
      } else {
        newObject = pState.addLocalVariable(typeSize, varName, cType);
      }
    }
  ...
  private List<SMGState> handleDeclaration(SMGState smgState, CDeclarationEdge edge)
      throws CPATransferException {
    ...
    List<SMGState> newStates =
        handleVariableDeclaration(newState, (CVariableDeclaration) cDecl, edge);

    return newStates;
  }

  private List<SMGState> handleInitializerForDeclaration(
      SMGState pState, SMGObject pObject,
  ...
  private List<SMGState> handleInitializer(
      SMGState pNewState, CVariableDeclaration pVarDecl,
      CFAEdge pEdge,
    ...
    if (pInitializer instanceof CInitializerExpression) {
      String LFieldName = pVarDecl.getQualifiedName();
      if (!LFieldName.contains(tempVarTemplate)) {
        LFieldName = null;
      }
      return assignFieldToState(pNewState, pEdge, pNewObject,
          pOffset, LFieldName, pLValueType,
      ...
      throw new AssertionError(
          "Error in handling initializer, designated Initializer " + pInitializer.toASTString()
  ...
  private List<SMGState> handleInitializerList(
    ...
    for (CInitializer initializer : pNewInitializer.getInitializers()) {
      ...
      for (Pair<SMGState, Integer> offsetAndState : offsetAndStates) {
        ...
        List<SMGState> pNewStates =
            handleInitializer(pNewState, pVarDecl, pEdge, pNewObject, offset, memberType,
                initializer);
        ...
        List<? extends Pair<SMGState, Integer>> newStatesAndOffset =
            FluentIterable.from(pNewStates)
                .transform(new ListToListOfPairFunction<SMGState, Integer>(offset))
                .toList();
    ...
    return FluentIterable.from(offsetAndStates)
        .transform(new Function<Pair<SMGState, Integer>, SMGState>() {
  ...
  private class SMGRightHandSideEvaluator extends SMGExpressionEvaluator {
    ...
    public SMGState deriveFurtherInformation(
        SMGState pNewState, boolean pTruthValue,
        CFAEdge pCfaEdge, CExpression rValue)
        throws CPATransferException {
    ...
    public SMGValueAndState readValue(
        SMGState pSmgState, SMGObject pObject,
        SMGExplicitValue pOffset, CType pType, CFAEdge pEdge, CExpression exp)
        throws SMGInconsistentException, UnrecognizedCCodeException {
      ...
      if (doesNotFitIntoObject) {
        ...
        return SMGValueAndState.of(pSmgState.setInvalidRead(exp));
      }

      return pSmgState.forceReadValue(pObject, fieldOffset, pType, exp);
    }

    public SMGValueAndStateList evaluateExpressionValue(
        SMGState smgState, CFAEdge cfaEdge,
        CRightHandSide rValue, String lName,
        CType lType)
        throws CPATransferException {
      ...
      if (expressionType instanceof CPointerType ||
        ...
        return evaluateAddress(smgState, cfaEdge, rValue, lName, lType);
      } else {
        return evaluateNonAddressValue(smgState, cfaEdge, rValue);
      }
    }

    private SMGAddressValueAndStateList evaluateAddress(
        SMGState state, CFAEdge cfaEdge,
        CRightHandSide rValue, String lName,
        CType lType)
        throws CPATransferException {
      ...
      if (expressionType instanceof CPointerType
        ...
        PointerAddressVisitor visitor = new PointerAddressVisitor(cfaEdge, state, lName, lType);
       ...
       */
        StructAndUnionVisitor visitor = getStructAndUnionVisitor(cfaEdge, state);
        ...
        ArrayVisitor visitor = getArrayVisitor(cfaEdge, state);
    ...
    private class AssigningValueVisitor
      ...
      public Void visit(CBinaryExpression binExp) throws CPATransferException {
        ...
        if (operand2 instanceof CLeftHandSide) {
          deriveFurtherInformation((CLeftHandSide) operand2, operand1,
              op.getOppositeLogicalOperator());
        }

        return null;
      }

      private void deriveFurtherInformation(
          CLeftHandSide lValue, CExpression exp,
          BinaryOperator op) throws CPATransferException {
        ...
        if (rSymValue.isUnknown()) {
          ...
          SMGExpressionEvaluator.LValueAssignmentVisitor visitor =
              getLValueAssignmentVisitor(edge, assignableState);
    ...
    private class LValueAssignmentVisitor extends SMGExpressionEvaluator.LValueAssignmentVisitor {
      ...
      @Override
      public List<SMGAddressAndState> visit(CIdExpression variableName)
          throws CPATransferException {
      ...
      @Override
      public List<SMGAddressAndState> visit(CArraySubscriptExpression lValue)
          throws CPATransferException {
    ...
    private class ExpressionValueVisitor extends SMGExpressionEvaluator.ExpressionValueVisitor {
      ...
      public SMGValueAndStateList visit(CFunctionCallExpression pIastFunctionCallExpression)
        ...
        if (builtins.isABuiltIn(functionName)) {
          if (builtins.isConfigurableAllocationFunction(functionName)) {
            possibleMallocFail = true;
            SMGAddressValueAndStateList configAllocEdge =
          ...
          switch (functionName) {
            ...
            case "__builtin_alloca":
              possibleMallocFail = true;
              SMGAddressValueAndStateList allocEdge = builtins
                  .evaluateAlloca(pIastFunctionCallExpression, getInitialSmgState(), getCfaEdge());
              return allocEdge;
            case "printf":
    ...
    private class ForceExplicitValueVisitor extends
      ...
      public ForceExplicitValueVisitor(
          SMGState pSmgState, String pFunctionName,
          MachineModel pMachineModel,
      ...
      private Value guessLHS(CLeftHandSide exp)
        ...
        // TODO: guess a possible value with respect to constraint CPA
    ...
    private class PointerAddressVisitor extends SMGExpressionEvaluator.PointerVisitor {
      ...
      public PointerAddressVisitor(CFAEdge pEdge, SMGState pSMGState, String lName, CType lType) {
        super(pEdge, pSMGState);
        leftName = lName;
        leftType = lType;
      }
      ...
      public SMGAddressValueAndStateList visit(CIdExpression exp) throws CPATransferException {
        // if the qualified name of identifier is in the template of temporary variable, then we
        // refine the type mapping
        String varName = exp.getDeclaration().getQualifiedName();
        if (varName.contains(tempVarTemplate)) {
          initialSmgState.reduceTempVar(varName, leftType);
        }
      ...
      public SMGAddressValueAndStateList visit(CCastExpression pCast) throws CPATransferException {
        return getAddressFromSymbolicValues(evaluateExpressionValue(getInitialSmgState(),
            getCfaEdge(), pCast.getOperand(), leftName, leftType));
      }

      @Override
      protected SMGAddressValueAndStateList createAddressOfFunction(
          CIdExpression pIdFunctionExpression)
          throws SMGInconsistentException {
      ...
      public SMGAddressValueAndStateList visit(CFunctionCallExpression pIastFunctionCallExpression)
        ...
        if (builtins.isABuiltIn(functionName)) {
          if (builtins.isConfigurableAllocationFunction(functionName)) {
            ...
            SMGAddressValueAndStateList configAllocEdge;
            if (leftType == null) {
              configAllocEdge = builtins.evaluateConfigurableAllocationFunction(
                  pIastFunctionCallExpression, getInitialSmgState(), getCfaEdge());
            } else {
              configAllocEdge = builtins.evaluateConfigurableAllocationFunction(
                  pIastFunctionCallExpression, getInitialSmgState(), getCfaEdge(),
          ...
          switch (functionName) {
            case "__builtin_alloca":
              ...
              return allocEdge;
            case "memset":
              SMGAddressValueAndStateList memsetTargetEdge = builtins
                  .evaluateMemset(pIastFunctionCallExpression, getInitialSmgState(), getCfaEdge());
              return memsetTargetEdge;
            case "memcpy":
              SMGAddressValueAndStateList memcpyTargetEdge = builtins
                  .evaluateMemcpy(pIastFunctionCallExpression, getInitialSmgState(), getCfaEdge());
    ...
    public List<SMGExplicitValueAndState> evaluateExplicitValue(
        SMGState pSmgState,
        CFAEdge pCfaEdge, CRightHandSide pRValue) throws CPATransferException {

      List<SMGExplicitValueAndState> explicitValues =
          super.evaluateExplicitValue(pSmgState, pCfaEdge, pRValue);
    ...
    @Override
    protected SMGValueAndState handleUnknownDereference(
        SMGState pSmgState,
        CFAEdge pEdge) {
  ...
  private Collection<SMGState> strengthen(
      AutomatonState pAutomatonState, SMGState pElement,
      CFAEdge pCfaEdge) throws CPATransferException {

    List<AssumeEdge> assumptions =
        pAutomatonState.getAsAssumeEdges(pCfaEdge.getPredecessor().getFunctionName());
    ...
    for (AssumeEdge assume : assumptions) {
      ...
      List<SMGState> newElements =
          handleAssumption(newElement, ((CAssumeEdge) assume).getExpression(), pCfaEdge,
              assume.getTruthAssumption(), pElement == newElement);
  ...
  public static Set<CExpression> errs = new HashSet<>();
  ...
  private Collection<? extends AbstractState> strengthen(
    ...
    for (MissingInformation missingInformation : missingInformationList) {
      if (missingInformation.isMissingAssumption()) {
        ...
        resolveRValue(pSMGState, newElement, explicitState,
            missingInformation.getMissingCExpressionInformation(), cfaEdge);
      }
    }
  ...
  private SMGState resolvingAssignment(
      SMGState pSmgState,
      ValueAnalysisState explicitState, MissingInformation pMissingInformation, CFAEdge edge)
      throws CPATransferException {
    ...
    if (memoryLocation == null || memoryLocation.isUnknown()) {
      ...
      if (pMissingInformation.isRequieredInformation()) {
        throw new UnrecognizedCCodeException(
            "Not able to compute allocation size", edge);
      }
  ...
  private SMGSymbolicValue resolveRValue(
      SMGState oldState, SMGState newSmgState,
  ...
  private SMGValueAndStateList resolveFunctionCall(
      SMGState pSmgState,
    ...
    if (builtins.isABuiltIn(functionName)) {
      if (builtins.isConfigurableAllocationFunction(functionName)) {
        SMGAddressValueAndStateList configAllocEdge = builtins
            .evaluateConfigurableAllocationFunction(pIastFunctionCallExpression, pSmgState, pEdge);
        return configAllocEdge;
      }
      if (builtins.isDeallocationFunction(functionName)) {
        List<SMGState> newStates =
      ...
      switch (functionName) {
        ...
        case "__builtin_alloca":
          SMGAddressValueAndStateList allocEdge =
              builtins.evaluateAlloca(pIastFunctionCallExpression, pSmgState, pEdge);
          return allocEdge;
        case "memset":
          SMGAddressValueAndStateList memsetTargetEdge =
              builtins.evaluateMemset(pIastFunctionCallExpression, pSmgState, pEdge);
          return memsetTargetEdge;
        default:
          // nothing to do here
      }
  ...
  private SMGAddress resolveMemoryLocation(
      SMGState pSmgState, ValueAnalysisState pExplicitState,
  ...
  @SuppressWarnings("unused")
  private SMGState resolvingAssumption(
      SMGState pSmgState, ValueAnalysisState pExplicitState,
  ...
  private Long resolveAssumptionValue(
      SMGState pSmgState, ValueAnalysisState pExplicitState,
  ...
  @SuppressWarnings("unused")
  private void checkForMissingRequiredInformation(CFAEdge cfaEdge)
      throws UnrecognizedCCodeException {
    for (MissingInformation missingInformation : missingInformationList) {
      if (missingInformation.isRequieredInformation()) {
        throw new UnrecognizedCCodeException(
            "Not able to compute allocation size", cfaEdge);
      }
    }
  ...
  private class SMGExplicitBuiltIns extends SMGBuiltins {
    ...
    protected SMGAddressValueAndStateList evaluateAddress(
      ...
      ValueAnalysisSMGCommunicator cc =
          new ValueAnalysisSMGCommunicator(explicitState, functionName,
              pState, machineModel, logger, pCfaEdge);

      return SMGAddressValueAndStateList
          .of(SMGAddressValueAndState.of(pState, cc.evaluateSMGAddressExpression(pRvalue)));
    }

    @Override
    protected SMGValueAndStateList evaluateExpressionValue(
        SMGState pSmgState, CFAEdge pCfaEdge,
        CExpression pRValue)
        throws CPATransferException {
      return SMGValueAndStateList.of(SMGValueAndState.of(pSmgState,
          resolveRValue(oldState, pSmgState, explicitState, pRValue, pCfaEdge)));
    }
    ...
    protected List<SMGExplicitValueAndState> evaluateExplicitValue(
      ...
      if (!valueV.isExplicitlyKnown() || !valueV.isNumericValue()) {
        ...
        return ImmutableList.of(SMGExplicitValueAndState.of(pState,
            SMGKnownExpValue.valueOf(valueV.asNumericValue().longValue())));
      }
    }
  }
  ...
  private static class MissingInformation {
    ...
    public MissingInformation(
        CExpression pMissingCLeftMemoryLocation,
    ...
    public MissingInformation(
        SMGAddress pCLeftMemoryLocation,
        CRightHandSide pMissingCExpressionInformation, boolean pRequieredInformation) {
      missingCExpressionInformation = pMissingCExpressionInformation;
      missingCLeftMemoryLocation = null;
      cExpressionValue = null;
  ...
  public static final class SMGUnknownValue
      implements SMGSymbolicValue, SMGExplicitValue, SMGAddressValue {
  ...
  public static final class SMGKnownAddVal extends SMGKnownSymValue implements SMGAddressValue {
    ...
    public static SMGKnownAddVal valueOf(
        SMGObject pObject, SMGKnownExpValue pOffset,
        SMGKnownSymValue pAddress) {
    ...
    public static SMGKnownAddVal valueOf(int pValue, SMGObject object, int offset) {
      return new SMGKnownAddVal(BigInteger.valueOf(pValue),
          SMGKnownAddress.valueOf(object, offset));
    }
  ...
  public static class SMGAddress {
    ...
    public final SMGAddress add(SMGExplicitValue pAddedOffset) {
      ...
      if (object == null || offset.isUnknown()
          || pAddedOffset.isUnknown()) {
        return SMGAddress.UNKNOWN;
      }
  ...
  public Collection<? extends AbstractState> checkAndRefineExpression(
      ...
      throws CPATransferException, InterruptedException {
    return Collections.singleton(pPreState);
  }
  ...
  public Collection<? extends AbstractState> checkAndRefineState(
      ...
      throws CPATransferException, InterruptedException {
    Collection<SMGState> resultStates = new HashSet<>();
    checkerManager.checkState((SMGState) pPostState, pPostOtherStates, pCfaEdge, resultStates);
    return resultStates;
  }
}
