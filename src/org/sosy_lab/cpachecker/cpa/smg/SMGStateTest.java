...
public class SMGStateTest {
  ...
  public void setUp() throws SMGInconsistentException {
    consistent_state =
        new SMGState(logger, MachineModel.LINUX64, true, true, SMGRuntimeCheck.NONE, 0);
    inconsistent_state =
        new SMGState(logger, MachineModel.LINUX64, true, true, SMGRuntimeCheck.NONE, 0);
  ...
  public void WriteReinterpretationTest() throws SMGInconsistentException {
    ...
    SMGEdgeHasValue new_hv =
        state.writeValue(pt.getObject(), 0, mockType16b, newer_value).getNewEdge();
    state.performConsistencyCheck(SMGRuntimeCheck.FORCED);
    values_for_obj = state.getHVEdges(filter);
    Assert.assertEquals(1, values_for_obj.size());
    ...
    SMGEdgeHasValue hv8at0 =
        state.writeValue(pt.getObject(), 0, mockType8b, new_value).getNewEdge();
    state.performConsistencyCheck(SMGRuntimeCheck.FORCED);
    values_for_obj = state.getHVEdges(filter);
    Assert.assertEquals(1, values_for_obj.size());
    ...
    SMGEdgeHasValue hv8at8 =
        state.writeValue(pt.getObject(), 8, mockType8b, new_value).getNewEdge();
    state.performConsistencyCheck(SMGRuntimeCheck.FORCED);
    values_for_obj = state.getHVEdges(filter);
    ...
    SMGEdgeHasValue hv8at4 =
        state.writeValue(pt.getObject(), 4, mockType8b, new_value).getNewEdge();
    state.performConsistencyCheck(SMGRuntimeCheck.FORCED);
    values_for_obj = state.getHVEdges(filter);
    Assert.assertEquals(1, values_for_obj.size());
  ...
  public void WriteReinterpretationNullifiedTest() throws SMGInconsistentException {
    // Empty state
    SMGState state =
        new SMGState(logger, MachineModel.LINUX64, true, true, SMGRuntimeCheck.FORCED, 0);
    state.performConsistencyCheck(SMGRuntimeCheck.FORCED);

    // Add an 16b object and write a 16b zero value into it
    SMGAddressValue pt = state.addNewHeapAllocation(16, "OBJECT");
    SMGEdgeHasValue hv =
        state.writeValue(pt.getObject(), 0, mockType16b, SMGKnownSymValue.ZERO).getNewEdge();
    state.performConsistencyCheck(SMGRuntimeCheck.FORCED);

    // Check the object values and assert it has only the written 16b value
    Set<SMGEdgeHasValue> values_for_obj =
    ...
    SMGEdgeHasValue hv8at4 =
        state.writeValue(pt.getObject(), 4, mockType8b, SMGUnknownValue.getInstance()).getNewEdge();
    state.performConsistencyCheck(SMGRuntimeCheck.FORCED);
    ...
    SMGEdgeHasValueFilter nullFilter =
        SMGEdgeHasValueFilter.objectFilter(pt.getObject()).filterHavingValue(0);
    Set<SMGEdgeHasValue> nulls_for_value = state.getHVEdges(nullFilter);
    Assert.assertEquals(2, nulls_for_value.size());

    Assert.assertEquals(1, state.getHVEdges(
        SMGEdgeHasValueFilter.objectFilter(pt.getObject()).filterHavingValue(0).filterAtOffset(0))
        .size());
    Assert.assertEquals(1, state.getHVEdges(
        SMGEdgeHasValueFilter.objectFilter(pt.getObject()).filterHavingValue(0).filterAtOffset(12))
        .size());
  ...
  public void getPointerFromValueTest() throws SMGInconsistentException {
    ...
    SMGAddressValue pt_obtained =
        Iterables.getOnlyElement(state.getPointerFromValue(pointer).asAddressValueAndStateList())
            .getObject();
