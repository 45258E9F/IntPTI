...
public class SMGConcreteErrorPathAllocator {
  ...
  private ConcreteStatePathNode createMultiEdge(
      SMGState pSMGState, MultiEdge multiEdge,
  ...
  private Map<Address, Object> createHeapValues(
      SMGState pSMGState,
      SMGObjectAddressMap pAdresses) {
    ...
    for (SMGEdgeHasValue hvEdge : symbolicValues) {
      ...
      if (symbolicValue == 0) {
        ...
        try {
          pointer = pSMGState.getPointerFromValue(symbolicValue).asAddressValueAndStateList().get(0)
              .getObject();
        ...
        value = pAdresses
            .calculateAddress(pointer.getObject(), pointer.getOffset().getAsInt(), pSMGState)
            .getAddressValue();
  ...
  private static class SMGObjectAddressMap {
    ...
    public Address calculateAddress(
