...
public class MemoryLocationExtractor {
  ...
  public MemoryLocationExtractor(
      ...
      final CFAEdge pEdge) {
    this(pCompoundIntervalManagerFactory, pMachineModel, pEdge, false,
  ...
  public MemoryLocationExtractor(
      ...
      final Map<? extends MemoryLocation, ? extends NumeralFormula<CompoundInterval>> pEnvironment) {
    this(pCompoundIntervalManagerFactory, pMachineModel,
        pUsePredecessorFunctionName ? pEdge.getPredecessor() : pEdge.getSuccessor(), pEnvironment);
  }
  ...
  private MemoryLocationExtractor(
      ...
      final Map<? extends MemoryLocation, ? extends NumeralFormula<CompoundInterval>> pEnvironment) {
    this(pCompoundIntervalManagerFactory, pMachineModel, pFunctionNode.getFunctionName(),
        pEnvironment);
  }
  ...
  public MemoryLocation getMemoryLocation(AExpression pLhs) throws UnrecognizedCodeException {
    if (pLhs instanceof AIdExpression) {
      ...
      // TODO
      return scope(
          pLhs.toString()); // This actually seems wrong but is currently the only way to deal with some cases of pointer arithmetics
    }
  }
  ...
  private MemoryLocation getFieldReferenceMemoryLocation(
      String pVarName, @Nullable AExpression pOwner,
  ...
  private MemoryLocation getArraySubscriptMemoryLocation(AExpression pOwner, AExpression pSubscript)
    ...
    if (pSubscript instanceof CIntegerLiteralExpression) {
      ...
      return MemoryLocation
          .valueOf(String.format("%s[%d]", getMemoryLocation(pOwner), literal.asLong()).toString());
    }
    final CompoundInterval subscriptValue;
    ExpressionToFormulaVisitor expressionToFormulaVisitor =
        new ExpressionToFormulaVisitor(compoundIntervalManagerFactory, machineModel, this,
            environment);
    if (pSubscript instanceof CExpression) {
      ...
      subscriptValue = compoundIntervalManagerFactory
          .createCompoundIntervalManager(machineModel, pOwner.getExpressionType())
          .allPossibleValues();
    }
    if (subscriptValue.isSingleton()) {
      return MemoryLocation.valueOf(
          String.format("%s[%d]", getMemoryLocation(pOwner), subscriptValue.getValue()).toString());
    }
