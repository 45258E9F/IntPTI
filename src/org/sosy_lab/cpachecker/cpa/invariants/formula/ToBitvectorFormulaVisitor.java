...
public class ToBitvectorFormulaVisitor implements
  ...
  public BitvectorFormula visit(
    ...
    return this.bvfmgr
        .makeVariable(bitVectorInfo.getSize(), pVariable.getMemoryLocation().getAsSimpleString());
  }
      ...
      for (SimpleInterval interval : rightValue.getIntervals()) {
        ...
        if (interval.isSingleton()) {
          ...
          if (interval.hasLowerBound()) {
            ...
            intervalFormula = bfmgr
                .and(intervalFormula, bvfmgr.greaterOrEquals(left, lb, bitVectorInfo.isSigned()));
          }
          if (interval.hasUpperBound()) {
            BitvectorFormula ub = asBitVectorFormula(bitVectorInfo, interval.getUpperBound());
            intervalFormula =
                bfmgr.and(intervalFormula, bvfmgr.lessOrEquals(left, ub, bitVectorInfo.isSigned()));
          }
        }
    ...
    if (operand1 == null || operand2 == null) {
      ...
      if (lessThan) {
        if (rightValue.hasUpperBound()) {
          return bvfmgr
              .lessThan(left, asBitVectorFormula(bitVectorInfo, rightValue.getUpperBound()),
                  bitVectorInfo.isSigned());
        }
      } else if (rightValue.hasLowerBound()) {
        return bvfmgr
            .greaterThan(left, asBitVectorFormula(bitVectorInfo, rightValue.getLowerBound()),
                bitVectorInfo.isSigned());
      }
      return null;
    }
