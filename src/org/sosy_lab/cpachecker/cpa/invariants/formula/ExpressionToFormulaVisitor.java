...
public class ExpressionToFormulaVisitor
   ...
   * @param pVariableNameExtractor          the variable name extractor used to obtain variable
   *                                        names for c id expressions.
   */
  public ExpressionToFormulaVisitor(
      CompoundIntervalManagerFactory pCompoundIntervalManagerFactory,
      MachineModel pMachineModel,
      MemoryLocationExtractor pVariableNameExtractor) {
    this(pCompoundIntervalManagerFactory, pMachineModel, pVariableNameExtractor,
  ...
  public NumeralFormula<CompoundInterval> visit(CIdExpression pCIdExpression)
      throws UnrecognizedCodeException {
    return asVariable(pCIdExpression.getExpressionType(),
  ...
  public NumeralFormula<CompoundInterval> visit(CFieldReference pCFieldReference)
      throws UnrecognizedCodeException {
    return asVariable(pCFieldReference.getExpressionType(),
        this.variableNameExtractor.getMemoryLocation(pCFieldReference));
  }

  @Override
  public NumeralFormula<CompoundInterval> visit(CArraySubscriptExpression pCArraySubscriptExpression)
      throws UnrecognizedCodeException {
    return asVariable(pCArraySubscriptExpression.getExpressionType(),
  ...
  public NumeralFormula<CompoundInterval> visit(CUnaryExpression pCUnaryExpression)
    ...
    BitVectorInfo bitVectorInfo =
        BitVectorInfo.from(machineModel, pCUnaryExpression.getExpressionType());
  ...
  public NumeralFormula<CompoundInterval> visit(CPointerExpression pCPointerExpression)
      throws UnrecognizedCodeException {
    return asVariable(pCPointerExpression.getExpressionType(),
  ...
  public NumeralFormula<CompoundInterval> visit(JCharLiteralExpression pCharLiteralExpression)
      throws UnrecognizedCodeException {
    return asConstant(pCharLiteralExpression.getExpressionType(),
        pCharLiteralExpression.getCharacter());
  }
    ...
    switch (pBinaryExpression.getOperator()) {
      ...
      case LOGICAL_XOR:
        return compoundIntervalFormulaManager.fromBoolean(
            bitVectorInfo,
            compoundIntervalFormulaManager.logicalOr(
                compoundIntervalFormulaManager.logicalAnd(logicalLeft,
                    compoundIntervalFormulaManager.logicalNot(logicalRight)),
                compoundIntervalFormulaManager
                    .logicalAnd(compoundIntervalFormulaManager.logicalNot(logicalLeft),
                        logicalRight)));
      case MINUS:
      ...
      case NOT_EQUALS:
        return compoundIntervalFormulaManager.fromBoolean(
            bitVectorInfo,
            compoundIntervalFormulaManager
                .logicalNot(compoundIntervalFormulaManager.equal(left, right)));
      case PLUS:
                ...
                compoundIntervalFormulaManager.shiftLeft(
                    asConstant(
                        ...
                        compoundIntervalManagerFactory
                            .createCompoundIntervalManager(left.getBitVectorInfo()).singleton(2)),
  ...
  private NumeralFormula<CompoundInterval> truncateShiftOperand(
      ...
      NumeralFormula<CompoundInterval> pOperand) {
    if (pExpressionType instanceof JSimpleType) {
      ...
      if (simpleType.getType() == JBasicType.INT) {
        return compoundIntervalFormulaManager
            .binaryAnd(pOperand, asConstant(pExpressionType, 0x1F));
      } else if (simpleType.getType() == JBasicType.LONG) {
        return compoundIntervalFormulaManager
            .binaryAnd(pOperand, asConstant(pExpressionType, 0x3F));
      }
    }
    return pOperand;
  }
  ...
  public NumeralFormula<CompoundInterval> visit(JUnaryExpression pUnaryExpression)
      throws UnrecognizedCodeException {
    BitVectorInfo bitVectorInfo =
        BitVectorInfo.from(machineModel, pUnaryExpression.getExpressionType());
    switch (pUnaryExpression.getOperator()) {
      ...
      case NOT:
        return compoundIntervalFormulaManager.fromBoolean(
            ...
            compoundIntervalFormulaManager.logicalNot(
                compoundIntervalFormulaManager
                    .fromNumeral(pUnaryExpression.getOperand().accept(this))));
      case PLUS:
  ...
  public NumeralFormula<CompoundInterval> visit(JIntegerLiteralExpression pIntegerLiteralExpression)
      throws UnrecognizedCodeException {
    return asConstant(pIntegerLiteralExpression.getExpressionType(),
        pIntegerLiteralExpression.getValue());
  }

  @Override
  public NumeralFormula<CompoundInterval> visit(JBooleanLiteralExpression pBooleanLiteralExpression)
      throws UnrecognizedCodeException {
    return asConstant(pBooleanLiteralExpression.getExpressionType(),
        pBooleanLiteralExpression.getValue());
  }
  ...
  public NumeralFormula<CompoundInterval> visit(JCastExpression pCastExpression)
    ...
    return compoundIntervalFormulaManager
        .cast(bitVectorInfo, pCastExpression.getOperand().accept(this));
  }
  ...
  public NumeralFormula<CompoundInterval> visit(JIdExpression pIdExpression)
      throws UnrecognizedCodeException {
    return asVariable(pIdExpression.getExpressionType(),
        variableNameExtractor.getMemoryLocation(pIdExpression));
  }
  ...
  public static NumeralFormula<CompoundInterval> handlePotentialOverflow(
    ...
    CompoundIntervalManager cim =
        pCompoundIntervalManagerFactory.createCompoundIntervalManager(bitVectorInfo);
    ...
    return InvariantsFormulaManager.INSTANCE
        .asConstant(bitVectorInfo, cim.union(negativePartResult, nonNegativePartResult));
  }
