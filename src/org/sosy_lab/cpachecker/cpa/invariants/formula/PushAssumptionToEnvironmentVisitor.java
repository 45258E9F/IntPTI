 ...
 */
public class PushAssumptionToEnvironmentVisitor implements
                                                ParameterizedBooleanFormulaVisitor<CompoundInterval, BooleanConstant<CompoundInterval>, Boolean> {
   ...
   * @param pEvaluationVisitor              the evaluation visitor used to evaluate compound state
  ...
  public PushAssumptionToEnvironmentVisitor(
    ...
    this.pushValueToEnvironmentVisitor =
        new PushValueToEnvironmentVisitor(this, pCompoundIntervalManagerFactory, pEvaluationVisitor,
            pEnvironment);
  ...
  private boolean areContradictory(
      ...
      BooleanConstant<CompoundInterval> pConstant) {
  ...
  public Boolean visit(
      ...
      BooleanConstant<CompoundInterval> pParameter) {
    ...
    if (rightValue.isSingleton() && !pEqual.getOperand1()
        .accept(this.pushValueToEnvironmentVisitor, rightValue.invert())) {
      return false;
    }
    if (leftValue.isSingleton() && !pEqual.getOperand2()
        .accept(this.pushValueToEnvironmentVisitor, leftValue.invert())) {
      return false;
    }
    ...
    if (pParameter.getValue()) {
      ...
      leftPushValue = rightValue.isSingleton()
                      ...
                      : (rightValue.hasUpperBound()
                         ? cim.singleton(rightValue.getUpperBound().subtract(BigInteger.ONE))
                             .extendToMinValue()
      ...
      rightPushValue = leftValue.isSingleton()
                       ...
                       : (leftValue.hasLowerBound()
                          ? cim.singleton(leftValue.getLowerBound().add(BigInteger.ONE))
                              .extendToMaxValue()
      ...
      for (Map.Entry<MemoryLocation, NumeralFormula<CompoundInterval>> entry : env2.entrySet()) {
        ...
        if (value1 != null) {
          ...
          final NumeralFormula<CompoundInterval> newValueFormula = compoundIntervalFormulaManager
              .union(value1, value2)
              .accept(new PartialEvaluator(compoundIntervalManagerFactory, this.environment),
                  evaluationVisitor);
          if (newValueFormula.accept(this.evaluationVisitor, this.environment).isBottom()) {
            return false;
          }
          if (newValueFormula instanceof Constant<?>
              && ((Constant<CompoundInterval>) newValueFormula).getValue()
