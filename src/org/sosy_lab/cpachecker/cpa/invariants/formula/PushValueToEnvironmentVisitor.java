 ...
 */
public class PushValueToEnvironmentVisitor
  ...
  public PushValueToEnvironmentVisitor(
    ...
    this.pushAssumptionToEnvironmentVisitor =
        new PushAssumptionToEnvironmentVisitor(this, pCompoundIntervalManagerFactory,
            pEvaluationVisitor, pEnvironment);
  ...
  public Boolean visit(Union<CompoundInterval> pUnion, CompoundInterval pParameter) {
    ...
    NumeralFormula<CompoundInterval> parameter =
        InvariantsFormulaManager.INSTANCE.asConstant(pUnion.getBitVectorInfo(), pParameter);
    ...
    return disjunctiveForm.accept(this.pushAssumptionToEnvironmentVisitor,
  ...
  public Boolean visit(Variable<CompoundInterval> pVariable, CompoundInterval pParameter) {
    ...
    if (newValue.containsAllPossibleValues()) {
      ...
      environment.put(memoryLocation,
          InvariantsFormulaManager.INSTANCE.asConstant(pVariable.getBitVectorInfo(), newValue));
    }
    return true;
  }
  ...
  public Boolean visit(IfThenElse<CompoundInterval> pIfThenElse, CompoundInterval pParameter) {
    ...
    if (positiveCaseIntersection.isBottom()) {
      if (!conditionFormula.accept(pushAssumptionToEnvironmentVisitor,
          BooleanConstant.<CompoundInterval>getFalse())) {
        return false;
      }
    }
      ...
      if (sourceInfo.isSigned()) {
        originFactors =
            sourceManager.add(
                originFactors,
                sourceManager
                    .singleton(numberOfPotentialOrigins.divide(BigInteger.valueOf(2).negate())));
      }
  ...
  private NumeralFormula<CompoundInterval> getFromEnvironment(Variable<CompoundInterval> pVariable) {
    ...
    if (result == null) {
      return InvariantsFormulaManager.INSTANCE.asConstant(pVariable.getBitVectorInfo(),
          getCompoundIntervalManager(pVariable).allPossibleValues());
    }
    return result;
  }
