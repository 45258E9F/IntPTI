...
public enum ISIOperatorFactory {
  ...
  public Operator<BitVectorInterval, BigInteger, BitVectorInterval> getAdd(
    ...
    return new Operator<BitVectorInterval, BigInteger, BitVectorInterval>() {
      ...
      public BitVectorInterval apply(BitVectorInterval pFirstOperand, BigInteger pSecondOperand) {
        ...
        return BitVectorInterval
            .cast(pFirstOperand.getBitVectorInfo(), lowerBound, upperBound, pAllowSignedWrapAround,
    ...
    return new Operator<BitVectorInterval, BigInteger, BitVectorInterval>() {
      ...
      public BitVectorInterval apply(BitVectorInterval pFirstOperand, BigInteger pSecondOperand) {
        ...
        if (pSecondOperand.equals(BigInteger.ZERO)) {
          return BitVectorInterval
              .cast(pFirstOperand.getBitVectorInfo(), BigInteger.ZERO, pAllowSignedWrapAround,
                  pOverflowEventHandler);
        }
        /*
        ...
        if (pSecondOperand.signum() < 0) {
          return apply(pFirstOperand.negate(pAllowSignedWrapAround, pOverflowEventHandler),
  ...
  public Operator<BitVectorInterval, BigInteger, BitVectorInterval> getDivide(
    ...
    return new Operator<BitVectorInterval, BigInteger, BitVectorInterval>() {
      ...
      public
      @Nullable
      BitVectorInterval apply(BitVectorInterval pFirstOperand, BigInteger pSecondOperand) {
         ...
         */
        if (pSecondOperand.equals(BigInteger.ONE) || pFirstOperand.isSingleton() && pFirstOperand
            .containsZero()) {
          return pFirstOperand;
        }
    ...
    return new Operator<BitVectorInterval, BigInteger, BitVectorInterval>() {
      ...
      public BitVectorInterval apply(BitVectorInterval pFirstOperand, BigInteger pSecondOperand) {
        ...
        if (pFirstOperand.isSingleton()) {
          return BitVectorInterval.cast(pFirstOperand.getBitVectorInfo(),
              pFirstOperand.getLowerBound().remainder(pSecondOperand), pAllowSignedWrapAround,
              pOverflowEventHandler);
        }
        ...
        if (pFirstOperand.containsNegative()) {
          ...
          if (pFirstOperand.containsZero()) {
            negPart = BitVectorInterval
                .cast(pFirstOperand.getBitVectorInfo(), pFirstOperand.getLowerBound(),
                    BigInteger.ZERO, pAllowSignedWrapAround, pOverflowEventHandler);
          } else {
        ...
        if (pFirstOperand.containsPositive()) {
           ...
           * from (divisor-1) to zero
           */
          BitVectorInterval posRange = BitVectorInterval
           ...
           */
          if (pFirstOperand.containsZero()) {
            posPart = BitVectorInterval.cast(pFirstOperand.getBitVectorInfo(), BigInteger.ZERO,
                pFirstOperand.getUpperBound(), pAllowSignedWrapAround, pOverflowEventHandler);
          } else {
  ...
  public Operator<BitVectorInterval, BigInteger, BitVectorInterval> getShiftLeft(
    ...
    return new Operator<BitVectorInterval, BigInteger, BitVectorInterval>() {
      ...
      public BitVectorInterval apply(BitVectorInterval pFirstOperand, BigInteger pSecondOperand) {
         ...
         */
        if (pSecondOperand.compareTo(BigInteger.valueOf(pFirstOperand.getBitVectorInfo().getSize()))
            <= 0) {
