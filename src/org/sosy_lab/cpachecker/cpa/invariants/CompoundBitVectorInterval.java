...
public class CompoundBitVectorInterval implements CompoundInterval, BitVectorType {
   ...
   *                   {@code null}. All intervals must have the same bit vector information as the
   *                   parameter.
   */
  ...
  public List<SimpleInterval> getIntervals() {
    return Lists
        .transform(getBitVectorIntervals(), new Function<BitVectorInterval, SimpleInterval>() {

          @Override
          public SimpleInterval apply(BitVectorInterval pBitVectorInterval) {
            return SimpleInterval
                .of(pBitVectorInterval.getLowerBound(), pBitVectorInterval.getUpperBound());
          }
  ...
  public List<CompoundBitVectorInterval> splitIntoIntervals() {
    return Lists.transform(Arrays.asList(this.intervals),
        new Function<BitVectorInterval, CompoundBitVectorInterval>() {
  ...
  public CompoundBitVectorInterval cast(
    ...
    if (pBitVectorInfo.getRange().contains(info.getRange())) {
      ...
      Lists
          .transform(getBitVectorIntervals(), new Function<BitVectorInterval, BitVectorInterval>() {
  ...
  public static CompoundBitVectorInterval cast(
    ...
    if (pLowerBound.equals(pUpperBound)) {
      return of(BitVectorInterval
          .cast(pInfo, pLowerBound, pAllowSignedWrapAround, pOverflowEventHandler));
    }
    ...
    if (ubExceedsBelow) { // Full interval is below the minimum value
      ...
      if (lowerBound.compareTo(pInfo.getMinValue()) < 0) {
        return union(
            ...
            cast(pInfo, lowerBound, pInfo.getMinValue().subtract(BigInteger.ONE),
  ...
  public CompoundBitVectorInterval invert() {
    ...
    if (!current.getLowerBound().equals(info.getMinValue())) {
      result = result.unionWith(BitVectorInterval
          .of(info, info.getMinValue(), current.getLowerBound().subtract(BigInteger.ONE)));
    }
    ...
    while (index < this.intervals.length) {
      ...
      result = result.unionWith(BitVectorInterval.of(info, lastUpperBound.add(BigInteger.ONE),
  ...
  public CompoundBitVectorInterval negate(
    ...
    for (BitVectorInterval simpleInterval : this.intervals) {
      result = result
          .unionWith(negate(info, simpleInterval, pAllowSignedWrapAround, pOverflowEventHandler));
    }
    return result;
  }
  ...
  public CompoundBitVectorInterval binaryAnd(
    ...
    if (!result.isSingleton()) {
      ...
      if (smallestUpperBound == null) {
        ...
        range = CompoundBitVectorInterval
            .of(BitVectorInterval.of(info, BigInteger.ZERO, smallestUpperBound));
      }
  ...
  public static CompoundBitVectorInterval logicalAnd(
      CompoundBitVectorInterval p1,
      CompoundBitVectorInterval p2) {
  ...
  private <T> CompoundBitVectorInterval applyOperationToAllAndUnite(
      Operator<BitVectorInterval, T, CompoundBitVectorInterval> pOperator,
      T pOperand) {
    CompoundBitVectorInterval result = bottom(info);
    for (BitVectorInterval interval : this.intervals) {
   ...
   * @return <code>true</code> if <code>pFirst</code> is less than <code>pSecond</code>,
   * <code>false</code> otherwise.
   */
  ...
  public static CompoundBitVectorInterval span(
      CompoundBitVectorInterval pLeftValue,
      CompoundBitVectorInterval pRightValue) {
  ...
  private static CompoundBitVectorInterval getZeroToOne(BitVectorInfo pInfo) {
    return CompoundBitVectorInterval
        .of(BitVectorInterval.of(pInfo, BigInteger.ZERO, BigInteger.ONE));
  }
