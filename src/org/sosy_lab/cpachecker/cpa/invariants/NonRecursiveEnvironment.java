...
public class NonRecursiveEnvironment
  ...
  private PersistentSortedMap<MemoryLocation, NumeralFormula<CompoundInterval>> sanitizedInnerPutAndCopy(
  ...
  private PersistentSortedMap<MemoryLocation, NumeralFormula<CompoundInterval>> sanitizedInnerPutAndCopyInternal(
      PersistentSortedMap<MemoryLocation, NumeralFormula<CompoundInterval>> pTarget,
      MemoryLocation pMemoryLocation, NumeralFormula<CompoundInterval> pValue) {
    if (isConstantAndContainsAllPossibleValues(pValue)) {
      return pTarget;
    }
    Preconditions.checkArgument(!pTarget.containsKey(pMemoryLocation),
        "Variable must be TOP in previous environment");
    NumeralFormula<CompoundInterval> previous = pTarget.get(pMemoryLocation);
    if (pValue.equals(previous)) {
      return pTarget;
    }
  ...
  public NonRecursiveEnvironment putAndCopyAll(Map<? extends MemoryLocation, ? extends NumeralFormula<CompoundInterval>> pM) {
    ...
    for (java.util.Map.Entry<? extends MemoryLocation, ? extends NumeralFormula<CompoundInterval>> entry : pM
        .entrySet()) {
  ...
  public NonRecursiveEnvironment removeAndCopy(Object pKey) {
    ...
    return new NonRecursiveEnvironment(this.compoundIntervalManagerFactory,
        this.inner.removeAndCopy(pKey));
  }
  ...
  public static NonRecursiveEnvironment of(CompoundIntervalManagerFactory pCompoundIntervalManagerFactory) {
    return new NonRecursiveEnvironment(pCompoundIntervalManagerFactory,
  ...
  public static NonRecursiveEnvironment copyOf(
    ...
    if (pInner instanceof NonRecursiveEnvironment
        && ((NonRecursiveEnvironment) pInner).compoundIntervalManagerFactory
        .equals(pCompoundIntervalManagerFactory)) {
  ...
  public static class Builder implements Map<MemoryLocation, NumeralFormula<CompoundInterval>> {
    ...
    public static Builder of(
      ...
      if (pTmpEnvironment instanceof Builder
          && ((Builder) pTmpEnvironment).current.compoundIntervalManagerFactory
          .equals(pCompoundIntervalManagerFactory)) {
