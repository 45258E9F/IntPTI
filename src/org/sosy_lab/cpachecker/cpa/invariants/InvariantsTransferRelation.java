...
class InvariantsTransferRelation extends SingleEdgeTransferRelation {
  ...
  public Collection<? extends AbstractState> getAbstractSuccessorsForEdge(
      AbstractState pState, List<AbstractState> otherStates, Precision pPrecision, CFAEdge pEdge)
  ...
  private InvariantsState getSuccessor(
      ...
      InvariantsState pState) throws UnrecognizedCFAEdgeException, UnrecognizedCodeException {
  ...
  private InvariantsState handleAssume(
      ...
      AssumeEdge pEdge,
      ExpressionToFormulaVisitor pExpressionToFormulaVisitor) throws UnrecognizedCodeException {
  ...
  private InvariantsState handleFunctionCall(
      final InvariantsState pElement,
      final CFunctionCallEdge pEdge) throws UnrecognizedCodeException {
    ...
    ExpressionToFormulaVisitor actualParamExpressionToFormulaVisitor =
        getExpressionToFormulaVisitor(
            new MemoryLocationExtractor(compoundIntervalManagerFactory, machineModel, pEdge, true,
                pElement.getEnvironment()), pElement);
  ...
  private CompoundInterval evaluate(
    ...
    return pFormula
        .accept(new FormulaCompoundStateEvaluationVisitor(compoundIntervalManagerFactory),
            pEnvironment);
  }
  ...
  private NumeralFormula<CompoundInterval> toConstant(
    ...
    return InvariantsFormulaManager.INSTANCE
        .asConstant(pFormula.getBitVectorInfo(), evaluate(pFormula, pEnvironment));
  }
  ...
  private InvariantsState handleStatement(
    ...
    if (pEdge.getStatement() instanceof CFunctionCall) {
      CExpression fn = ((CFunctionCall) pEdge.getStatement()).getFunctionCallExpression()
        ...
        if (functionNameExpression instanceof CIdExpression) {
          ...
          if (idExpression.getName().equals("__VERIFIER_nondet_uint")) {
            BitVectorInfo bitVectorInfo =
                BitVectorInfo.from(machineModel, leftHandSide.getExpressionType());
  ...
  private InvariantsState handleAssignment(
    ...
    MemoryLocationExtractor variableNameExtractor =
        new MemoryLocationExtractor(compoundIntervalManagerFactory, machineModel, pEdge,
  ...
  private NumeralFormula<CompoundInterval> handlePotentialOverflow(
      ...
      NumeralFormula<CompoundInterval> pFormula,
      CType pType) {
    return ExpressionToFormulaVisitor
        .handlePotentialOverflow(compoundIntervalManagerFactory, pFormula, machineModel, pType,
            pState.getEnvironment());
  }

  private InvariantsState handleReturnStatement(
      InvariantsState pElement,
      CReturnStatementEdge pEdge) throws UnrecognizedCodeException {
    ...
    MemoryLocation returnValueName = MemoryLocation
        .valueOf(pEdge.getSuccessor().getEntryNode().getReturnVariable().get().getQualifiedName());
  ...
  private InvariantsState handleFunctionReturn(
    ...
    if (expression instanceof CFunctionCallAssignmentStatement) {
      ...
      if (var.isPresent()) {
        ...
        result = handleAssignment(pElement, pFunctionReturnEdge, funcExp.getLeftHandSide(), value,
            pPrecision);
      }
    } else {
      Iterator<CExpression> actualParamIterator =
          summaryEdge.getExpression().getFunctionCallExpression().getParameterExpressions()
              .iterator();
      for (String formalParamName : pFunctionReturnEdge.getPredecessor().getEntryNode()
        ...
        if (actualParamFormula instanceof Variable) {
          ...
          for (Entry<? extends MemoryLocation, ? extends NumeralFormula<CompoundInterval>> entry : pElement
            ...
            if (varName.startsWith(formalParamPrefixDeref)) {
              ...
              result = result
                  .assign(MemoryLocation.valueOf(actualParamName + "->" + formalParamSuffix),
                      entry.getValue());
    ...
    result = result.clearAll(new Predicate<MemoryLocation>() {
      ...
      public boolean apply(MemoryLocation pMemoryLocation) {
        return pMemoryLocation.isOnFunctionStack() && pMemoryLocation.getFunctionName()
            .equals(calledFunctionName);
      }
  ...
  public Collection<? extends AbstractState> strengthen(
    ...
    for (AbstractStateWithAssumptions assumptionState : FluentIterable.from(pOtherElements)
        .filter(AbstractStateWithAssumptions.class)) {
      for (AssumeEdge edge : assumptionState
          .getAsAssumeEdges(pCfaEdge.getSuccessor().getFunctionName())) {
    ...
    if (edge instanceof MultiEdge) {
      for (CFAEdge subEdge : ((MultiEdge) edge)) {
        ...
        Collection<? extends AbstractState> next =
            strengthen(state, pOtherElements, subEdge, pPrecision);
        if (next == null) {
          state = current;
    ...
    if (leftHandSide instanceof CPointerExpression
        || leftHandSide instanceof CFieldReference && ((CFieldReference) leftHandSide)
          ...
          if (hasArrow || hasDot) {
            ...
            Iterable<? extends MemoryLocation> targets =
                FluentIterable.from(result.getEnvironment().keySet())
                    .filter(new Predicate<MemoryLocation>() {

                      @Override
                      public boolean apply(MemoryLocation pVar) {
                        return pVar != null && (pVar.getIdentifier().endsWith("." + end) || pVar
                            .getIdentifier().endsWith("->" + end));
                      }
  ...
  private ExpressionToFormulaVisitor getExpressionToFormulaVisitor(
      final CFAEdge pEdge,
      final InvariantsState pState) {
    return getExpressionToFormulaVisitor(
        new MemoryLocationExtractor(compoundIntervalManagerFactory, machineModel, pEdge,
            pState.getEnvironment()), pState);
  }

  private ExpressionToFormulaVisitor getExpressionToFormulaVisitor(
      final MemoryLocationExtractor pVariableNameExtractor,
      final InvariantsState pState) {
    return new ExpressionToFormulaVisitor(compoundIntervalManagerFactory, machineModel,
        pVariableNameExtractor, pState.getEnvironment());
  }
