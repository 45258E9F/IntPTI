...
public class CompoundMathematicalIntervalTest {
  ...
  public void testUnionWith() {
    assertTrue(CompoundMathematicalInterval.of(negInfToZeroInterval).unionWith(zeroToPosInfInterval)
        .isTop());
    assertTrue(CompoundMathematicalInterval.of(negInfToZeroInterval)
        .unionWith(CompoundMathematicalInterval.of(zeroToPosInfInterval)).isTop());
    assertTrue(CompoundMathematicalInterval.bottom().unionWith(CompoundMathematicalInterval.top())
        .isTop());
    assertFalse(CompoundMathematicalInterval.of(negInfToZeroInterval)
        .unionWith(CompoundMathematicalInterval.of(oneToTenInterval)).isTop());
    assertEquals(CompoundMathematicalInterval.of(oneToFiveInterval).unionWith(sixToTenInterval),
        CompoundMathematicalInterval.of(oneToTenInterval));
    assertEquals(CompoundMathematicalInterval.of(oneToTenInterval).unionWith(sixToTenInterval),
        CompoundMathematicalInterval.of(oneToTenInterval));
    SimpleInterval zero = SimpleInterval.singleton(BigInteger.ZERO);
    assertEquals(1, CompoundMathematicalInterval.of(zero.extendToNegativeInfinity())
        .unionWith(zero.extendToPositiveInfinity()).getIntervals().size());
    ...
    CompoundMathematicalInterval stepsNegInf = steps.unionWith(
        CompoundMathematicalInterval.singleton(BigInteger.valueOf(-4)).extendToMinValue());
    for (int i = -6; i <= 6; i += 2) {
      assertTrue(stepsNegInf.contains(i));
    }

    CompoundMathematicalInterval zeroOrTenToInf = CompoundMathematicalInterval.singleton(0)
    ...
    assertEquals(CompoundMathematicalInterval
            .of(SimpleInterval.of(BigInteger.valueOf(-1), BigInteger.valueOf(1))),
        negOne.unionWith(CompoundMathematicalInterval.one()).unionWith(zero));
    assertEquals(1,
        negOne.unionWith(CompoundMathematicalInterval.one()).unionWith(zero).getIntervals().size());
  }
  ...
  public void testIntersectWith() {
    ...
    assertTrue(
        CompoundMathematicalInterval.bottom().intersectWith(CompoundMathematicalInterval.top())
            .isBottom());
    assertEquals(CompoundMathematicalInterval.of(oneToTenInterval),
    ...
    assertEquals(CompoundMathematicalInterval.singleton(1),
        notTwo.intersectWith(CompoundMathematicalInterval.of(oneToTwo)));
  }

  @Test
  public void testInvert() {
    assertEquals(CompoundMathematicalInterval.bottom(),
        CompoundMathematicalInterval.top().invert());
    assertEquals(CompoundMathematicalInterval.top(),
        CompoundMathematicalInterval.bottom().invert());
    ...
    assertEquals(CompoundMathematicalInterval.singleton(0).extendToMinValue()
            .unionWith(CompoundMathematicalInterval.singleton(6)),
        CompoundMathematicalInterval.of(SimpleInterval.of(BigInteger.ONE, BigInteger.valueOf(5)))
            .unionWith(CompoundMathematicalInterval.singleton(7).extendToMaxValue()).invert());
  }
  ...
  public void moduloTest() {
    ...
    CompoundMathematicalInterval zeroToThree = CompoundMathematicalInterval.of(zeroToThreeInterval);
    CompoundMathematicalInterval six = CompoundMathematicalInterval.singleton(6);
    SimpleInterval sixToTenInterval =
        SimpleInterval.of(BigInteger.valueOf(6), BigInteger.valueOf(10));
    ...
    assertEquals(zeroToThree.unionWith(six).negate(),
        sixToTen.negate().modulo(BigInteger.valueOf(7)));
    assertEquals(zeroToThree.unionWith(six), sixToTen.modulo(BigInteger.valueOf(7).negate()));
    assertEquals(zeroToThree.unionWith(six).negate(),
        sixToTen.negate().modulo(BigInteger.valueOf(7).negate()));
  }

  @Test
  public void testNegate() {
    assertEquals(CompoundMathematicalInterval.singleton(-1),
        CompoundMathematicalInterval.one().negate());
    CompoundMathematicalInterval twoToFour = CompoundMathematicalInterval
        .of(SimpleInterval.of(BigInteger.valueOf(2), BigInteger.valueOf(4)));
    CompoundMathematicalInterval negTwoToNegOne = CompoundMathematicalInterval
        .of(SimpleInterval.of(BigInteger.valueOf(-2), BigInteger.valueOf(-1)));
    CompoundMathematicalInterval negFourToNegTwo = CompoundMathematicalInterval
        .of(SimpleInterval.of(BigInteger.valueOf(-4), BigInteger.valueOf(-2)));
  ...
  public void testIsSingleton() {
    ...
    assertFalse(
        CompoundMathematicalInterval.span(CompoundMathematicalInterval.one(), ten).isSingleton());
    assertFalse(zero.unionWith(ten).isSingleton());
    assertFalse(
        negOne.unionWith(CompoundMathematicalInterval.span(CompoundMathematicalInterval.one(), ten))
            .isSingleton());
  }
  ...
  public void containsTest() {
    ...
    assertTrue(CompoundMathematicalInterval.singleton(-1)
        .contains(CompoundMathematicalInterval.singleton(-1)));
    assertTrue(CompoundMathematicalInterval.singleton(0)
        .contains(CompoundMathematicalInterval.singleton(0)));
    assertTrue(CompoundMathematicalInterval.singleton(1)
        .contains(CompoundMathematicalInterval.singleton(1)));
    assertTrue(CompoundMathematicalInterval.of(SimpleInterval.of(BigInteger.ZERO, BigInteger.TEN))
        .contains(
            CompoundMathematicalInterval.of(SimpleInterval.of(BigInteger.ONE, BigInteger.TEN))));
    assertFalse(CompoundMathematicalInterval.of(SimpleInterval.of(BigInteger.ONE, BigInteger.TEN))
        .contains(
            CompoundMathematicalInterval.of(SimpleInterval.of(BigInteger.ZERO, BigInteger.TEN))));
    assertTrue(CompoundMathematicalInterval.of(SimpleInterval.of(BigInteger.ZERO, BigInteger.TEN))
        .contains(5));
    assertFalse(CompoundMathematicalInterval.of(SimpleInterval.of(BigInteger.ZERO, BigInteger.TEN))
        .contains(-1));
    assertFalse(
        CompoundMathematicalInterval.of(SimpleInterval.of(BigInteger.ZERO, BigInteger.valueOf(4)))
            .unionWith(SimpleInterval.of(BigInteger.valueOf(6), BigInteger.TEN)).contains(5));
  }
  ...
  public void testAbsolute() {
    ...
    assertEquals(CompoundMathematicalInterval.one(),
        CompoundMathematicalInterval.one().negate().absolute());
    CompoundMathematicalInterval twoToFour = CompoundMathematicalInterval
        .of(SimpleInterval.of(BigInteger.valueOf(2), BigInteger.valueOf(4)));
    CompoundMathematicalInterval negTwoToNegOne = CompoundMathematicalInterval
        .of(SimpleInterval.of(BigInteger.valueOf(-2), BigInteger.valueOf(-1)));
