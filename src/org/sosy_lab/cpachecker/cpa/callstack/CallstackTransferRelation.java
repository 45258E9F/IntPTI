...
public class CallstackTransferRelation extends SingleEdgeTransferRelation {
  ...
  protected int recursionBoundDepth = 10;

  @Option(secure = true, name = "analyzeSummary",
      description = "whether summaryEdge should be used")
  protected boolean analyzeSummary = false;
  ...
  public Collection<? extends AbstractState> getAbstractSuccessorsForEdge(
    ...
    switch (pEdge.getEdgeType()) {
      case StatementEdge: {
        ...
        if (pEdge instanceof CFunctionSummaryStatementEdge) {
          if (!shouldGoByFunctionSummaryStatement(e, (CFunctionSummaryStatementEdge) pEdge)) {
            ...
            // TODO: if the depth reaches the threshold, we should return the successor here
            if (analyzeSummary) {
      ...
      case AssumeEdge: {
        ...
        boolean isSuccessorAritificialPCNode = succFunction
            .equals(CFASingleLoopTransformation.ARTIFICIAL_PROGRAM_COUNTER_FUNCTION_NAME);
        boolean isPredecessorAritificialPCNode = predFunction
            .equals(CFASingleLoopTransformation.ARTIFICIAL_PROGRAM_COUNTER_FUNCTION_NAME);
      ...
      case FunctionCallEdge: {
        ...
        if (hasRecursion(e, calledFunction)) {
          if (skipRecursiveFunctionCall(e, (FunctionCallEdge) pEdge)) {
            ...
            // recursion is actually supported, since {@code hasRecursion} returns true when
            // the recursion level exceeds the specified bound
      ...
      case FunctionReturnEdge: {
        ...
        if (isWildcardState(e)) {
          ...
          assert callerFunction.equals(returnElement.getCurrentFunction()) || isWildcardState(
              returnElement);
        }
  ...
  protected boolean isWildcardState(final CallstackState pState) {
    ...
    if (CFAUtils.successorsOf(pState.getCallNode()).filter(FunctionEntryNode.class)
        .anyMatch(new Predicate<FunctionEntryNode>() {
  ...
  protected boolean skipRecursiveFunctionCall(
    ...
    if (leavingEdges(callEdge.getPredecessor()).filter(CFunctionSummaryStatementEdge.class)
        .isEmpty()) {
      return false;
    }
  ...
  protected boolean shouldGoByFunctionSummaryStatement(
      CallstackState element,
      CFunctionSummaryStatementEdge sumEdge) {
